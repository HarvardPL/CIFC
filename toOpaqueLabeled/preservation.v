...
- pose proof (exclude_middle_val_eq v1 v2).
  assert (valid_syntax result).
  destruct H5; auto. apply H1 in H5. subst; auto.
  auto. apply H2 in H5. subst; auto.
  assert (hole_free result = true).
  destruct H5; auto. apply H1 in H5. subst; auto. 
  apply H2 in H5. subst; auto.
  apply valid_conf; auto.
    match goal with
    | H : valid_ctn _ _ _ |- _
      => inversion H; subst; auto 
...
(*newly added rule for field write opaque call context*)
  ...
  - inversion H17; subst; auto.
   ...
   - inversion H20; subst; auto.
     ...
     destruct H3 as [cls0].
     destruct H3 as [F0].
     destruct H3 as [lo0]. destruct H3; subst; auto.
   ...
   -  match goal with
    | H : valid_ctn _ _ _ |- _
      ...
      rewrite H12. rewrite H14. rewrite H17. auto. 
...
-  inversion H_typing; subst; auto.
  ...
  - inversion H_typing; subst; auto.
    ...
    inversion H4; subst; auto.
    inversion H9; subst; auto.
    apply T_ctn_type with (classTy cls') T2; auto.
    inversion H3; subst; auto.   
...
-  inversion H_typing; subst; auto.
   ...
   + inversion H8; subst; auto.
     ...
     assert ( tm_hole_has_type ct Gamma' h' (MethodCall hole meth e2)
                               (ArrowTy (classTy T0) (OpaqueLabeledTy (classTy returnT)))).
     eauto using tm_hole_has_type.
     ...
     ++ apply T_fs_hole with (OpaqueLabeledTy (classTy returnT))
          ...
          intros. split; auto.
          intro contra; inversion contra.
          assert ( T2 = (OpaqueLabeledTy (classTy returnT))); auto; subst; auto.
   ...
   + inversion H8; subst; auto.
     rename id into meth.
     assert ( tm_hole_has_type ct Gamma' h' (MethodCall hole meth e2)
                               (ArrowTy (classTy T3) (OpaqueLabeledTy (classTy returnT)))).
     eauto using tm_hole_has_type.          
...
(*(MethodCall t id0 e2) *)
  - inversion H_typing; subst; auto. 
    ...
    inversion H6.
    inversion H19; subst; auto.
    apply T_ctn_type with (OpaqueLabeledTy (classTy returnT)) T4; auto.
...
(*Method call normal *)
  ...
  - inversion H_typing; subst; auto.
    ...
    rewrite <- H5 in H14. inversion H14; subst; auto.
    ...
    intros.
    case_eq (beq_id arg_id0 x);intro;
      unfold sf_update. rewrite H16.
    exists v; split; auto.
    subst; auto. unfold update_typing in H4; rewrite H16 in H4.
    inversion H4; subst; auto. 
...
(*Method call opaque call *)
  - inversion H_typing; subst; auto.  
    ...
    case_eq (beq_id arg_id0 x);intro;
      unfold sf_update. rewrite H16.
    exists v; split; auto.
    subst; auto. unfold update_typing in H1; rewrite H16 in H1.
    inversion H1; subst; auto.        
    apply value_typing_invariant_gamma with Gamma';auto.
...
- inversion H_typing; subst; auto.
  ...
  inversion H8; subst; auto.
  inversion H9; subst; auto.
  + assert (tm_hole_has_type ct Gamma' h' (labelOf hole) (ArrowTy (LabelelTy T3) LabelTy)).
    eauto using tm_hole_has_type.          
...
- inversion H_typing; subst; auto. 
  ...
  destruct H17 with (labelOf hole) fs; auto.
  subst; auto.
  eauto using tm_has_type. 
  intros. destruct H21 with top fs' ; auto.
  subst; auto.
...
- inversion H_typing; subst; auto.
  ...
  inversion H8; subst; auto.
  inversion H9; subst; auto.
  + assert (tm_hole_has_type ct Gamma' h' (unlabelOpaque hole) (ArrowTy (OpaqueLabeledTy T1) T1)).
    eauto using tm_hole_has_type.          
    apply T_ctn_type with (OpaqueLabeledTy T1) (OpaqueLabeledTy T1) ; auto.
    destruct fs.          
    ++ apply T_fs_hole with T1 T1; auto.
       assert ( T2 = T1); auto; subst; auto.
    ++ case_eq (hole_free t); intro.           
...
- inversion H_typing; subst; auto. 
  ...
  inversion H19; subst; auto.
  apply T_ctn_type with T' T4; auto.
  destruct H17 with (unlabelOpaque hole) fs; auto.
  subst; auto.
  intros. destruct H21 with top fs' ; auto.
...
(*v_opa_l t lb0*)
  - inversion H_typing; subst; auto.
    ...
    + inversion H9; subst; auto.
      ...
      apply T_ctn_type with (OpaqueLabeledTy T1) T5; auto.
      ...
      intros.
      destruct H24 with top fs'0; auto.
      subst; auto.
      inversion H15; subst; auto.
      assert (T0 = T1); auto; subst; auto.
      split; auto.
...
(* toOpaqueLabeled *)
  - inversion H_typing; subst; auto.
    inversion H2; subst; auto.
    inversion H7;subst; auto. 
    apply T_config_ctns with T3 Gamma'; auto.
    apply T_ctn_type with T3
                           T3; auto.
    apply T_ctn_list with  T0 Gamma'; auto.
    apply T_ctn_caller with 
        T2; auto.
    intros. split; auto. apply H16 in H; auto.
    apply H.
