...
Inductive ctn_list_has_type : Class_table -> typing_context -> heap -> list container -> ty -> Prop := 
  ...
  | T_ctn_list : forall h Gamma CT T T' ctn ctns' fs lb sf Gamma' T0 top T1, 
      ...
      ctn = (Container hole (top :: fs) lb sf) ->
...
Inductive config_has_type : Class_table -> typing_context -> heap -> config -> ty -> Prop :=
  ...
  | T_config_ctns : forall h Gamma CT T T' ctn ctns Gamma' fs lb sf, 
      ctn_has_type CT Gamma' h ctn T ->
      ctn_list_has_type CT Gamma h ((Container hole fs lb sf) :: ctns) (ArrowTy T T') ->
      config_has_type CT Gamma h (Config CT ctn ((Container hole fs lb sf) :: ctns) h) T'
...
-   inversion H_config. inversion H7. destruct H19. inversion H21. 
Qed. 
...
Lemma field_access_erasure_L_fun : forall t2 fs1 lb2 sf2 ctns_stack h2 f ct , 
...
remember ctns_stack as ctns. 
  ...
  destruct ctns; try (induction c); unfold erasure; unfold erasure_fun;  rewrite H_flow;
 ...
 pose proof (multi_erasure_heap_equal h2) as H_h.  
  ...
  destruct ctns; try (induction c); unfold erasure; unfold erasure_fun;  rewrite H_flow; rewrite H0;  
  rewrite H_h;rewrite Ht2; auto. 
    rewrite <- Heqctns in H0. 
  apply L_reduction with (Config ct
          (Container (erasure_L_fun t2) (FieldAccess hole f :: fs1) lb2
             (fun x : id => erasure_obj_null (sf2 x) h2)) ctns
...
Lemma fs_pro : forall (t : tm) (fs : list tm),
  t :: fs <> fs.
Proof.  induction fs. intro contra. inversion contra.  intro contra.  inversion contra. 
subst. rewrite H1 in IHfs. auto. Qed. 

Lemma fs_pro_false : forall (t : tm) (fs : list tm),
  fs = t :: fs-> False.
Proof.  induction fs. intro contra. inversion contra.  intro contra.  inversion contra. 
subst. rewrite H1 in IHfs. auto. Qed. 
...
Lemma erasure_L_fun_sub_exp_pop : forall t1 fs lb sf t1'  h ctns_stack ct t2, 
  valid_syntax t1->
  flow_to lb L_Label = true ->
 (~ value (erasure_L_fun t2)) ->
 Config ct (Container t1 fs lb sf) ctns_stack h 
      ==> Config ct (Container t2 (t1' :: fs) lb sf) ctns_stack h ->
  exp_with_hole t1' -> hole_free t2 = true ->
  erasure_fun ct (Container t1 fs lb sf) ctns_stack h  ==l>
  erasure_fun ct (Container t2 (t1' :: fs) lb sf) ctns_stack h.
Proof with eauto. intros t1 fs lb sf t1'  h ctns_stack ct t2. 
intro H_valid_syntax.  intro H_flow. intro Hy. 
  intro H_reduction.  intro H_hole_exp. intro H_hole_t2.
remember ctns_stack as ctns. 
assert (forall a, (fun x : id =>
 ...
 pose proof (multi_erasure_heap_equal h) as H_h.  
  ...
  assert (erasure_fun ct (Container t1 fs lb sf) ctns h ==>
    Config ct
    (Container (erasure_L_fun t2) (t1' :: fs) lb
       (fun x : id => erasure_obj_null (sf x) h)) ctns (erasure_heap h) ).
  destruct ctns; try (induction c); unfold erasure; unfold erasure_fun;  rewrite H_flow.
  remember t1' as tmp2; rewrite Heqtmp2 in H_hole_exp;
  inversion H_hole_exp; rewrite Heqtmp2 in H_reduction; rewrite <- H0 in H_reduction;

  inversion H_reduction; try (apply fs_pro_false in H10; intuition;fail);
  try (subst; apply fs_pro_double_false in H10; intuition);
  try (subst; auto;   try (eauto using reduction));
  try (subst; inversion H_valid_syntax; inversion H3; fail);
  try (subst; inversion H_valid_syntax; inversion H3; case_eq (surface_syntax x); intro; rewrite H4 in H5; 
  inversion H5; fail); 
  try (subst; inversion H_valid_syntax; inversion H3; case_eq (surface_syntax e); intro; rewrite H4 in H5; 
  inversion H5; fail);

try (subst; inversion H_valid_syntax;  try (inversion H5; fail); try (inversion H1; fail) ;  try (subst; contradiction); fail).

  admit.


 admit. admit.   admit.
inversion H13. - unfold erasure_L_fun. fold erasure_L_fun.  apply ST_fieldWrite4. 

 admit. admit. 



 try (inversion H5; fail); try (inversion H1; fail).
 try (inversion H5; fail); try (inversion H1; fail).
 try (subst; contradiction). try (subst; contradiction). admit. 
inversion H_valid_syntax. inversion H3. case_eq (surface_syntax e); intro. 
 try (inversion H5; fail); try (inversion H1; fail).
 try (inversion H5; fail); try (inversion H1; fail).
 ...
 try (case_eq (surface_syntax x); intro; intuition); fail).


  assert ( erasure ( Config ct
       (Container (erasure_L_fun t2) (t1' :: fs) lb
          (fun x : id => erasure_obj_null (sf x) h)) ctns_stack (erasure_heap h)) =
    erasure_fun ct (Container t2 (t1' :: fs) lb sf) ctns_stack h).
  rewrite <- Heqctns. 
  
  destruct ctns; try (induction c); unfold erasure; unfold erasure_fun;  rewrite H_flow; rewrite H;  
  rewrite H_h;rewrite Ht2; auto. 
    rewrite <- Heqctns in H11. 
  apply L_reduction with (Config ct
          (Container (erasure_L_fun t2) (FieldAccess hole f :: fs1) lb2
             (fun x : id => erasure_obj_null (sf2 x) h2)) ctns
          (erasure_heap h2)); auto.
  ...
  assert (t :: fs = fs -> False).

  subst. admit. apply congruence in H9. 

  
 subst.  


subst. remember t1 as tmp. induction tmp. inversion H_reduction; subst.
  assert ((fs = FieldAccess hole f :: fs) = false).

  try (pose proof (fs_pro (FieldAccess hole f ) fs) as Htmp; rewrite <- H8 in Htmp;  intuition; fail). 
  
  try (pose proof (fs_pro_double (FieldAccess hole f ) fs) as Htmp0; rewrite <- H8 in Htmp0;  intuition). 

    assert ()
  induction fs. inversion H8. inversion H8. 
induction fs; subst; inversion H8.
 inversion H8.  inversion H_reduction.
  inversion H_reduction. 
 inversion H_reduction; subst; 
    auto; try (inversion H_hole_t2; fail).
  try (eauto using reduction).
...
- apply field_access_erasure_L_fun; auto. 
 ...
 rewrite H0;  
  ...
  assert (Config ct
    ...
    Config ct
    (Container (erasure_L_fun t2) (FieldAccess hole f :: fs1) lb2
       (fun x : id => erasure_obj_null (sf2 x) h2)) ctns_stack (erasure_heap h2) ); auto.
  rewrite <-  Heqctns in H.
...
destruct ctns; try (induction c); unfold erasure_fun; fold erasure_fun; try (rewrite H_flow).

  + rewrite Heqctns. unfold erasure_L_fun. fold erasure_L_fun. auto.
...
apply L_reduction with (Config ct
          (Container (erasure_L_fun t2) (FieldAccess hole f :: fs1) lb2
             (fun x : id => erasure_obj_null (sf2 x) h2)) ctns_stack
          (erasure_heap h2)); auto. 
