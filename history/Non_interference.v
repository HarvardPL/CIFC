...
Fixpoint erasure_tm (t: tm) : tm :=
  match t with
  | Tvar id => Tvar id
  | null => null
  | FieldAccess e f => FieldAccess (erasure_tm e) f 
  | MethodCall e1 meth e2 => MethodCall (erasure_tm e1) meth (erasure_tm e2)
  | NewExp cls_name => NewExp cls_name
(* label related *)
  | l l0 => l l0
  | labelData e l0  => labelData (erasure_tm e) l0
  | unlabel e => unlabel (erasure_tm e)
  | labelOf e => labelOf (erasure_tm e)
  | unlabelOpaque e  => unlabelOpaque (erasure_tm e)
  | opaqueCall e => opaqueCall (erasure_tm e)
(* statements *)
  | Skip  => Skip
  | Assignment x e => Assignment x (erasure_tm e)
  | FieldWrite e f e' =>  FieldWrite  (erasure_tm e) f  (erasure_tm e')
  | If id1 id2 e1 e2 => If id1 id2  (erasure_tm e1)  (erasure_tm e2)
  | Sequence e1 e2 => Sequence (erasure_tm e1)  (erasure_tm e2) 
  | Return e => Return (erasure_tm e)

(* special terms *)
  | ObjId o => ObjId o
  (* runtime labeled date*)
  | v_l v lb => v_l v lb
  | v_opa_l v lb => v_opa_l v lb
  | dot => dot
  end. 



Inductive erasure_stack : stack -> stack -> Prop :=
      | empty_stack_erasure : forall s s', 
          s = nil ->
          s' = nil ->
          erasure_stack s s'
      | stack_erasure : forall s s' s0 s0' sf' sf lb, 
          s = cons (Labeled_frame lb sf) s0 ->
          s' = cons (Labeled_frame lb sf') s0' ->
          (forall x v, 
              sf x = Some v ->
              sf' x = Some (erasure_tm v))->
          erasure_stack s0 s0' -> 
          erasure_stack s s'.


Inductive L_reduction : config -> config -> Prop :=
  | L_eval : forall ct t t' s s' h h' t'' s'' ,
      (Config ct t (SIGMA s h)) ==> (Config ct t' (SIGMA s' h')) -> 
      erasure_stack s' s'' -> 
      t'' = erasure_tm t' -> 
      L_reduction  (Config ct t (SIGMA s h)) (Config ct  t'' (SIGMA s'' h')).


Inductive L_equivalence_tm : tm -> tm -> Prop :=
  | L_equivalence_tm_eq: forall t, L_equivalence_tm t t.

Inductive L_equivalence_stack : stack -> stack -> Prop :=
  | L_equivalence_stack_eq: forall s, L_equivalence_stack s s.

Inductive L_equivalence_config : config -> config -> Prop :=
  | L_equivalence_config_eq: forall c, L_equivalence_config c c.
...
Lemma Simulation : forall ct t t' s s' h h' t0 t0' s0 s0' h0 h0',
      field_wfe_heap CT h -> wfe_heap CT h -> wfe_stack CT h s
      forall t T, has_type CT empty_context h t T -> 
      forall t',  Config CT t (SIGMA s h) ==> Config CT t' (SIGMA s' h') ->
      forall t T, has_type CT empty_context h t T -> 
      erasure_stack s s0 -> 
      t0 = erasure_tm t ->
      erasure_stack s' s0' -> 
      t0' = erasure_tm t' ->
      L_reduction (Config ct t0 (SIGMA s0 h)) (Config ct t0' (SIGMA s0' h0)).
...
Compute flow_to L_Label H_Label.

Theorem Non_interference : forall ct t s s' h sf sf' v1 v2 lb x s0 s0' c1 c2,
      s = cons (Labeled_frame lb sf) s0 ->
      s' = cons (Labeled_frame lb sf') s0' ->
      sf x = Some (v_l v1 H_Label) ->
      sf' x = Some (v_l v2 H_Label) ->
      L_equivalence_tm v1 v2 ->
      multi_step_L_reduction (Config ct t (SIGMA s h)) c1 -> 
      multi_step_L_reduction (Config ct t (SIGMA s' h)) c2 -> 
      L_equivalence_config c1 c2.
