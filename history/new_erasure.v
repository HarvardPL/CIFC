...
(* method call *)
  ...
  | ST_MethodCall_normal : forall o h cls fields v lx sf  arg_id cls_a body meth returnT ct fs ctns lb sf',
    ...
    Config ct (Container (MethodCall (ObjId o) meth v) fs lb sf ) ctns h 
      ==> Config ct (Container body nil lb sf' ) ((Container (hole) fs lb sf ) :: ctns) h
...
(* skip *)
  ...
  | ST_val2 : forall h v ct ctns sf lb lb' sf' fs',
    value v ->
    (*ctn must have hole*)
    Config ct (Container v nil lb sf) ((Container hole fs' lb' sf') :: ctns) h 
      ==> Config ct (Container (v_opa_l v lb) fs' lb' sf') ctns h

  | ST_val3 : forall CT v fs lb sf h ctns, 
    value v ->
    Config CT (Container v (hole :: fs) lb sf) ctns h ==>
      Config CT (Container v fs lb sf) ctns h
...
Inductive ctn_list_has_type : Class_table -> typing_context -> heap -> list container -> ty -> Prop := 
  | T_ctn_nil : forall h Gamma CT T ctn fs lb sf, 
      ctn_has_type CT Gamma h ctn T ->
      ctn = (Container hole fs lb sf) ->
      ctn_list_has_type CT Gamma h (ctn :: nil) T
  | T_ctn_list : forall h Gamma CT T T' ctn ctns' fs lb sf Gamma', 
      ctn_has_type CT Gamma' h ctn T ->
      (*The first container has hole in it*)
      ctn = (Container hole fs lb sf) ->
      ctn_list_has_type CT Gamma h ctns' T' ->
      ctn_list_has_type CT Gamma h (ctn :: ctns') T'.


Inductive config_has_type : Class_table -> typing_context -> heap -> config -> ty -> Prop :=

  | T_config_nil : forall h Gamma CT T ctn t fs lb sf,  
      ctn_has_type CT Gamma h ctn T ->
      ctn = (Container t fs lb sf) ->
      hole_free t  = true ->
      config_has_type CT Gamma h (Config CT ctn nil h) T

  | T_config_ctns_top_hole_free : forall h Gamma CT T T' T1 T0 ctn ctns Gamma' fs' lb sf ctns' top, 
      ctn_has_type CT Gamma' h ctn T0 ->
      hole_free top = true ->
      ctns = (Container hole (top :: fs') lb sf) :: ctns' ->
      fs_has_type CT Gamma h (top :: fs') (ArrowTy T1 T)  ->
      ctn_list_has_type CT Gamma h ctns T' ->
      config_has_type CT Gamma h (Config CT ctn ctns h) T'
  | T_config_ctns_top_with_hole : forall h Gamma CT T T' T0 ctn ctns Gamma' fs' lb sf ctns' top, 
      ctn_has_type CT Gamma' h ctn T0 ->
      hole_free top = false ->
      ctns = (Container hole (top :: fs') lb sf) :: ctns' ->
      fs_has_type CT Gamma' h (top :: fs') (ArrowTy T0 T)  ->
      ctn_list_has_type CT Gamma h ctns T' ->
      config_has_type CT Gamma h (Config CT ctn ctns h) T'
  ...
  | T_config_new_container : forall Gamma Gamma' e meth argu CT h T returnT cls_def body arg_id arguT lb sf ctn ctns T'
      ...
      ctn = (Container hole fs0 lb0 sf0) ->
      ctn_list_has_type CT Gamma h (ctn :: ctns) T'  ->
...
destruct fs.  destruct ctns. auto. inversion H_typing.   inversion H3. subst.
right.  subst. admit. admit.   auto.
subst. 
...
- inversion H_config. inversion H7.  subst. inversion H16. inversion H1. subst.  
...
+ subst. apply value_is_hole_free in H6.  apply surface_syntax_is_hole_free in H13.
assert (hole_free (MethodCall t1 i t2) = true). unfold hole_free. fold hole_free. rewrite H6. rewrite H13. auto.
exists (Config CT (Container (MethodCall t1 i t2) fs lb sf) ctns h); auto.
+ subst. exists (Config CT (Container (MethodCall t1 i v)  fs lb sf) ctns h); auto.
+ subst. apply value_is_hole_free in H6. apply value_is_hole_free in H13. 
assert (hole_free (MethodCall t1 i t2) = true). unfold hole_free. fold hole_free. rewrite H6. rewrite H13. auto.
exists (Config CT (Container (MethodCall t1 i t2) fs lb sf) ctns h); auto. 
...
+ subst. apply value_is_hole_free in H6.  apply surface_syntax_is_hole_free in H13.
assert (hole_free (FieldWrite t1 i t2) = true). unfold hole_free. fold hole_free. rewrite H6. rewrite H13. auto.
exists (Config CT (Container (FieldWrite t1 i t2) fs lb sf) ctns h); auto.
+ subst. exists (Config CT (Container (FieldWrite v i t2) fs lb sf) ctns h); auto. 
+ subst. subst. apply value_is_hole_free in H6.  apply value_is_hole_free in H13.
assert (hole_free (FieldWrite t1 i t2) = true). unfold hole_free. fold hole_free. rewrite H6. rewrite H13. auto.
exists (Config CT (Container (FieldWrite t1 i t2)  fs lb sf) ctns h); auto.
+ subst. exists (Config CT (Container (FieldWrite t1 i v) fs lb sf) ctns h); auto.
- inversion H_config. inversion H7.  subst. inversion H16. inversion H1. subst.  
  apply surface_syntax_is_hole_free in H6.   apply surface_syntax_is_hole_free in H14.
  assert (hole_free (If i i0 t1 t2) = true). unfold  hole_free. fold hole_free.  rewrite H6. rewrite H14.  auto. 
  exists (Config CT (Container (If i i0 t1 t2) fs lb sf) ctns h); auto.
- inversion H_config. inversion H7.  subst. inversion H16. inversion H1. subst.  
  apply surface_syntax_is_hole_free in H6.   apply surface_syntax_is_hole_free in H12.
  assert (hole_free (Sequence t1 t2) = true). unfold  hole_free. fold hole_free.  rewrite H6. rewrite H12.  auto. 
...
-  exists (Config CT (Container v fs lb sf) ctns h); auto.
- inversion H_config. inversion H7.  subst. inversion H16. destruct H18. 
   inversion H4. 
Qed. 
*) Admitted. 
...
- inversion H_typing. subst.  inversion H6.
  +  subst. inversion H2. subst. inversion H12. inversion H5. 
      inversion H14. inversion H20.
  + subst.  apply T_config_ctns_top_hole_free with T0 T1 T2 Gamma' fs'0
        lb sf ctns'0 top; auto.
  + subst. apply T_config_ctns_top_with_hole with T0 T1 Gamma' fs'0 lb
            sf ctns'0 top; auto. 
  + subst. remember ((update_typing empty_context arg_id (classTy arguT)) ) as gamma0.
       apply T_config_new_container with gamma0 e meth argu T0
        returnT cls_def arg_id arguT fs0 lb0 sf0; auto. 
...
- inversion H_typing. subst. 
  ...
  + subst. apply T_config_ctns with T0 Gamma' fs lb sf ctns'0; auto. 
    inversion H2; subst.
    ++  inversion H14; subst. 
    +++ inversion H10. 
    +++ inversion H12.  +++ inversion H15. 
    +++ 
      apply T_ctn_fs with  (classTy cls'); auto.
      apply T_FieldAccess with cls_def clsT (find_fields cls_def); auto.
    ++  subst. inversion H14. subst. inversion H12.
...
+ subst. apply T_config_ctns with T0 Gamma' fs lb sf ctns'0; auto.  auto.
    inversion H3.  subst. inversion H14.
    ++  subst. 
      apply T_ctn_fs with  (classTy cls'); auto.
     inversion H5. subst.    
      destruct H16 as [field_defs]. destruct H1 as [method_defs].
      assert (t' = null \/
             (exists
                (o' : oid) (field_defs0 : list field) (method_defs0 : 
                                                       list method_def) 
      ...
      destruct H18 as [F0]. destruct H2 as [lo0]. inversion H_valid_config. subst. 
      rewrite H2 in H. inversion H. subst.
        apply field_consist_typing with o cls_def F0 fname lo0 clsT field_defs method_defs; auto.
     rewrite <- H6 in H7. inversion H7.  auto.     rewrite <- H6 in H7. inversion H7.  auto. 
      rewrite <- H6 in H7. inversion H7.  auto. rewrite <- H8 in H17.    
     unfold find_fields in H15. auto.
    
     destruct H2. subst. apply T_null. auto. destruct H2 as [o']. destruct H2 as [field_defs0]. 
     destruct H2 as [method_defs0]. destruct H2 as [field_cls_def]. destruct H2 as [FF]. destruct H2 as [loF].
     destruct H2. destruct H8. destruct H11. subst.  apply T_ObjId with  (class_def cls' field_defs0 method_defs0); auto.
     exists field_defs0. exists method_defs0. auto. 
     exists FF. exists loF. auto.
    ++  admit. 
  + subst. inversion H17. 
...
-  inversion H_typing. subst. 
  ...
  +  subst. apply T_config_ctns with T0 Gamma' fs lb sf ctns'0; auto.  inversion H2.  subst. inversion H13. 
  ++ subst. 
...
-   inversion H_typing. subst. 
  + apply T_config_nil. inversion H6. subst.  inversion H11. subst.
   ...
   ++   subst.
      inversion H_valid_config. inversion H14. subst. inversion H28. subst. 
      inversion H2; subst; try (unfold surface_syntax in H4; inversion H4; fail); 
      try (unfold surface_syntax in H1; inversion H1; fail).
   ++ subst. inversion H11. subst. inversion H9.  
  + subst. apply T_config_ctns with T0 Gamma' fs lb sf ctns'0; auto.  inversion H2.  subst. inversion H14. 
    ++ subst. inversion H10.   
    ++ subst. inversion H12. 
    ++ subst. inversion H15. 
    ++ subst. 
    ...
    ++  subst.
      inversion H_valid_config. subst.  inversion H17. subst. inversion H6. subst.
      inversion H4; subst; try (unfold surface_syntax in H28; inversion H28; fail); 
      try (unfold surface_syntax in H1; inversion H1; fail); 
      try (unfold surface_syntax in H4; inversion H4; fail).
    ++ subst. inversion H14. subst. inversion H12. 
-   inversion H_typing. subst. 
  + apply T_config_nil. inversion H7. subst.  inversion H12. 
    ...
    ++ subst.  
      ...
      inversion H_valid_config.  inversion H15. subst. inversion H29. subst. 
  ...
  + subst. apply T_config_ctns with T0 Gamma' fs lb sf ctns'0; auto.  inversion H3.  subst. inversion H15. 
     ++ subst.   inversion H11.  case_eq (hole_free v1); intro; rewrite H1 in H2; inversion H2. 
      ++ subst.  inversion H13.  case_eq (hole_free v1); intro; rewrite H1 in H2; inversion H2. 
     ++ subst. inversion H16.  case_eq (hole_free v1); intro; rewrite H1 in H2; inversion H2.
     ++ subst. inversion H_valid_config.  inversion H18. subst. inversion H32. subst. 
      inversion H5; subst; try (unfold surface_syntax in H7; inversion H7; fail); 
      try (unfold surface_syntax in H2; inversion H2; fail).
      ++       apply T_ctn_fs with (OpaqueLabeledTy (classTy returnT)); auto. subst. 
      remember (update_typing empty_context arg_id (classTy arguT)) as gamma0. 
      apply T_MethodCall with gamma0 T0 cls_def body arg_id arguT; auto.
       ++ subst. inversion H15. subst. inversion H13. 
      case_eq (hole_free v1); intro; rewrite H1 in H2; inversion H2.
...
- inversion H_typing. subst. 
    ...
    apply T_ctn_nil with fs0 lb' sf0; auto. 
    ...
    admit. admit.  
    apply T_ctn_fs with (classTy returnT0); auto. 
    apply T_ctn_list with (OpaqueLabeledTy T') fs0 lb' sf0 Gamma'; auto. 
    apply T_ctn_fs  with (OpaqueLabeledTy (classTy returnT0)) ; auto.  
+++ subst. admit. 
++ subst. inversion H18. 
-  (*(Container t' nil (join_label lb0 lx) (sf_update empty_stack_frame arg_id v))*)
   inversion H_typing. subst. 
    ...
    apply T_ctn_fs with (classTy returnT0); auto. 
    apply T_ctn_nil with fs0 lb0 sf0; auto. 
    apply T_ctn_fs with (OpaqueLabeledTy (classTy returnT0)) ; auto.  
    ...
    admit. admit.  
    apply T_ctn_fs with (classTy returnT0); auto. 
    apply T_ctn_list with (OpaqueLabeledTy T') fs0 lb0 sf0 Gamma'; auto. 
    apply T_ctn_fs  with (OpaqueLabeledTy (classTy returnT0)) ; auto.  
+++ subst. admit. 
++ subst. inversion H18. 
(*new expression*)
...
- inversion H_typing. subst.
  ...
  ++  inversion H13. subst. inversion H11. subst.  inversion H12. subst. 
  +++  subst. 
     inversion H2. subst. 
  +++ 

    subst. apply T_config_nil with (v_opa_l t lb0) (top :: fs')  lb1 sf1; auto. 

  ++++ 
 
apply T_config_ctns_top_hole_free. with T T1 (OpaqueLabeledTy T') Gamma' fs'
lb1 sf1 nil top; auto. 

 inversion H17. subst. apply T_config_nil.
      inversion H10; subst. 
    ++++ apply T_ctn_fs with (OpaqueLabeledTy T'). admit. 
  

 
  ++ inversion H11. subst.  inversion H13. subst. 
    +++ inversion H2.  subst. inversion H17. subst. inversion H11. subst. 

inversion H10. subst. inversion H12. subst. inversion H7. subst.
  ++++ inversion H13. subst. apply T_config_nil. inversion H6.  subst. 
  +++++ apply T_ctn_fs with (OpaqueLabeledTy T'); auto. 

 inversion H7.  subst. inversion H15. subst.  
        apply T_ctn_fs with (T'); auto. 
      inversion H7. subst. inversion H10. subst. inversion H7. subst. inversion H1.
      subst. inversion H9. subst. 

 
apply T_config_ctns with (OpaqueLabeledTy T') Gamma'; auto.  
  inversion H13; subst; try ( inversion H10; fail); try (inversion H9;fail).
  apply T_ctn_sequential_exec with T2 t fs'0 T3; auto. admit. 
  inversion H9.  case_eq (hole_free e); intro; rewrite H in H0; inversion H0.
 inversion H9.  case_eq (hole_free x); intro; rewrite H in H0; inversion H0.
...
Inductive ctn_has_type : Class_table -> typing_context -> heap -> container -> ty -> Prop :=
  | T_ctn_fs : forall h Gamma CT T lb fs open_t sf T', 
      ...
      ctn_has_type CT Gamma h (Container open_t fs lb sf)  (OpaqueLabeledTy T')
  (*method call crosses container boundary*)
  | T_ctn_meth : forall Gamma Gamma' e meth argu CT h T returnT cls_def body arg_id arguT lb sf,
      tm_has_type CT Gamma h e (classTy T) ->
      tm_has_type CT Gamma h argu (classTy arguT) ->
      Some cls_def = CT(T) ->
      find_method cls_def meth = Some (m_def returnT meth arguT arg_id  body)  ->
...
Inductive ctn_list_has_type : Class_table -> typing_context -> heap -> list container -> ty -> Prop := 
  | T_ctn_nil : forall h Gamma CT T ctn fs lb sf, 
      ...
      ctn = (Container hole fs lb sf) ->
      ctn_list_has_type CT Gamma h (ctn :: nil) T
  | T_ctn_list : forall h Gamma CT T T' ctn ctns' fs lb sf, 
      ctn_has_type CT Gamma h ctn T ->
      (*The first container has hole in it*)
      ctn = (Container hole fs lb sf) ->
      ctn_list_has_type CT Gamma h ctns' T' ->
...
Inductive config_has_type : Class_table -> typing_context -> heap -> config -> ty -> Prop :=
  | T_config_nil : forall h Gamma CT T ctn,  
      ctn_has_type CT Gamma h ctn T ->
      config_has_type CT Gamma h (Config CT ctn nil h) T
  | T_config_ctns : forall h Gamma CT T T' ctn ctns, 
      ctn_has_type CT Gamma h ctn T ->
      ctn_list_has_type CT Gamma h ctns T' ->
      config_has_type CT Gamma h (Config CT ctn ctns h) T'.
...
match ctn with 
 | (Container t fs lb sf) =>
       if (flow_to lb L_Label) then (Config ct (Container (erasure_L_fun t) fs lb (fun x => (erasure_obj_null (sf x) h))) ctns_stack (erasure_heap h))
...
intro. pose proof  (multi_erasure_heap_equal h2) as Hy; rewrite Hy; auto. 
- try (case_eq (flow_to lb2 L_Label)); try (auto); try (intro H_lb2_true; 
unfold erasure_L_fun; fold erasure_L_fun; rewrite H_flow).
assert ( forall a, (fun x : id => erasure_obj_null (sf2 x) h2) a = (fun x : id => (erasure_obj_null (sf2 x) (erasure_heap h2))) a).
...
unfold erasure_L_fun; fold erasure_L_fun; rewrite H_flow).
assert ( forall a, (fun x : id => erasure_obj_null (sf2 x) h2) a = (fun x : id => (erasure_obj_null (sf2 x) (erasure_heap h2))) a).
  intro a. apply erasure_obj_null_equal_erasure_h_helper with ct; auto. apply functional_extensionality in H1.
  rewrite H1. pose proof (H_Label_not_flow_to_L lb1 H_flow) as H_lb1; rewrite H_lb1; auto.
pose proof  (multi_erasure_heap_equal h2) as Hy; rewrite Hy; auto. 
- try (case_eq (flow_to lb2 L_Label)); try (auto); try (intro H_lb2_true; 
unfold erasure_L_fun; fold erasure_L_fun; rewrite H_flow).
assert ( forall a, (fun x : id => erasure_obj_null (sf2 x) h2) a = (fun x : id => (erasure_obj_null (sf2 x) (erasure_heap h2))) a).
  intro a0. apply erasure_obj_null_equal_erasure_h_helper with ct; auto. apply functional_extensionality in H1.
  rewrite H1. pose proof (H_Label_not_flow_to_L lb1 H_flow) as H_lb1; rewrite H_lb1; auto.
pose proof  (multi_erasure_heap_equal h2) as Hy; rewrite Hy; auto. 
pose proof  (multi_erasure_heap_equal h2) as Hy; rewrite Hy. auto.
-  induction ctns_stack1.  unfold erasure_fun. rewrite H_flow. auto. 
 unfold erasure_fun. rewrite H_flow. auto.
  pose proof  (multi_erasure_heap_equal h2) as Hy. rewrite Hy. auto.
-  assert (flow_to (join_label lb1 lx) L_Label = false).  apply flow_join_label with lb1 lx;auto.
apply join_label_commutative. 
induction ctns_stack1;  unfold erasure_fun; rewrite H_flow; rewrite H0; auto.
  pose proof  (multi_erasure_heap_equal h2) as Hy. rewrite Hy. auto.
- assert (erasure_heap
...
apply join_label_commutative. rewrite H. auto. 
-  assert (flow_to (join_label lb1 lo) L_Label = false).  apply flow_join_label with lb1 lo;auto.
apply join_label_commutative. rewrite H. auto. 
- try (case_eq (flow_to lb2 L_Label)); try (auto); try (intro H_lb2_true; 
unfold erasure_L_fun; fold erasure_L_fun; rewrite H_flow).
...
pose proof  (multi_erasure_heap_equal h2) as Hy; rewrite Hy; auto. 
- try (case_eq (flow_to lb2 L_Label)); try (auto); try (intro H_lb2_true; 
unfold erasure_L_fun; fold erasure_L_fun; rewrite H_flow). intro. 
assert ( forall a, (fun x : id => erasure_obj_null (sf2 x) h2) a = (fun x : id => (erasure_obj_null (sf2 x) (erasure_heap h2))) a).
  intro a. apply erasure_obj_null_equal_erasure_h_helper with ct; auto. apply functional_extensionality in H0.
  rewrite H0. pose proof  (multi_erasure_heap_equal h2) as Hy; rewrite Hy; auto. 

- try (case_eq (flow_to lb2 L_Label)); try (auto); try (intro H_lb2_true; 
unfold erasure_L_fun; fold erasure_L_fun; rewrite H_flow). intro. 
assert ( forall a, (fun x : id => erasure_obj_null (sf2 x) h2) a = (fun x : id => (erasure_obj_null (sf2 x) (erasure_heap h2))) a).
  intro a0. apply erasure_obj_null_equal_erasure_h_helper with ct; auto. apply functional_extensionality in H0.
  rewrite H0. pose proof  (multi_erasure_heap_equal h2) as Hy; rewrite Hy; auto. intro.  
pose proof  (multi_erasure_heap_equal h2) as Hy; rewrite Hy; auto. 
Qed.
*) Admitted. 
