...
Inductive L_equivalence_tm : tm -> heap -> tm -> heap ->  (bijection oid oid )->  Prop :=
  ...
  | L_equivalence_tm_eq_newexp_labeled : forall cls1 cls2 h1 h2 l1 l2 φ,
      cls1 = cls2 ->
      l1 = l2 ->
      L_equivalence_tm (NewExpLabeled cls1 l1) h1 (NewExpLabeled cls2 l2) h2  φ
...
Lemma extend_h1h2_with_H_preserve_tm_L_eq : forall t1 t2 ct h1 h2 φ cls1 cls2 lo F1 F2,
      wfe_heap ct h1 ->
      wfe_heap ct h2 ->
     flow_to lo L_Label = false ->
     L_equivalence_tm t1 h1 t2 h2 φ ->
     L_equivalence_tm t1  (add_heap_obj h1 (get_fresh_oid h1) (Heap_OBJ cls1 F1 lo)) t2
                     (add_heap_obj h2 (get_fresh_oid h2) (Heap_OBJ cls2 F2 lo))  φ.
Proof with eauto.
  intros.
  generalize dependent t2. 
  induction t1; intros; inversion H2; subst; auto.
  apply L_equivalence_tm_eq_object_L with cls0 F0 lb1 cls3 F3 lb2; auto.
  assert (lookup_heap_obj (add_heap_obj h1 (get_fresh_oid h1) (Heap_OBJ cls1 F1 lo)) o  =
          Some (Heap_OBJ cls0 F0 lb1)).
  apply extend_heap_lookup_eq; auto.
  apply lookup_extend_heap_fresh_oid with ct  (Heap_OBJ cls0 F0 lb1) ; auto.
  auto.

  assert (lookup_heap_obj (add_heap_obj h2 (get_fresh_oid h2) (Heap_OBJ cls2 F2 lo)) o2  =
          Some (Heap_OBJ cls3 F3 lb2)).
  apply extend_heap_lookup_eq; auto.
  apply lookup_extend_heap_fresh_oid with ct  (Heap_OBJ cls3 F3 lb2) ; auto.
  auto.
  
  apply L_equivalence_tm_eq_object_H with cls0 cls3 F0 lb1  F3 lb2; auto.
  assert (lookup_heap_obj (add_heap_obj h1 (get_fresh_oid h1) (Heap_OBJ cls1 F1 lo)) o  =
          Some (Heap_OBJ cls0 F0 lb1)).
  apply extend_heap_lookup_eq; auto.
  apply lookup_extend_heap_fresh_oid with ct  (Heap_OBJ cls0 F0 lb1) ; auto.
  ...
  apply lookup_extend_heap_fresh_oid with ct  (Heap_OBJ cls3 F3 lb2) ; auto.
  auto.
Qed. Hint Resolve extend_h1h2_with_H_preserve_tm_L_eq.   


  
  

Lemma extend_h1h2_with_H_preserve_fs_L_eq : forall fs1 fs2 ct h1 h2 φ cls1 cls2 lo F1 F2,
    wfe_heap ct h1 ->
    wfe_heap ct h2 ->
     flow_to lo L_Label = false ->
     L_equivalence_fs fs1 h1 fs2 h2 φ ->
     L_equivalence_fs fs1 (add_heap_obj h1 (get_fresh_oid h1) (Heap_OBJ cls1 F1 lo)) fs2
                      (add_heap_obj h2 (get_fresh_oid h2) (Heap_OBJ cls2 F2 lo)) φ.
Proof with eauto.
  intros.
  generalize dependent fs2. 
...
Qed. Hint Resolve extend_h1h2_with_H_preserve_fs_L_eq.



Lemma extend_h1h2_with_H_preserve_L_eq_store {DecOid : forall a1 a2 : oid, Decision (a1 = a2)} :
  forall sf1 h1  sf2 h2  lo cls1 cls2 F1 F2
          ct  (φ:bijection oid oid)  (H1: (left φ (get_fresh_oid h1)  = None)) ( H2: (right φ (get_fresh_oid h2)  = None)) ,
  wfe_heap ct h2 ->  wfe_heap ct h1 -> 
  L_equivalence_store sf1 h1 sf2 h2 φ ->
  L_equivalence_heap h1 h2 φ ->
  wfe_stack_frame ct h1 sf1 ->
  wfe_stack_frame ct h2 sf2 ->
  flow_to lo L_Label = false ->
  (L_equivalence_store sf1 (add_heap_obj h1 (get_fresh_oid h1) (Heap_OBJ cls1 F1 lo))
                    sf2 (add_heap_obj h2 (get_fresh_oid h2) (Heap_OBJ cls2 F2 lo))
                      φ).
Proof.
  intros sf1 h1  sf2 h2 lo cls1 cls2 F1 F2  ct φ H1 H2.
  intros.
  inversion H4; subst; auto.
  inversion H3; subst; auto. 
  apply L_equivalence_store_L; auto.
  ...
  split; auto. intros. 
  assert (L_equivalence_tm v1 h1 v2 h2 φ).
  apply H13 with x; auto.
  apply extend_h1h2_with_H_preserve_tm_L_eq with ct; auto.
  apply H13.
Qed. Hint Resolve extend_h1h2_with_H_preserve_L_eq_store . 


Lemma extend_h1h2_heap_preserve_L_eq_ctn {DecOid : forall a1 a2 : oid, Decision (a1 = a2)} :
  forall h1  h2 lo cls1 cls2 F1 F2 ct  (φ:bijection oid oid)
         (H1: (left φ (get_fresh_oid h1)  = None)) ( H2: (right φ (get_fresh_oid h2)  = None))
  ctn1 ctn2,
    wfe_heap ct h2 ->  wfe_heap ct h1 ->
    valid_ctn ct ctn1 h1 -> valid_ctn ct ctn2 h2  ->
  ...
  (L_eq_container ctn1 (add_heap_obj h1 (get_fresh_oid h1) 
                                     (Heap_OBJ  cls1 F1 lo))
                    ctn2 (add_heap_obj h2 (get_fresh_oid h2)
                      (Heap_OBJ cls2 F2 lo)) φ).
Proof.
  intros h1  h2 lo cls1 cls2 F1 F2 ct φ  H1 H2 ctn1 ctn2.
  intros.
  ...
  inversion H5; subst; auto.
  apply  L_eq_ctn; auto.
  eauto using extend_h1h2_with_H_preserve_tm_L_eq.
  ...
  inversion H3; auto. inversion H4; auto. 
Qed. Hint Resolve extend_h1h2_heap_preserve_L_eq_ctn. 

Lemma extend_h1h2_heap_preserve_L_eq_ctns {DecOid : forall a1 a2 : oid, Decision (a1 = a2)} : forall ctns1 h1  ctns2 h2  lo cls1 cls2 F1 F2 ct  (φ:bijection oid oid)  (H1: (left φ (get_fresh_oid h1)  = None)) ( H2: (right φ (get_fresh_oid h2)  = None)) ,
  wfe_heap ct h2 ->  wfe_heap ct h1 -> 
  L_eq_ctns ctns1 h1 ctns2 h2 φ ->
  L_equivalence_heap h1 h2 φ ->
  valid_ctns ct ctns1 h1 ->
  ...
  (L_eq_ctns ctns1 (add_heap_obj h1 (get_fresh_oid h1) 
                    ...
                    ctns2 (add_heap_obj h2 (get_fresh_oid h2)
                      (Heap_OBJ cls2 F2 lo)) φ).
Proof.
  intros ctns1 h1 ctns2 h2 lo cls1 cls2 F1 F2 ct φ H1 H2.
  intros.
  generalize dependent ctns2.  
  induction ctns1; intros; inversion H3; subst; auto.
  apply L_eq_ctns_list; auto.
  inversion H6; subst; auto. 
  apply extend_h1h2_heap_preserve_L_eq_ctn with ct ; auto.
  inversion H5; subst; auto. 

  apply   IHctns1; auto.
  inversion H5; subst; auto.
  inversion H6; subst; auto. 
Qed. Hint Resolve extend_h1h2_heap_preserve_L_eq_ctns.
