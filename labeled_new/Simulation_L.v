...
- inversion H_reduction2; subst; auto; try (solve_by_invert_ctn).
  ...
  (*newly created object is L labeled*)  
  exists  (bijection.extend_bijection φ (get_fresh_oid h1)  (get_fresh_oid h2) H6 H8). split; auto.
  (* L_equivalence_heap  *)
  inversion H18; subst; auto.
  inversion H30; subst; auto. rename cls_name0 into cls_name.
  rewrite H7 in H; inversion H; subst; auto. 

  remember (class_def cls_name field_defs method_defs) as cls_def.
  remember (add_heap_obj h1 (get_fresh_oid h1) 
        (Heap_OBJ  cls_def (init_field_map (find_fields cls_def) empty_field_map) lo0)) as h1'.
  remember (add_heap_obj h2 (get_fresh_oid h2) 
                         (Heap_OBJ  cls_def (init_field_map (find_fields cls_def) empty_field_map) lo0)) as h2'.
 ...
 (* apply H7 in H4. apply H8 in H5. *)
  ...
  destruct (oid_decision o3 o1). subst; auto. simpl in H20.
  destruct (decision.decide (get_fresh_oid h1 = get_fresh_oid h1) ) in H20. inversion H20; subst; auto. 
  ...
  assert (lookup_heap_obj h1' (get_fresh_oid h1) = Some (Heap_OBJ  cls_def (init_field_map (find_fields cls_def) empty_field_map) lo0) ).
  subst; auto.

  assert (lookup_heap_obj h2' (get_fresh_oid h2) = Some (Heap_OBJ  cls_def (init_field_map (find_fields cls_def) empty_field_map) lo0) ).
  subst; auto.
  
  apply object_equal_L with lo0 lo0 cls_def cls_def (init_field_map (find_fields cls_def) empty_field_map) (init_field_map (find_fields cls_def) empty_field_map); auto.
  
  split; auto. 
  split; auto.
  apply L_Label_flow_to_L in H28.
  apply L_Label_flow_to_L in H29. rewrite <- H29 in H28. auto. 
  ...
  intuition.  
   assert ( bijection.left (bijection.extend_bijection φ o3 o4 H6 H8)  o1 = 
           bijection.left φ o1) by (apply bijection.left_extend_bijection_neq; auto).
   rewrite H20 in H22. assert ( bijection.left φ o1 = Some o2). auto.
   apply H0 in H23. inversion H23; subst; auto.
   destruct H33; subst; auto. destruct H34; subst; auto. 

  remember (class_def cls_name field_defs method_defs) as cls_def.
  assert ( lookup_heap_obj
     (add_heap_obj h1 (get_fresh_oid h1)
       (Heap_OBJ cls_def
          (init_field_map (find_fields cls_def) empty_field_map) lo0)) o1 = Some (Heap_OBJ cls2 F1 lb3) ).
  apply extend_heap_lookup_eq; auto. 

  destruct (oid_decision (get_fresh_oid h2) o2 ).
   assert (lookup_heap_obj h2 (get_fresh_oid h2)  = None). 
   apply fresh_oid_heap with ct; auto.  rewrite e in H35.
   rewrite H35 in H25. inversion H25.
   
   assert ( lookup_heap_obj
     (add_heap_obj h2 (get_fresh_oid h2)
       (Heap_OBJ cls_def
          (init_field_map (find_fields cls_def) empty_field_map) lo0)) o2 = Some (Heap_OBJ cls2 F2 lb3) ).
   apply extend_heap_lookup_eq; auto.

   apply object_equal_L with lb3 lb3 cls2 cls2 F1 F2; auto.  
   split; auto.
   split; auto. 
   destruct H34. destruct H36.     
   split; auto. split; auto.
   intros.
   destruct H37 with fname fo1 fo2; auto. rename x into hof1.
   destruct H40 as [hof2]. destruct H40. destruct H41.
   exists hof1. exists hof2.
   split; auto. 
   apply extend_heap_lookup_eq; auto. 
   apply lookup_extend_heap_fresh_oid with ct hof1 ; auto. 
   ...
   assert (fo1 <> get_fresh_oid h1  ).
   apply lookup_extend_heap_fresh_oid with ct hof1 ; auto.
   assert ( bijection.left (bijection.extend_bijection φ (get_fresh_oid h1) (get_fresh_oid h2) H6 H8)  fo1 = 
          bijection.left φ fo1) by (apply bijection.left_extend_bijection_neq; auto).
   rewrite H44.   auto.
   
   intros. subst; auto.  destruct (oid_decision (get_fresh_oid h1) o). subst; auto.
   unfold  lookup_heap_obj in H20. unfold add_heap_obj in H20.
   
   rewrite H14 in H20. inversion H20.
   remember (bijection.extend_bijection φ (get_fresh_oid h1) (get_fresh_oid h2) H6 H8) as φ'.
   rewrite   Heqφ'.
   assert (bijection.left (bijection.extend_bijection φ (get_fresh_oid h1) (get_fresh_oid h2) H6 H8)  o = 
           bijection.left φ o) by (apply bijection.left_extend_bijection_neq; auto).
   remember (class_def cls_name field_defs method_defs) as cls_def. 
   assert ( lookup_heap_obj h1 o = None).
   (apply lookup_extended_heap_none with
        (Heap_OBJ cls_def (init_field_map (find_fields cls_def)
                                          empty_field_map) lo0) ; auto).
   rewrite H22.  apply H1 in H23. auto.
   rewrite   Heqh2'. intros. subst. 
   destruct (oid_decision (get_fresh_oid h2) o). subst; auto.
   unfold  lookup_heap_obj in H20. unfold add_heap_obj in H20.
   rewrite H15 in H20. inversion H20.

   assert (bijection.right (bijection.extend_bijection φ (get_fresh_oid h1) (get_fresh_oid h2) H6 H8)  o = 
           bijection.right φ o). (apply bijection.right_extend_bijection_neq; auto).
   remember (class_def cls_name field_defs method_defs) as cls_def. 
   assert ( lookup_heap_obj h2 o = None) by (apply lookup_extended_heap_none with
                                                 (Heap_OBJ cls_def (init_field_map (find_fields cls_def)
                                                                                   empty_field_map) lo0) ; auto).
   rewrite H22. apply H2 in H23. auto.
   intros.
   destruct (oid_decision o3 o). subst; auto. simpl in H20.
   rewrite H14 in H20. inversion H20. subst; auto.
   try (inconsist_label).
   subst; auto.

   unfold lookup_heap_obj in H20. unfold add_heap_obj in H20.
   assert (o <> get_fresh_oid h1) by (auto). apply  beq_oid_not_equal in H23. rewrite H23 in H20.
   fold lookup_heap_obj in H20. apply H3 in H20.
   assert (bijection.left (bijection.extend_bijection φ (get_fresh_oid h1) (get_fresh_oid h2) H6 H8)  o = 
           bijection.left φ o) by (apply bijection.left_extend_bijection_neq; auto).
   rewrite H24; auto. auto. 

   intros.
   destruct (oid_decision o4 o). subst; auto. simpl in H20.
   rewrite H15 in H20. inversion H20. subst; auto.
   try (inconsist_label).
   subst; auto.

   unfold lookup_heap_obj in H20. unfold add_heap_obj in H20.
   assert (o <> get_fresh_oid h2) by (auto). apply  beq_oid_not_equal in H23. rewrite H23 in H20.
   fold lookup_heap_obj in H20. apply H5 in H20.
   assert (bijection.right (bijection.extend_bijection φ (get_fresh_oid h1) (get_fresh_oid h2) H6 H8)  o = 
           bijection.right φ o) by (apply bijection.right_extend_bijection_neq; auto).
   rewrite H24; auto. auto.
  ...
  rewrite H7 in H; inversion H; subst; auto.
  apply  L_Label_flow_to_L in H10.
  subst; auto.
  ...
  ; auto. 
  apply extend_heap_preserve_L_eq_fs with ct φ H6 H8; auto.
  apply extend_heap_preserve_L_eq_store with ct φ H6 H8; auto.
  inversion  H_valid1; subst; auto.
  inversion H24; auto. 
  inversion  H_valid2; subst; auto.
  inversion H24; auto. 
  ...
  apply  L_Label_flow_to_L in H10.
  subst; auto.
  apply extend_heap_preserve_L_eq_ctns with ct φ H6 H8; auto.
    inversion  H_valid1; subst; auto.
  inversion  H_valid2; subst; auto.



(*newly created object is H labeled*)  
  exists  φ. split; auto.
  (* L_equivalence_heap  *)
  inversion H18; subst; auto.
  inversion H30; subst; auto.
  rename cls_name0 into cls_name.
  rewrite H7 in H; inversion H; subst; auto. 
  ...
  assert (lookup_heap_obj h1' o1 = Some (Heap_OBJ cls1 F1 lb0)).
  subst; auto. 
  apply extend_heap_lookup_eq; auto.
  eauto using lookup_extend_heap_fresh_oid .
  assert (lookup_heap_obj h2' o2 = Some (Heap_OBJ cls2 F2 lb3)).
  subst; auto. 
  apply extend_heap_lookup_eq; auto.
  eauto using lookup_extend_heap_fresh_oid .

  
  destruct H27. destruct H40. destruct H41. destruct H42. 
  apply object_equal_L with lb0 lb3 cls1 cls2 F1 F2; auto.
  split; auto.  split; auto.   split; auto.   split; auto.
  intros;subst; auto. 

  destruct H43 with fname fo1 fo2; auto. rename x into hof1.
  destruct H27 as [hof2]. destruct H27. destruct H33.
  exists hof1. exists hof2.
  split; auto. 
  apply extend_heap_lookup_eq; auto. 
  apply lookup_extend_heap_fresh_oid with ct hof1 ; auto. 
  ...
  intros. subst; auto.  destruct (oid_decision (get_fresh_oid h1) o). subst; auto.
  unfold  lookup_heap_obj in H20. unfold add_heap_obj in H20.
  assert (o <> get_fresh_oid h1). auto. 
  apply  beq_oid_not_equal in H22. rewrite H22 in H20. fold lookup_heap_obj in H20.
  apply H1 in H20. auto.

  intros. subst; auto.  destruct (oid_decision (get_fresh_oid h2) o). subst; auto.
  unfold  lookup_heap_obj in H20. unfold add_heap_obj in H20.
  assert (o <> get_fresh_oid h2). auto. 
  apply  beq_oid_not_equal in H22. rewrite H22 in H20. fold lookup_heap_obj in H20.
  apply H2 in H20. auto.

  intros. 
  destruct (oid_decision o3 o).
  subst; auto.
  subst; auto. 
  unfold  lookup_heap_obj in H20. unfold add_heap_obj in H20.
  assert (o <> get_fresh_oid h1). auto. 
  apply  beq_oid_not_equal in H23. rewrite H23 in H20. fold lookup_heap_obj in H20.
  apply H3 in H20; auto.

  intros.
  destruct (oid_decision o4 o); subst; auto.
  unfold  lookup_heap_obj in H20. unfold add_heap_obj in H20.
  assert (o <> get_fresh_oid h2). auto. 
  apply  beq_oid_not_equal in H23. rewrite H23 in H20. fold lookup_heap_obj in H20.
  apply H5 in H20; auto.
  ...
  rewrite H7 in H; inversion H; subst; auto.
  apply L_eq_ctn; auto.
  eauto using L_equivalence_tm.
  eauto using  extend_h1h2_with_H_preserve_fs_L_eq.
  apply extend_h1h2_with_H_preserve_L_eq_store with ct; auto.
  inversion H_valid1; subst; auto.
  inversion H25; subst; auto. 
  ...
  inversion H18; subst; auto.
  inversion H30; subst; auto. 
  apply extend_h1h2_heap_preserve_L_eq_ctns with ct; auto.
