...
(* new exp labeled *)
  | Valid_newexp_labeled1 : forall cls lb,
      valid_syntax (NewExpLabeled cls lb)
...
(* new expression labeled *)
  | ST_NewExp_Labeled : forall h h' o lb lo cls_name field_defs method_defs cls F ct fs ctns sf,
      ct cls_name = Some cls -> 
      o = get_fresh_oid h ->
      cls = (class_def cls_name field_defs method_defs) ->
      F =  (init_field_map (find_fields cls) empty_field_map) ->
      ...
      flow_to lb lo = true ->
      Config ct (Container (NewExpLabeled cls_name lo) fs lb sf ) ctns h ==> Config ct (Container (ObjId o) fs lb sf ) ctns h'

(* new expression labeled leaked  *)

  | ST_NewExp_Labeled_leak : forall h h' o lb lo cls_name field_defs method_defs cls F ct fs ctns sf,
      ct cls_name = Some cls -> 
      o = get_fresh_oid h ->
      cls = (class_def cls_name field_defs method_defs) ->
      F =  (init_field_map (find_fields cls) empty_field_map) ->
      h' = add_heap_obj h o (Heap_OBJ cls F lo) ->
      flow_to lb lo = false ->
      Config ct (Container (NewExpLabeled cls_name lo) fs lb sf ) ctns h ==> Error_state
