...
Proof with eauto.
  ...
  - destruct H as [cls].
    ...
    destruct H. remember (get_fresh_oid h) as o. 
    remember (init_field_map (find_fields cls) empty_field_map) as F. 
    remember (add_heap_obj h o (Heap_OBJ cls F lo)) as h'.
    right.
    case_eq (flow_to lb lo); intro.   
    exists (Config ct (Container (ObjId o) fs lb sf ) ctns h'). 
    apply ST_NewExp_Labeled with field_defs method_defs cls F; auto.

    exists Error_state. eauto using ST_NewExp_Labeled_leak.  
