...
- assert (valid_syntax result);
  ...
  - inversion H20; subst; auto.
    ...
    destruct H33 as [F'].
    destruct H14 as [lo'].
    destruct H32 as [field_defs].
    destruct H17 as [method_defs].
    destruct H14 as [ll]. 
    subst; auto. 
    apply stack_val_object with arguT F' lo'
                                field_defs method_defs ll; auto.
...
(*   valid_config (Config ct (Container (MethodCall (ObjId o) meth v) fs lb sf) ctns0 h0) *)
  - inversion H20; subst; auto.
    ...
    rewrite <- H18 in H14; inversion H14; subst; auto.
    ...
    intros.
    case_eq (beq_id arg_id0 x);intro.
    unfold sf_update in H4. rewrite H19 in H4.
    inversion H4; subst; auto.
    split; auto. 
    ...
    apply stack_val_object with arguT F' lo'
                                field_defs method_defs ll; auto.
    apply stack_val_opa_labeled.
    inversion H37; subst; auto; inversion H33; subst; auto.
    destruct H41 as [F'].
    destruct H28 as [lo'].
    destruct H40 as [field_defs].
    destruct H31 as [method_defs].
    destruct H28 as [ll].
    subst; auto. 
    apply stack_val_object with arguT F' lo'
                                field_defs method_defs ll; auto.
    destruct H39 with v0 lb1; auto.     
...
Qed. Hint Resolve  expand_heap_preserve_typed_tm.
  ...
  Proof with eauto.
    ...
    eauto using tm_has_type.
    apply T_objectLabelOf with clsT0; auto. 
...
- (* (Container result fs lb sf)  *)
  ...
  destruct H4.
  apply H1 in H4; subst; auto.
  apply H2 in H4; subst; auto.
  intros. subst; auto.
  inversion H10; subst; auto.
  intros. subst; auto.
...
-  inversion H_typing; subst; auto.
  ...
  - inversion H_typing; subst; auto.
    ...
    destruct H1 as [ll0].
    rewrite H1 in H; inversion H; subst; auto.
    rewrite <- H13 in H5; inversion H5; subst; auto.
    ...
    destruct H2 as [llF].
    destruct H2.
    destruct H14. destruct H20. subst;auto.
    apply T_ObjId with  (class_def cls' field_defs0 method_defs0); auto.
    exists field_defs0. exists method_defs0. auto. 
    exists FF. exists loF. exists llF. auto.
...
(* config_has_type ct empty_context (Config ct (Container body nil (join_label lb ell) (sf_update empty_stack_frame arg_id v)) (Container return_hole fs lb sf :: ctns0) h') T *)
  ...
  - inversion H_typing; subst; auto.  
    ...
    destruct H2 as [ll0].
    rewrite H2 in H; inversion H; subst; auto.
    rewrite <- H21 in H13; inversion H13; subst; auto.
    rewrite H14 in H0; inversion H0; subst; auto.
    remember ( (update_typing empty_context arg_id0 (classTy arguT))) as gamma0.
    apply T_config_ctns with (classTy returnT0) gamma0; auto.
    apply T_ctn_type with  (classTy returnT0)
    ...
    exists v; split; auto.
    subst; auto. unfold update_typing in H3; rewrite H22 in H3.
    inversion H3; subst; auto. 
    apply value_typing_invariant_gamma with Gamma';auto.
...
(* (ObjectLabelOf hole :: fs0) *)
- inversion H_typing; subst; auto.
  apply T_config_ctns with T0 Gamma'; auto.
  inversion H3; subst; auto.
  inversion H8; subst; auto.
  inversion H9; subst; auto. 
  + assert (tm_hole_has_type ct Gamma' h' (objectLabelOf hole)
                                   (ArrowTy (classTy clsT)  LabelTy)).
    eauto using tm_hole_has_type.          
    apply T_ctn_type with (classTy clsT)  (classTy clsT) ; auto.
    destruct fs.          
    ++ apply T_fs_hole with LabelTy LabelTy; auto.
    ...
    ++ case_eq (hole_free t); intro.
       ...
       +++ apply T_fs_hole with LabelTy T2 ; auto.
           intros. inversion H4; subst; auto.
           destruct H17 with top fs'; subst; auto.
       ++ intros. subst; auto. split; auto.
          intro contra; inversion contra. 
...
- inversion H_typing; subst; auto. 
  ...
  + inversion H19; subst; auto.
    apply T_ctn_type with LabelTy T4; auto.
    destruct H17 with (objectLabelOf hole) fs; auto.
    subst; auto.
    eauto using tm_has_type.
    intros.
...
-  (*(Container (raiseLabel (v_opa_l (ObjId o) ell) lo') fs lb sf)*)
  ...
  apply lookup_updated with h0 ((Heap_OBJ cls F lo ll)); auto.
  exists F. exists lo'. exists ll. auto. 
  rewrite H22 in H; inversion H; subst; auto. 
  ...
  destruct H15 as [lo0].
  destruct H15 as [ll0].
  rewrite H15 in H; inversion H; subst; auto. 
  apply change_obj_label_preserve_typed_fs with lo0 clsT field_defs
                                                method_defs ll0; auto.
  inversion H_valid_config; subst; auto. 
...
(* (Container e2 (toLabeled e1 hole :: fs) lb sf)  *)
  -  inversion H_typing; subst; auto.
     apply T_config_ctns with T0 Gamma'; auto.
     inversion H3; subst; auto.
     inversion H8; subst; auto.
     inversion H9; subst; auto.
     + assert (tm_hole_has_type ct Gamma' h' (toLabeled e1 hole)
                                   (ArrowTy LabelTy (LabelelTy T3))).
       eauto using tm_hole_has_type.
       assert (hole_free (toLabeled e1 hole) = false).
       unfold hole_free. fold hole_free.
       case_eq (hole_free e1); intro; auto.    
       apply T_ctn_type with LabelTy LabelTy ; auto.
       destruct fs.          
       ++ apply T_fs_hole with (LabelelTy T3) (LabelelTy T3); auto.
          intros. inversion H4. 
          assert ( T2 = (LabelelTy T3)); auto; subst; auto.
  ...
  - inversion H_typing; subst; auto. 
    ...
    ++
      ...
      *        
        inversion H20; subst; auto.
        apply T_ctn_type with (LabelelTy T3) T4; auto.
        destruct H18 with (toLabeled e1 hole) fs; auto.
        subst; auto.
        intros. subst; auto.
        split; auto.  intro contra; inversion contra. 
...
- (*(Config ct (Container (toLabeled e1 (l lo)) fs lb sf) ctns0 h') *)
  inversion H_typing; subst; auto.
  inversion H2; subst; auto.
  inversion H7; subst; auto. 
  apply T_config_ctns with (LabelelTy T3) Gamma'; auto.
  +
    apply T_ctn_type with T3 T3; auto.
    apply  T_fs_hole with (LabelelTy T3) (LabelelTy T3); auto.
    intros. split; auto. intro contra; inversion contra. intro contra; inversion contra.
  +
    intro contra; inversion contra.
  +
    apply  T_ctn_list with T0 Gamma'  ; auto.
    eauto using  T_ctn_caller .
  +
    intros. inversion H; subst; auto.
    exists fs. exists lb. exists sf. auto. 
    
- (*  toLabeled e1 opaque v  *)
  inversion H_typing; subst; auto.
  apply T_config_ctns with T0 Gamma'; auto.
  inversion H3; subst; auto.
  inversion H8; subst; auto.
...
(* config_has_type ct empty_context (Config ct (Container (v_opa_l v lb) fs' lb' sf') ctns' h') T *)
  - inversion H_typing; subst; auto.
    ...
    + inversion H10; subst; auto.
      ...
      destruct H25 with top fs'0; auto.
