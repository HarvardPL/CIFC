...
- right.  destruct IHhas_type. auto. auto. auto.    
            ...
            subst. destruct H3 as [t']. destruct H as [sigma'].
                ...
                destruct H3.
                 ...
                 rewrite H3 in H.  inversion H. inversion H5. inversion H5.  
                  ...
                  auto. rewrite -> H6 in Heqsigma. inversion Heqsigma. auto. auto. auto. auto. 
...
Theorem preservation : forall t t' sigma sigma' gamma CT s s' h h', 
    ...
    sigma' = SIGMA s' h' -> 
    t <> NPE -> 
    (     forall T, has_type CT gamma h t T -> ( has_type CT gamma h' t' T)  ).
Proof with auto.
    intros. 
...
intro T. intro.  induction T. 
 ...
 intro contra. rewrite contra in H2. inversion H2. 
    auto. auto.  auto.      auto. 
   (* subgoal *)
     + inversion H5.   
 (* subgoal *)
...
intro T. intro.  inversion H11. subst.  (*inversion H13. destruct H5. *)
inversion H14. 
assert (v = null \/
             (exists
                (o' : oid) (field_defs0 : list field) (method_defs0 : 
 ...
 destruct H17 as [field_defs0]. destruct H17 as [method_defs0].
...
auto. auto. auto. rewrite <- H15 in H9. inversion H9. rewrite H19 in H17.
rewrite H17 in H21. unfold find_fields in H21.  auto. 
inversion H2. inversion H3. subst. rewrite <- H5 in H16. inversion H16.
rewrite H10 in H7. auto. 

inversion H3. inversion H2.  

destruct H18. rewrite H18. 

apply T_null with (Gamma:=gamma) (h:=h') (CT:=CT) (T:=(classTy cls')).

destruct H18 as [o']. destruct H18 as [field_defs0]. destruct H18 as [method_def0]. 
destruct H18 as [field_cls_def]. destruct H18 as [F']. destruct H18 as [lx]. 
destruct H18. rewrite H18. 

apply T_ObjId with (h:=h') (Gamma:=gamma) (CT:=CT) (o:=o') (cls_name:=cls') 
          (F:=F') (lo:=lx) (cls_def:=field_cls_def).

destruct H19. destruct H25.  auto. 
destruct H19. destruct H25.  rewrite <- H22. rewrite <- H24.   auto. 
exists field_defs0. exists method_def0. destruct H19.  auto. 
