...
(* field read *)
  ...
  | ST_fieldRead3 : forall h o fname lo lb cls F v l'  ct fs ctns sf,
      ...
      Config ct (Container (FieldAccess (ObjId o) fname) fs lb sf) ctns h ==>
             Config ct (Container v nil l' sf) ((Container (return_hole) fs lb sf ) :: ctns) h

  (* field access with opaque obj *)
  | ST_fieldRead4 : forall h o fname lo lb cls F v l'  ct fs ctns sf ell,
      Some (Heap_OBJ cls F lo) = lookup_heap_obj h o -> 
      Some v = F(fname) -> 
...
(* method call *)
  ...
  (* normal method call *)
  | ST_MethodCall_normal : forall o h cls fields v lo sf  arg_id cls_a body meth returnT ct fs ctns lb sf',
      ...
      ==> Config ct (Container body nil (join_label lo lb) sf' ) ((Container (return_hole) fs lb sf ) :: ctns) h

  (* normal method call opaque obj *)
  | ST_MethodCall_normal_opaque_obj : forall o h cls fields v lo sf  arg_id cls_a body meth returnT ct fs ctns lb sf' ell lb',
      Some (Heap_OBJ cls fields lo) = lookup_heap_obj h o -> 
      Some (m_def returnT meth cls_a arg_id body) = find_method cls meth -> 
      value v ->
      sf' = sf_update empty_stack_frame arg_id v ->
...
(* objectLabelOf *)
  ...
  | ST_objectlabelof3 : forall h ct fs sf ctns cls F lb lo o,
      Some (Heap_OBJ cls F lo) = lookup_heap_obj h o ->
      Config ct (Container (objectLabelOf (ObjId o)) fs lb sf) ctns h ==>  Config ct (Container (l lo) fs (join_label lo lb) sf) ctns h

  | ST_objectlabelof4 : forall h ct fs sf ctns cls F lb lo o l' ell,
      Some (Heap_OBJ cls F lo) = lookup_heap_obj h o ->
      l' = join_label lb ell ->
      Config ct (Container (objectLabelOf (v_opa_l (ObjId o) ell)) fs lb sf) ctns h ==>  Config ct (Container (l lo) fs (join_label lo l') sf) ctns h
...
(* raise label of an object *)
  ...
  | ST_raiseLabel4 : forall h ct fs sf ctns cls F lb lo lo' o h' ell,
      Some (Heap_OBJ cls F lo) = lookup_heap_obj h o -> 
  ...
  | ST_raiseLabelException3 : forall h lb ct fs o ctns sf lo' cls F lo ell,
      Some (Heap_OBJ cls F lo) = lookup_heap_obj h o -> 
...
(* Field Write *)
  ...
  | ST_fieldWrite_normal : forall o h h' f lo lb cls F F' v ct fs ctns sf,
      ...
      flow_to (join_label (runtime_label v) lb) lo = true -> 
      h' = update_heap_obj h o (Heap_OBJ cls F' lo) ->
      value v ->
  ...
  | ST_fieldWrite_normal_opa_obj : forall o h h' f lo lb cls F F' v ct fs ctns sf lb' ell,
      Some (Heap_OBJ cls F lo) = lookup_heap_obj h o -> 
      F' = fields_update F f (runtime_val v) ->
      lb' = (join_label (runtime_label v) lb) ->
      flow_to (join_label lb' ell) lo = true -> 
      h' = update_heap_obj h o (Heap_OBJ cls F' lo) ->
      value v ->
  ...
  | ST_fieldWrite_leak : forall o h f lo lb cls F v ct fs ctns sf,
      Some (Heap_OBJ cls F lo) = lookup_heap_obj h o ->
      flow_to (join_label (runtime_label v) lb) lo = false -> 
      value v ->
    Config ct (Container (FieldWrite (ObjId o) f v) fs lb sf ) ctns h 
           ==>  Error_state

  (* opaque obj field information leak *)
  | ST_fieldWrite_leak_opa : forall o h f lo lb cls F v ct fs ctns sf lb' ell,
      Some (Heap_OBJ cls F lo) = lookup_heap_obj h o ->
      lb' = (join_label (runtime_label v) lb) ->
      flow_to (join_label lb' ell) lo = false -> 
      value v ->
