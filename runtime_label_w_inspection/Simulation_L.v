...
- inversion H_reduction2; subst; auto; try (solve_by_invert_ctn).
  ...
  assert (flow_to (join_label lo lb1) L_Label = true).   
  apply join_L_label_flow_to_L; auto.

  assert (flow_to (join_label lo0 lb2) L_Label = true).   
  apply join_L_label_flow_to_L; auto.
  apply L_equivalence_config_L; auto.
  apply L_eq_ctn; auto.

  apply L_Label_flow_to_L in H6.
  apply L_Label_flow_to_L in H10; subst; auto. 
  ...
  apply L_eq_ctn; auto.
  apply L_equivalence_store_L; auto.
  split; auto; intros. inversion H7. split; auto.
...
- inversion H_reduction2; subst; auto; try (solve_by_invert_ctn).
  ...
  assert (flow_to (join_label lo (join_label lb1 ell0)) L_Label = true).
  apply join_L_label_flow_to_L; auto.
  apply join_L_label_flow_to_L; auto.
  
  assert (flow_to (join_label lo0 (join_label lb2 ell0)) L_Label = true). 
  apply join_L_label_flow_to_L; auto.
  apply join_L_label_flow_to_L; auto.
  apply L_equivalence_config_L; auto.
  apply L_eq_ctn; auto.
  apply L_Label_flow_to_L in H7.
  apply L_Label_flow_to_L in H23; subst; auto.

  +++ exists φ. split; auto.
  assert (flow_to (join_label lo (join_label lb1 ell0)) L_Label = false).
  apply flow_join_label  with lo (join_label lb1 ell0); auto.
  rewrite <- H in H3; inversion H3; subst; auto.
  apply join_label_commutative; auto.
  
  assert (flow_to (join_label lo0 (join_label lb2 ell0)) L_Label = false). 
  apply flow_join_label  with lo0 (join_label lb2 ell0); auto.
  rewrite <- H2 in H7; inversion H7; subst; auto.
  apply join_label_commutative; auto.
  ...
  apply L_eq_ctn; auto.
  apply L_equivalence_store_L; auto.
  split; auto; intros. inversion H10. split; auto.
  ...
  apply L_eq_ctn; auto.
  apply L_equivalence_store_L; auto.
  split; auto; intros. inversion H3. split; auto.
