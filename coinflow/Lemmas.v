...
Proof.
  ...
  * 
  ...
  exists (Config CT (Container (v_opa_l  (ObjId o) lb) f l s) ctns h); auto.
  apply ST_val_return_obj; auto; intros; intro contra; inversion contra.
 
  exists (Config CT (Container (v_opa_l null lb) f l s) ctns h); auto.
  apply ST_val_return_obj; auto; intros; intro contra; inversion contra. 

  exists (Config CT (Container (v_opa_l (Language.l lb0) lb) f l s) ctns h); auto.
  apply ST_val_return_obj; auto; intros; intro contra; inversion contra.

  exists (Config CT (Container (v_opa_l (v_l v0 lb0) lb) f l s) ctns h); auto.
  apply ST_val_return_obj; auto; intros; intro contra; inversion contra. 

  exists (Config CT (Container (v_opa_l v0 (join_label lb0 lb)) f l s) ctns h); auto.

  exists (Config CT (Container (v_opa_l  B_true lb) f l s) ctns h); auto;
    apply ST_val_return_obj; auto; intros; intro contra; inversion contra. 

  exists (Config CT (Container (v_opa_l  B_false lb) f l s) ctns h); auto;
    apply ST_val_return_obj; auto; intros; intro contra; inversion contra. 

  * right. inversion H_typing; subst; auto.
    inversion H4; subst; auto.
    inversion H13; subst; auto.
    inversion H11; subst; auto;
      eauto using reduction.
    **
      inversion H_config; subst; auto.
      inversion H20; subst; auto.
      inversion H18; subst; auto.
      inversion H3; subst; auto.
      inversion H27. inversion H28. inversion H0. inversion H28. 
      exists (Config CT (Container (EqCmp e1 v) fs lb sf) ctns h). auto.
      inversion H28.
    **
      inversion H_config; subst; auto.
      inversion H20; subst; auto.
      inversion H18; subst; auto.
      inversion H3; subst; auto.
      inversion H29. inversion H0. inversion H29.
      eauto using reduction. inversion H29.   
    **
      inversion H_config; subst; auto.
      inversion H18; subst; auto.
      inversion H16; subst; auto.
      inversion H1; subst; auto; try (inversion H26).
      inversion H0.
      exists ( Config CT (Container (raiseLabel e1 v) fs lb sf) ctns h ). 
      eauto using reduction.
    **
      inversion H_config; subst; auto.
      inversion H19; subst; auto.
      inversion H17; subst; auto.
      inversion H2; subst; auto; try (inversion H28).
      inversion H0. 
      eauto using reduction. 
Qed. 
