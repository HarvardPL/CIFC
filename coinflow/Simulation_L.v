...
(* + try (invert_l_eq_ctn; exists φ; split;  auto; invert_l_eq_tm ). *)
  ...
  + invert_l_eq_ctn.
    invert_l_eq_tm.
    assert (value e2).
    apply value_L_eq with v2 h1' h2' φ; auto.
    intuition. 
...
- inversion H_reduction2; subst; auto; try (solve_by_invert_ctn).
  ...
  + invert_l_eq_ctn.
    ...
    pose proof config_typing_lead_to_tm_typing h1' ct  (EqCmp v1 v2) fs1 lb2 sf1 ctns_stack1' T H_typing1.
    destruct H4 as [Ttm]. destruct H4 as [gamma]. inversion H4; subst; auto.

    inversion H; subst; auto; inversion H7; subst; auto.
    (* v = ObjId o*)
    ++
      inversion H10; subst; auto.
      +++
        (* two objects are with low labels and low_eq *)
        inversion H0; subst; auto; inversion H25; subst; auto.
        ++++
          unfold runtime_val in H3. (* inversion H3; subst; auto. *)
          inversion H12; subst; auto.
          unfold runtime_label. unfold runtime_val. unfold object_label. rewrite <- H9.
          rewrite <- H26. rewrite <- H32. rewrite <- H34.
          remember ((join_label
          ...
          rewrite H6 in H30; inversion H30; subst; auto. 
          ...
          subst; auto. apply bijection.right_left in H6.
          apply bijection.right_left in H30. 
          rewrite H30 in H6; inversion H6; subst; auto. 
          ...
          apply L_equivalence_config_H; auto.
           ...
           unfold runtime_label. unfold runtime_val. unfold object_label. rewrite <- H9.
           rewrite <- H26. rewrite <- H32. rewrite <- H30.
        ...
        ++++
          ...
          rewrite H31. rewrite H30. auto.
          subst; auto.

        ++++
          inversion H12; subst; auto.
             +++++
               inversion H5; subst; auto; inversion H34; subst; auto.
             ++++++
               inversion H44; subst; auto.
             +++++++
               unfold runtime_label.
             unfold runtime_val. unfold object_label. rewrite <- H9. rewrite <- H26.              
             rewrite <- H41.             rewrite <- H43.
             ...
             * 
               inversion H3; subst; auto.
               rewrite H6 in H32; inversion H32; subst; auto. 
               assert (result0 = B_true). auto.
               assert (result = B_true). auto.
               rewrite H48. rewrite H47. auto. 
             *          
               unfold runtime_val in H2. unfold runtime_val in H17.
               assert (ObjId o2 <> ObjId o3). intro contra; inversion contra.
               subst; auto. apply bijection.right_left in H6.
               apply bijection.right_left in H32. 
               rewrite H32 in H6; inversion H6; subst; auto. 
               assert (result = B_false). auto.
               assert (result0 = B_false). auto.
               subst; auto.
             *
               ...
               unfold object_label. rewrite <- H9. rewrite <-H41. rewrite <- H32. rewrite <- H26.
             ...
             * unfold runtime_val in H2. unfold runtime_val in H17.
               ...
               rewrite H32. rewrite H39 . auto. 
             *          
               apply L_equivalence_config_H; auto.

               ++++++
                 destruct H30 with v0 lb0; auto.
               +++++
                 apply L_equivalence_config_H; auto.
               +++
                 assert (flow_to
    (join_label (join_label (join_label (runtime_label (ObjId o)) (runtime_label v2)) lb2)
             (join_label (object_label (runtime_val (ObjId o)) h1') (object_label (runtime_val v2) h1'))) L_Label = false).
                 ...
                 unfold runtime_val.                   unfold object_label. rewrite <- H6; auto. 
    ...
    (join_label (join_label (join_label (runtime_label (ObjId o2)) (runtime_label v3)) lb2)
             (join_label (object_label (runtime_val (ObjId o2)) h2') (object_label (runtime_val v3) h2'))) L_Label = false).
                 ...
                 unfold runtime_val.                   unfold object_label. rewrite <- H9; auto. 
                 apply join_def_flow1; auto. 
                 apply join_def_flow2; auto.
                 apply L_equivalence_config_H; auto.
  ...
  ++ inversion H10; subst; auto.
     ...
     +++
        ...
        ++++
          ...
          ***
          assert (result = B_false). auto.
          assert (result0 = B_false). apply H17; intro contra; inversion contra.
          rewrite H30. rewrite H31. auto.
          **
            eauto using L_equivalence_config_H; auto.
          *
            unfold runtime_label.  unfold runtime_val. unfold object_label. rewrite <- H6. rewrite <- H9.                 
   ...
   +++  inversion H12; subst; auto.
        ++++
          inversion H31; subst; inversion H26; subst; auto.
          * inversion H37; subst; auto.
            **
              unfold runtime_label. unfold runtime_val. unfold object_label. rewrite <- H34.
              rewrite <- H36.
            ...
            **
             ...
             (join_label (object_label null h1') (object_label (ObjId o) h1')))
              ...
              unfold object_label ;auto. rewrite <- H9; auto.
             ...
             (join_label (object_label null h2') (object_label (ObjId o2) h2')))
              ...
              unfold object_label ;auto. rewrite <- H34; auto.
  ...
  ++ (* v =  (v_opa_l v lb)  *)
    ...
    (* v2 cases *) 
     +++
       (* two objects are with low labels and low_eq *)
       inversion H5; subst; auto; inversion H26; subst; auto.
       ++++
         unfold runtime_val in H3.
         inversion H12; subst; auto.
          *
            ...
            **
              ...
              rewrite <- H40. rewrite <- H44. rewrite <- H46. 
            ...
            ****
              inversion H3; subst; auto.              
              rewrite H9 in H43; inversion H43; subst; auto. 
              assert (result0 = B_true). auto.
              assert (result = B_true). auto.
              subst; auto. 
            ****
              unfold runtime_val in H2. unfold runtime_val in H17.
               assert (ObjId o2 <> ObjId o3). intro contra; inversion contra.
               subst; auto. apply bijection.right_left in H9.
               apply bijection.right_left in H43. 
               rewrite H43 in H9; inversion H9; subst; auto. 
               assert (result = B_false). auto.
               assert (result0 = B_false). auto.
               subst; auto.
            ***
          ...
          **
            unfold runtime_val. unfold runtime_label. unfold object_label.
            rewrite <- H35.              rewrite <- H40. rewrite <- H43. rewrite <- H44. 
          ...
          * unfold runtime_val. unfold runtime_label.
            ...
            apply flow_transitive with (object_label (ObjId o) h1'); auto.
            unfold object_label. rewrite <- H9; auto. 
            apply join_def_flow2.
            apply join_def_flow2.
  ...
  +++
    ...
    ++++
      ...
      inversion H37; subst; auto.
       *
         unfold object_label. rewrite <- H34.
         rewrite <- H36.
   ...
   +++
       ...
       *
         inversion H37; subst; auto.
            **
              unfold runtime_label. unfold runtime_val.
              inversion H8; subst; auto; inversion H36; subst; auto.
              ***
                inversion H50; subst; auto.
                ****
                  unfold object_label. rewrite <- H46.
              rewrite <- H48. rewrite <- H52. rewrite <- H54. 
              ...
              *****
              ...
              destruct H3.
              {  inversion H3; subst; auto.              
              rewrite H34 in H49; inversion H49; subst; auto. 
              assert (result0 = B_true). auto.
              assert (result = B_true). auto.
              subst; auto.
              }
              {
                 unfold runtime_val in H2. unfold runtime_val in H17.
                 assert (ObjId o2 <> ObjId o3). intro contra; inversion contra.
               subst; auto. apply bijection.right_left in H34.
               apply bijection.right_left in H49. 
               rewrite H49 in H34; inversion H34; subst; auto. 
               assert (result = B_false). auto.
               assert (result0 = B_false). auto.
               subst; auto.
              }
              *****
              eauto using L_equivalence_config_H; auto.
                ****
                  unfold object_label. rewrite <- H46.
                  rewrite <- H48. rewrite <- H49. rewrite <- H52.
            ...
            **
              ...
              apply flow_transitive with (object_label (ObjId o) h1'); auto.
              unfold object_label. rewrite <- H34; auto. 
              apply join_def_flow1.
              apply join_def_flow2.
              ...
              apply flow_transitive with (object_label (ObjId o2) h2'); auto.
              unfold object_label. rewrite <- H46; auto. 
              apply join_def_flow1.
              apply join_def_flow2.
...
- inversion H_reduction2; subst; auto; try (solve_by_invert_ctn).
  ...
  + invert_l_eq_ctn.
    ...
    ++
      ...
      apply H1 in H23; auto; inversion H23.
      +++
        subst; auto.
      rewrite <- H11 in H; inversion H; subst; auto. 
      rewrite <- H12 in H4; inversion H4; subst; auto.
      rewrite <- H24 in H11; inversion H11; subst; auto. 
      rewrite <- H27 in H12; inversion H12; subst; auto. 

      assert (flow_to (join_label (join_label (join_label lo1 lb2) ell0) ll1) L_Label = false).
      apply flow_transitive with ((join_label lo1 lb2)); auto.
      apply flow_transitive with lo1; auto.
      apply join_def_flow1; auto.
      apply flow_trans with (join_label (join_label lo1 lb2) ell0); auto.
      apply join_def_flow1; auto.
      apply join_def_flow1; auto.
      ...
      +++
        ...
        rewrite <-H27 in H4; inversion H4; subst; auto.
         ...
         assert (flow_to (join_label (join_label (join_label lo1 lb2) ell0) ll1) L_Label = true).
        apply join_L_label_flow_to_L ; auto.
        apply join_L_label_flow_to_L ; auto.
        apply join_L_label_flow_to_L ; auto. 
        ...
        destruct H56 as [fields_defs'].
       
        destruct H57 as [F'].
        destruct H41 as [lo'].
        destruct H41 as [ll'].
        destruct H36 as [method_defs'].
        rewrite H41 in H27; inversion H27; subst; auto.
        rewrite <- H49 in H43; inversion H43; subst; auto.
        apply field_value with h2' o0 (class_def clsT0 fields_defs' method_defs') F' lo' fname0 ct cls'0 gamma2 ll'; auto.        
        apply  T_FieldAccess with (class_def clsT0 fields_defs' method_defs') clsT0 fields_defs'; auto.                

        destruct H35; subst; auto. 
        destruct H32 with fname0.
        assert (F2 fname0 = Some null). auto.
        apply H41 in H49; auto. rewrite <- H0 in H49; inversion H49; subst; auto.
        ...
        apply H35 in H51; auto. rewrite H51 in H13; inversion H13; subst; auto.

        destruct H35 as [o1].
        destruct H36. subst; auto.
        destruct H32 with fname0.
        assert (F1 fname0 = Some null).
        apply H36. auto.
        rewrite H41 in H0; inversion H0; subst; auto. 
        ...
        destruct H35. destruct H36. destruct H41. 
    ...
    *
        ...
        **
        apply L_equivalence_tm_eq_object_L with cls0 F0 lb cls0 F3 ll ; auto.
        apply H41.
        **
          apply L_equivalence_tm_eq_object_L with cls1 F0 lb cls0 F3 ll ; auto.
          apply H41. 
        * apply L_equivalence_tm_eq_object_H with cls_f1 cls_f2 FF1 lof1 FF2 lof2 llf1 llf2 ; auto;
            apply H41.
          +++
            ...
            rewrite <- H24 in H4; inversion H4; subst; auto.

            assert (flow_to (join_label (join_label (join_label lo1 lb2) ell0) ll1) L_Label = false).
            apply flow_transitive with ll1; auto.
            apply join_def_flow2; auto.

            assert (flow_to (join_label (join_label (join_label lo2 lb2) ell0) ll2) L_Label = false).
            apply flow_transitive with ll2; auto.
            apply join_def_flow2; auto.
  ...
  + invert_l_eq_ctn.
    ...
    *
      ...
      **
      ...
      inversion H12; subst; auto.
      destruct H38 as [F']. destruct H3 as [lo']. destruct H3 as [ll'].    
      rewrite H3 in H5; inversion H5; subst; auto.
      rewrite <- H10 in H32; inversion H32; subst; auto.
  ...
  + invert_l_eq_ctn.
    ...
    ++ 
      ...
      +++ assert (cls = cls0).
          ...
          inversion H27; subst; auto.          
          inversion H30; subst; auto.
          destruct H46 as [F']. destruct H3 as [lo']. destruct H3 as [ll'].
          rewrite H3 in H5; inversion H5; subst; auto.
          rewrite <- H34 in H36; inversion H36; subst; auto.
          rewrite H40 in H0; inversion H0; subst; auto.
...
- inversion H_reduction2; subst; auto; try (solve_by_invert_ctn).
  ...
  + invert_l_eq_ctn.
    invert_l_eq_tm.
    assert (value e1). apply value_L_eq with v h1' h2'  φ; auto.
    intuition.
...
(*L_eq_container (Container (labelData v lo) fs1 lb1 sf1) h1' (Container t2 fs2 lb2 sf2) h2 φ*)
  ...
  + invert_l_eq_ctn.
    invert_l_eq_tm.
    assert (value e2). apply value_L_eq with (l lo) h1' h2'  φ; auto.
    intuition.
    
  + invert_l_eq_ctn.
    invert_l_eq_tm.
    assert (value e2). apply value_L_eq with (v_opa_l (l lo) ell) h1' h2'  φ; auto.
    apply v_opa_labeled; auto.
    intros. intro contra; inversion contra. 
    intuition.

- inversion H_reduction2; subst; auto; try (solve_by_invert_ctn).
  (* (Container (labelData v1 t1) fs lb2 sf1) ctns_stack1' h1') *)
...
- inversion H_reduction2; subst; auto; try (solve_by_invert_ctn).
  (*  (Container (v_l v lo) fs1 lb2 sf1) *)
  + invert_l_eq_ctn.
    invert_l_eq_tm.
    assert (value e1). apply value_L_eq2 with v h1' h2'  φ; auto.
    intuition. 
  ...
  + invert_l_eq_ctn.
    invert_l_eq_tm.
    assert (value e2). apply value_L_eq2 with (l lo) h1' h2'  φ; auto.
    intuition.
...
- inversion H_reduction2; subst; auto; try (solve_by_invert_ctn).
  (* L_equivalence_config (Config ct (Container e2 (raiseLabel v hole :: fs1) lb2 sf1) ctns_stack1' h1')
     (Config ct ctn2' ctns_stack2' h2') φ' *)
  + invert_l_eq_ctn.
    invert_l_eq_tm.
    assert (value e1).
  ...
  + invert_l_eq_ctn.
    invert_l_eq_tm.
    assert (value e2).
    apply value_L_eq with (v_opa_l (l lo') ell) h1' h2 φ; auto.
    apply v_opa_labeled; auto.
    intros. intro contra; inversion contra.
    contradiction.
...
- inversion H_reduction2; subst; auto; try (solve_by_invert_ctn).
  ...
  + invert_l_eq_ctn.
    invert_l_eq_tm.
    assert (value e2).
    apply value_L_eq2 with (v_opa_l (l lo') ell) h1 h2' φ; auto.
    apply v_opa_labeled; auto.
    intros. intro contra; inversion contra.
    contradiction.
  ...
  + invert_l_eq_ctn.
    ...
    *
      ...
      apply lbl_raise_H_obj_lbl_preserve_bijection  with ct lo lo0; auto.
      eauto using lbl_raise_H_obj_lbl_preserve_bijection.
  ...
  + invert_l_eq_ctn.
    ...
    *
      ...
      **
        ...
        rewrite <- H13 in H6; inversion H6; subst; auto. 
        exists φ. split; auto.
        eauto using lbl_raise_H_obj_lbl_preserve_bijection.
        apply lbl_raise_H_obj_lbl_preserve_l_eq_config with lo1 lo2; auto. 
    *
      assert (flow_to (join_label lb2 ell0) L_Label = false).
      apply flow_join_label  with ell0 lb2; auto.
      assert (flow_to (join_label lb2 ell) L_Label = false).
      ...
      apply lbl_raise_H_obj_lbl_preserve_bijection  with ct lo lo0; auto.
      eauto using lbl_raise_H_obj_lbl_preserve_bijection.
      apply lbl_raise_H_obj_lbl_preserve_l_eq_config with lo lo0; auto.
      eauto using lbl_raise_H_obj_lbl_preserve_bijection.
      eauto using lbl_raise_H_obj_lbl_preserve_bijection.
...
- inversion H_reduction2; subst; auto; try (solve_by_invert_ctn).
  ...
  + invert_l_eq_ctn.
    ...
    apply flow_trans with (join_label lb2 ell); auto.
    apply join_def_flow1; auto.
    apply flow_trans with (join_label (join_label lb2 ell) ell'); auto.
    apply join_def_flow1; auto.     

    assert (flow_to lb2 ll0 = true). 
    apply flow_trans with (join_label lb2 ell0); auto. 
    apply join_def_flow1; auto.
    apply flow_trans with (join_label (join_label lb2 ell0) ell'0); auto.
    apply join_def_flow1; auto.

    assert (forall a1 a2 : oid, Decision (a1 = a2)). auto.
    exists φ. split; auto.
    *
      inversion H12; subst; auto. inversion H28; subst; auto.
      **
        inversion H10; subst; auto.
        ***
          apply change_obj_preserve_bijection with ct lo lo0 lb2 lb2; auto.
        ***
          ...
          apply flow_transitive with ell; auto.
          apply join_def_flow2; auto.
          apply flow_trans with  (join_label (join_label lb2 ell) ell'0); auto.
          apply join_def_flow1; auto.
      ...
      **
          ...
          assert (flow_to ll0 L_Label = false).
          apply flow_transitive with ell'0; auto.
          apply flow_trans with  (join_label (join_label lb2 ell0) ell'0); auto.
          apply join_def_flow2; auto.
          apply lbl_raise_H_obj_lbl_preserve_bijection with ct lo lo0; auto.
          eauto using  L_equivalence_tm_eq_object_H .
...
- inversion H_reduction2; subst; auto; try (solve_by_invert_ctn).
  ...
  + invert_l_eq_ctn.
    ...
    pose proof config_typing_lead_to_tm_typing h1 ct (FieldWrite (ObjId o) f0 v) fs1 lb2 sf1 ctns_stack1' T H_typing1.
    destruct H0 as [Ty1]. destruct H0 as [gamma1].
    inversion H0; subst; auto.

    pose proof config_typing_lead_to_tm_typing h2 ct (FieldWrite (ObjId o0) f0 v0) fs2 lb2 sf2 ctns_stack2' T H_typing2.
    destruct H2 as [Ty2]. destruct H2 as [gamma2].
    inversion H2; subst; auto.

    inversion H3; subst; auto; inversion H11; subst; auto.
    ++
      inversion H20; subst; auto; inversion H28; subst; auto.
      *
        destruct H32 as [F1]. destruct H4 as [lo1]. destruct H4 as [ll1].
        destruct H35 as [F2]. destruct H5 as [lo2]. destruct H5 as [ll2].
        exists φ. split; auto.
        **
        ...
        **
          ...
          ***
            ...
            assert (join_label L_Label lb2  = join_label lb2 L_Label).
            apply join_label_commutative. rewrite <- H27. auto. 
    ...
    ++ inversion H24; subst; auto.
       ...
       +++ (* v_opa_l low label *)
         ...
         ++++
           inversion H4; subst; auto; inversion H27; subst; auto.
           right. destruct H43 as [F1]. destruct H10 as [lo1]. destruct H10 as [ll1].
           exists o1. exists cls_def1. exists F1. exists lo1. exists ll1. 
           split; auto.
           destruct H36 with v lb0; auto.           
         ++++
           inversion H4; subst; auto; inversion H27; subst; auto.
           right. destruct H43 as [F1]. destruct H10 as [lo1]. destruct H10 as [ll1].
       ...
       +++ (* v_opa_l hight label *)
         ...
         ++++
           ...
           inversion H43; subst; auto; inversion H38; subst; auto.
           destruct H47 as [F2].  destruct H10 as [lo2]. destruct H10 as [ll2].
           right. exists o1. exists cls_def1. exists F2. exists lo2. exists ll2. 
           split; auto.
           destruct H45 with v lb0; auto.
         ++++
           unfold runtime_label in H1. rewrite <- H8; auto.
         ++++
           unfold runtime_label in H15.
...
- inversion H_reduction2; subst; auto; try (solve_by_invert_ctn).
  ...
  + invert_l_eq_ctn.
    ...
    pose proof config_typing_lead_to_tm_typing h1 ct (FieldWrite (v_opa_l (ObjId o) ell) f0 v) fs1 lb2 sf1 ctns_stack1' T H_typing1.
    destruct H0 as [Ty1]. destruct H0 as [gamma1].
    inversion H0; subst; auto.

    pose proof config_typing_lead_to_tm_typing h2 ct (FieldWrite (v_opa_l (ObjId o0) ell0) f0 v0) fs2 lb2 sf2 ctns_stack2' T H_typing2.
    destruct H2 as [Ty2]. destruct H2 as [gamma2].
    inversion H2; subst; auto.
      ...
      *
        ...
        **
          ...
          ***
             ...
             pose proof  join_label_commutative L_Label lb2.
             rewrite <- H23; auto.
             apply flow_trans with (join_label (join_label (runtime_label (ObjId o2)) lb2) ell0); auto. 
             unfold runtime_label. rewrite H23.
             apply  join_def_flow1; auto.
          ...
          ***            
            ...
            apply update_field_preserve_bijection_opa with ct L_Label L_Label lb2 lb2
                                                         ell ell0 ; auto. 
            unfold runtime_val. right.
          exists o1.  exists cls_def1. exists F1. exists lo1.  exists ll1. split; auto.
          unfold runtime_val. right.
        ...
        **
          ...
          ***
            assert (join_label L_Label lb2  = join_label lb2 L_Label).
            apply join_label_commutative.
            assert (flow_to (join_label lb2 L_Label) (join_label lo ll) = true). 
            apply flow_trans with (join_label (join_label (runtime_label (ObjId o2)) lb2) ell0); auto.
            unfold runtime_label. 
            rewrite <- H27; auto.
            apply join_def_flow1.

            assert (flow_to (join_label lb2 L_Label) (join_label lo0 ll0) = true). 
            apply flow_trans with (join_label (join_label (runtime_label (ObjId o2)) lb2) ell0); auto.
            unfold runtime_label. 
            rewrite <- H27; auto.
            apply join_def_flow1.
          ...
          ***
            ...
            apply flow_transitive with  (join_label (join_label (runtime_label (ObjId o1)) lb2) ell); auto.
            apply flow_transitive with ell; auto.
            apply join_def_flow2.
            assert ( flow_to (join_label lo0 ll0) L_Label = false).
            apply flow_transitive with  (join_label (join_label (runtime_label (ObjId o2)) lb2) ell0); auto.
            apply flow_transitive with ell0; auto.
            apply join_def_flow2. 

            apply L_equivalence_config_L; auto.
            apply update_field_preserve_L_eq_ctn_opa with ct L_Label lb2 L_Label lb2
                                                          ell ell0 ; auto.
            unfold runtime_val.
            apply L_equivalence_tm_eq_v_opa_l_L; auto;
             apply v_opa_labeled; auto;
               intros; intro contra; inversion contra.
            apply update_field_preserve_L_eq_ctns_opa with ct L_Label lb2 L_Label lb2
                                                          ell ell0 ; auto.
            unfold runtime_val.
            apply L_equivalence_tm_eq_v_opa_l_L; auto;
             apply v_opa_labeled; auto;
    ...
    ++ 
      ...
      assert (forall a1 a2 : oid, decision.Decision (a1 = a2)). auto.
      inversion H13; subst; auto.
      *        (*  flow_to ell0 L_Label = true *)
        assert (join_label L_Label lb2 =join_label lb2 L_Label).
        apply join_label_commutative.
        assert (flow_to (join_label lb2 L_Label) (join_label lo ll) = true).        
        rewrite <- H5; auto.
        apply flow_trans with (join_label (join_label (runtime_label null) lb2) ell0); auto.
        apply join_def_flow1.
        assert (flow_to (join_label lb2 L_Label) (join_label lo0 ll0) = true).        
        rewrite <- H5; auto.
        apply flow_trans with (join_label (join_label (runtime_label null) lb2) ell0); auto.
      ...
      *
        ...
        assert ( flow_to (join_label lo ll) L_Label = false).
        apply flow_transitive with  (join_label (join_label (runtime_label null) lb2) ell); auto.
        apply flow_transitive with ell; auto.
        apply join_def_flow2.
        ...
        split; auto.
        **
          apply update_field_preserve_bijection_opa with ct L_Label L_Label lb2 lb2 ell ell0; auto.
          unfold runtime_label in H1. rewrite <- H10. auto.
          unfold runtime_label in H15. rewrite <- H10. auto. 
        ...
        **
          apply L_equivalence_config_L; auto.
          apply update_field_preserve_L_eq_ctn_opa with ct lb2 L_Label lb2 L_Label
                                                        ell ell0 ; auto.
          unfold runtime_label in H1. rewrite <- H10. auto.
          unfold runtime_label in H15. rewrite <- H10. auto.
          apply  L_equivalence_tm_eq_v_opa_l_L; auto.
          ...
          apply update_field_preserve_L_eq_ctns_opa with ct lb2 L_Label lb2 L_Label
                                                        ell ell0 ; auto.
          unfold runtime_label in H1. rewrite <- H10. auto.
          unfold runtime_label in H15. rewrite <- H10. auto.
          apply  L_equivalence_tm_eq_v_opa_l_L; auto.
    ...
    ++ assert (join_label lb lb2  =join_label lb2 lb).
       ...
       *
       (* v_opa_l low label *)
         ...
         ++++
           inversion H4; subst; auto; inversion H27; subst; auto.
           right. destruct H47 as [F1]. destruct H10 as [lo1]. destruct H10 as [ll1].
           exists o1. exists cls_def1. exists F1. exists lo1. exists ll1. 
           split; auto.
           destruct H36 with v lb0; auto.           
         ++++
           ...
           **
             destruct H36 with v lb0; auto. 
             ++++
               apply flow_trans with (join_label (join_label lb2 lb) ell0); auto.
               apply join_def_flow1.
             ++++
               apply flow_trans with (join_label (join_label lb2 lb) ell0); auto.
               apply join_def_flow1.
       ...
       *
         assert (flow_to (join_label lo ll) L_Label = false).
         apply flow_transitive with  (join_label (join_label lb2 lb) ell); auto.
         apply flow_transitive with ell; auto.
         apply join_def_flow2.
         
         assert (flow_to (join_label lo0 ll0) L_Label = false).
         apply flow_transitive with  (join_label (join_label lb2 lb) ell0); auto.
         apply flow_transitive with ell0; auto.
         apply join_def_flow2.

         exists φ. split; auto.
         ++++
         apply update_field_preserve_bijection_opa with ct lb lb lb2 lb2 ell ell0; auto.
         **
           inversion H4; subst; auto; inversion H27; subst; auto.
           right. destruct H49 as [F1]. destruct H37 as [lo1]. destruct H37 as [ll1].
           exists o1. exists cls_def1. exists F1. exists lo1. exists ll1. 
           split; auto.
           destruct H36 with v lb0; auto.
         ...
         **
           ...
           ***
             ...
             ++++
               ...
               assert (forall a1 a2 : oid, decision.Decision (a1 = a2)). auto.          
               apply L_equivalence_config_L; auto.
               apply update_field_preserve_L_eq_ctn_opa with  ct lb2 lb lb2 lb ell ell0; auto.
               apply update_field_preserve_L_eq_ctns_opa  with  ct lb2 lb lb2 lb ell ell0; auto.
       ...
       +++ (* v_opa_l hight label *)
          ...
          inversion H13; subst; auto.
       *
       (* v_opa_l of object being written has low label *)
         split; auto.
         apply update_field_preserve_bijection_new with ct lb l2 lb2 lb2; auto.
         ++++
           inversion H4; subst; auto; inversion H27; subst; auto.
         ...
         ++++
           inversion H4; subst; auto; inversion H27; subst; auto.
           +++++
           destruct H49 as [F1]. destruct H37 as [lo1]. destruct H37 as [ll1].
           inversion H41; subst; auto.
           inversion H28; subst; auto. 
           inversion H43; subst; auto; inversion H50; subst; auto.
           destruct H58 as [F2]. destruct H38 as [lo2]. destruct H38 as [ll2].
           **
           right. exists o2. exists cls_def2. exists F2. exists lo2. exists ll2. 
           split; auto.
           **
             destruct H45 with v lb0; auto.
           +++++
             inversion H28; subst; auto.
           inversion H49; subst; auto; inversion H40; subst; auto.
           **
             destruct H53 as [F2]. destruct H37 as [lo2]. destruct H37 as [ll2].
             right. exists o1. exists cls_def1. exists F2. exists lo2. exists ll2. 
             split; auto.
           **
             destruct H51 with v lb0; auto.
           +++++
             destruct H36 with v lb0; auto.
        ++++
           apply flow_trans with  (join_label (join_label (runtime_label (v_opa_l v1 lb)) lb2) ell0); auto.
           rewrite <- H8.
           apply join_def_flow1.
        ++++
          apply flow_trans with  (join_label (join_label (runtime_label (v_opa_l e2 l2)) lb2) ell0); auto.
           rewrite <- H29.
           apply join_def_flow1.
        ++++
          inversion  H_valid1; subst;  auto.
          ...
          assert (flow_to (join_label lb2 lb) (join_label lo ll) = true). 
          unfold runtime_label in H1.
          rewrite <- H8; auto.
          apply flow_trans with (join_label (join_label lb lb2) ell0); auto.
          apply join_def_flow1.

          assert (flow_to (join_label lb2 l2) (join_label lo0 ll0) = true). 
          unfold runtime_label in H15. 
          rewrite <- H29; auto.
          apply flow_trans with (join_label (join_label l2 lb2) ell0); auto.
          apply join_def_flow1.
          ...
          apply update_field_preserve_L_eq_ctns  with  ct lb2 lb lb2 l2; auto.
       *
         (* v_opa_l of object being written has high label *)
         unfold runtime_label in H1.
         unfold runtime_label in H15. 
         assert (join_label lb lb2 =join_label lb2 lb).
         apply join_label_commutative.
         ...
         split; auto.
         apply update_field_preserve_bijection_opa with ct lb l2 lb2 lb2 ell ell0; auto.
         ++++
           inversion H4; subst; auto; inversion H27; subst; auto.
           right. destruct H51 as [F1]. destruct H39 as [lo1]. destruct H39 as [ll1].
           exists o1. exists cls_def1. exists F1. exists lo1. exists ll1. 
           split; auto.
           destruct H36 with v lb0; auto.           
         ++++
           inversion H4; subst; auto; inversion H27; subst; auto.
           +++++
           destruct H51 as [F1]. destruct H39 as [lo1]. destruct H39 as [ll1].
           inversion H41; subst; auto.
           inversion H28; subst; auto. 
           inversion H46; subst; auto; inversion H52; subst; auto.
           destruct H60 as [F2]. destruct H40 as [lo2]. destruct H40 as [ll2].
           **
           right. exists o2. exists cls_def2. exists F2. exists lo2. exists ll2. 
           split; auto.
           **
             destruct H49 with v lb0; auto.
           +++++
             inversion H28; subst; auto.
           inversion H51; subst; auto; inversion H45; subst; auto.
           **
             destruct H55 as [F2]. destruct H39 as [lo2]. destruct H39 as [ll2].
             right. exists o1. exists cls_def1. exists F2. exists lo2. exists ll2. 
             split; auto.
           **
             destruct H53 with v lb0; auto.
           +++++
             destruct H36 with v lb0; auto.
        ++++
          inversion  H_valid1; subst;  auto.
          inversion H52; subst; auto.
          inversion  H_valid2; subst;  auto.
          inversion H52; subst; auto. 
          ...
          apply join_def_flow2. 
          assert (flow_to (join_label lo0 ll0) L_Label = false).
          apply flow_transitive with  (join_label (join_label lb2 l2) ell0); auto.
          apply flow_transitive with ell0; auto.
          apply join_def_flow2.
          
          apply L_equivalence_config_L; auto.
          inversion H61; subst; auto. 

          apply update_field_preserve_L_eq_ctn_opa with  ct lb2 lb lb2 l2 ell ell0; auto.                    
          apply update_field_preserve_L_eq_ctns_opa  with  ct lb2 lb lb2 l2 ell ell0; auto.
