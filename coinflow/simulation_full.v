...
(* objectLabelof v*)
  ...
  (* raise 1 *)  
  - split; auto.
    apply change_obj_lbl_h1_with_H_preserve_bijection with ct lo lb1' bot; auto.
    inversion H_valid1; auto.
    assert (join_label lb1' bot = lb1').
    apply join_bot_same.
    rewrite H2; auto.

    inversion H_valid1; subst; auto.
    inversion H_valid2; subst; auto.
    apply change_obj_lbl_h1_preserve_config_eq with lo lb1' bot; auto.
    apply L_equivalence_config_H; auto.
    destruct ctns1'.
  ...
  - split; auto.
    ...
    unfold low_component in H19.
    unfold low_component.
    rewrite   H_flow1'. rewrite H17 in H19. auto.
    assert (join_label lb1 bot = lb1).
    apply join_bot_same.
    rewrite H2; auto.
    apply flow_trans with (join_label lb1 ell) ; auto.
  ...
  - split; auto.
    ...
    apply flow_trans with (join_label lb1' ell) ; auto.
    eauto using join_def_flow1.
    apply flow_trans with (join_label (join_label lb1' ell) ell') ; auto.
    eauto using join_def_flow1. 
...
(*field update normal *)
  - assert ((join_label lb1' bot) = lb1').
    apply join_bot_same ; auto.

    assert (flow_to lb1' (join_label  (runtime_label t1')  lb1') = true).
    ...
    apply update_h1_with_H_preserve_bijection with ct lb1' bot; auto.
    inversion H_valid1; auto.
    rewrite H0; auto.
    

    assert (forall a1 a2 : oid, decision.Decision (a1 = a2)). auto.     
    apply update_field_h1_preserve_config_eq with lb1' bot; auto.
  ...
  - assert ((join_label lb1' (runtime_label t1')) = (join_label (runtime_label t1') lb1')).
    apply join_label_commutative.
    assert (flow_to (join_label lb1' (runtime_label t1')) (join_label lo ll) = true).
    rewrite H0. 
    apply flow_trans with (join_label (join_label (runtime_label t1') lb1') ell); auto. 
    apply join_def_flow1.

    split; auto.
    apply update_h1_with_H_preserve_bijection with ct lb1' (runtime_label t1') ; auto.
    inversion H_valid1; auto.    

    assert (forall a1 a2 : oid, decision.Decision (a1 = a2)). auto.     
    apply update_field_h1_preserve_config_eq with lb1' (runtime_label t1') ; auto.
    inversion H_valid2. auto.
    inversion H_valid1; auto.
...
+ assert (flow_to (join_label lb2 ell) L_Label = false).
  ...
  + split; auto.
    ...
    assert ((low_component ct (Container e fs1' lb1' sf1') ctns1' h1') =
           Config  ct (Container e fs1' lb1' sf1') ctns1' h1').
    apply low_component_with_L_Label; auto.
    assert ((low_component ct (Container e0 fs2' lb2' sf2') ctns2' h2') =
           Config  ct (Container e0 fs2' lb2' sf2') ctns2' h2').
    apply low_component_with_L_Label; auto.
...
(* objectLabelOf v*)
  ...
  - split; auto.
    inversion H_valid1; auto. 
    inversion H_valid2; auto. 
    apply change_obj_lbl_h2_with_H_preserve_bijection with ct lo lb2' bot ;auto.
    subst; auto.
    assert ((join_label lb2' bot) = lb2').
    apply join_bot_same; auto. rewrite H2. auto. 

    inversion H_valid1; subst; auto. 
    inversion H_valid2; subst; auto. 
    apply change_obj_lbl_h2_preserve_config_eq with lo lb2' bot; auto.
  ...
  - split; auto.
    ...
    subst; auto.
    assert ((join_label lb2 bot) = lb2).
    apply join_bot_same ; auto. rewrite H2.
    ...
    rewrite  H_flow2'. rewrite H18 in H19. auto.
    unfold low_component in H19.
    unfold low_component.
    rewrite  H_flow2'. rewrite H18 in H19. auto.
 ...
 - assert ((join_label lb2' bot) = lb2').
    ...
    assert (flow_to lb2' (join_label  (runtime_label t2')  lb2') = true).
    apply join_def_flow2.
    assert (flow_to lb2' (join_label lo ll) = true).
    eauto using flow_trans. 
    
    split; auto.       
    apply update_h2_with_H_preserve_bijection with ct lb2' bot; auto.
    inversion H_valid2; auto.
    inversion H_valid1; auto. 
    rewrite H0; auto.
    ...
    assert (forall a1 a2 : oid, decision.Decision (a1 = a2)). auto.     
    apply update_field_h2_preserve_config_eq with lb2' bot; auto.
    inversion H_valid2. auto.
    inversion H_valid1; auto.
    ...
    (*field update v_opa_l *)
  - assert ((join_label lb2' (runtime_label t2')) = (join_label (runtime_label t2') lb2')).
    apply join_label_commutative.
    assert (flow_to (join_label lb2' (runtime_label t2')) (join_label lo ll) = true).
    rewrite H0. 
    apply flow_trans with (join_label (join_label (runtime_label t2') lb2') ell); auto. 
    apply join_def_flow1.

    split; auto.
    apply update_h2_with_H_preserve_bijection with ct lb2' (runtime_label t2') ; auto.
    inversion H_valid1; auto.    

    assert (forall a1 a2 : oid, decision.Decision (a1 = a2)). auto.     
    apply update_field_h2_preserve_config_eq with lb2' (runtime_label t2') ; auto.
    inversion H_valid2. auto.
    inversion H_valid1; auto.
