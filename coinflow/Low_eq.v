...
Inductive L_equivalence_tm : tm -> heap -> tm -> heap ->  (bijection oid oid )->  Prop :=
  ...
  | L_equivalence_tm_eq_labelData : forall e1 e2 el1 el2 h1 h2  φ,
      L_equivalence_tm e1 h1 e2 h2  φ->
      L_equivalence_tm el1 h1 el2 h2  φ->
      L_equivalence_tm (labelData e1 el1) h1 (labelData e2 el2) h2  φ
  ...
  | L_equivalence_tm_eq_raiseLabel : forall e1 e2 h1 h2 el1 el2 φ,
      L_equivalence_tm e1 h1 e2 h2  φ->
      L_equivalence_tm el1 h1 el2 h2  φ->
      L_equivalence_tm (raiseLabel e1 el1) h1 (raiseLabel e2 el2) h2  φ
  | L_equivalence_tm_eq_toLabeled : forall e1 e2 el1 el2 h1 h2  φ,
      L_equivalence_tm e1 h1 e2 h2  φ->
      L_equivalence_tm el1 h1 el2 h2  φ->
      L_equivalence_tm (toLabeled e1 el1) h1 (toLabeled e2 el2) h2  φ
...
match ctn with 
 | (Container t fs lb sf) =>
       if (flow_to lb L_Label) then (Config ct (Container t fs lb sf) ctns_stack h) 
                ...
                | nil => (Config ct (Container hole nil L_Label empty_stack_frame) nil h) 
...
(* heap bijection preservation if raise obj on H objects *)
Lemma lbl_raise_H_obj_lbl_preserve_bijection {DecOid : forall a1 a2 : oid, Decision (a1 = a2)} : forall  ct h1 h2 φ lo lo0 ll ll0 lo' lo0' F F0 o o0 cls cls0,
    wfe_heap ct h1 -> field_wfe_heap ct h1 ->
    wfe_heap ct h2 -> field_wfe_heap ct h2 ->  
    L_equivalence_heap h1 h2 φ ->
    Some (Heap_OBJ cls F lo ll) = lookup_heap_obj h1 o ->
    ...
    L_equivalence_tm (ObjId o) h1 (ObjId o0) h2 φ ->
    flow_to ll L_Label = false ->
    flow_to ll0 L_Label = false ->
    flow_to lo lo' = true ->
    flow_to lo0 lo0' = true ->
    L_equivalence_heap
        (update_heap_obj h1 o (Heap_OBJ cls F lo' ll))
        (update_heap_obj h2 o0 (Heap_OBJ cls0 F0 lo0' ll0)) φ.
...
Proof with eauto.
  ...
  + intros.
    ...
    *
      ...
      apply H11 in H16. 
      inversion H16; subst; auto.
      rewrite <- H17 in H4; inversion H4; subst; auto.
      try (inconsist).
    ...
    *
      apply H11 in H16; inversion H16; subst; auto.
      **
        assert ( Some (Heap_OBJ cls2 F1 lb ll1)  =
             lookup_heap_obj (update_heap_obj h1 o (Heap_OBJ cls F lo' ll))  o1).
        apply lookup_updated_not_affected with o (Heap_OBJ cls F lo' ll)  h1; auto.
        intro contra. subst; auto.
        assert (beq_oid o o = true). apply beq_oid_same. try (inconsist).
        assert ( Some (Heap_OBJ cls2 F2 lb ll1)  =
                 lookup_heap_obj  (update_heap_obj h2 o0 (Heap_OBJ cls0 F0 lo0' ll0))  o2).
        apply lookup_updated_not_affected with o0 (Heap_OBJ cls0 F0 lo0' ll0) h2; auto.
        intro contra. subst; auto.
        rewrite <- H19 in H5; inversion H5; subst; auto.
        try (inconsist).
        eauto using object_equal_L.
      **
        assert ( Some (Heap_OBJ cls1 F1 lb ll1) =
             lookup_heap_obj (update_heap_obj h1 o (Heap_OBJ cls F lo' ll))  o1).
        apply lookup_updated_not_affected with o (Heap_OBJ cls F lo' ll)  h1; auto.
        intro contra. subst; auto.
        assert (beq_oid o o = true). apply beq_oid_same. try (inconsist).
        ...
        rewrite <- H19 in H5; inversion H5; subst; auto.
        try (inconsist).
        apply object_equal_L with lb cls1 cls2 F1 F2 ll1; auto.
        destruct H22. destruct H25. destruct H26.  
        split; auto. split; auto.
        split; auto. 

        intros.
        destruct H27 with fname fo1 fo2; auto. rename x into cls_f1.
        destruct H30 as [cls_f2].
        destruct H30 as [lof1].
        destruct H30 as [lof2]. destruct H30 as [FF1]. destruct H30 as [FF2].
        destruct H30. destruct H30.  destruct H30.
        destruct H31.
        destruct H32.
        ***
          (* fields are both L *)
          ...
          apply beq_oid_equal in H33.
          subst; auto.
          destruct H32. destruct H32. destruct H33. destruct H33. 
          rewrite H30 in H4; inversion H4; subst; auto.
          try (inconsist).
          assert (fo2 <> o0).
          intro contra. subst; auto.
          rewrite H31 in H5; inversion H5; subst; auto.
          destruct H32. destruct H32. destruct H34. destruct H34. 
          try (inconsist).
          
          apply beq_oid_not_equal in H34. 

          (* beq_oid fo1 o = false*)
          (* beq_oid fo2 o0 = false*)
          assert ( Some (Heap_OBJ cls_f1 FF1 lof1 x)  =
                 lookup_heap_obj  (update_heap_obj h1 o (Heap_OBJ cls F lo' ll))  fo1).
          apply lookup_updated_not_affected with o (Heap_OBJ cls F lo' ll)  h1; auto.
          intro contra; subst; auto.
          assert (beq_oid o o = true). apply beq_oid_same. try (inconsist).
          
          assert (Some (Heap_OBJ cls_f2 FF2 lof2 x0) =
                  lookup_heap_obj (update_heap_obj h2 o0   (Heap_OBJ cls0 F0 lo0' ll0)) fo2
                 ).
          apply lookup_updated_not_affected with o0  (Heap_OBJ cls0 F0 lo0' ll0) h2; auto.
          intro contra; subst; auto.
          assert (beq_oid o0 o0 = true). apply beq_oid_same. try (inconsist).
        ...
        ***
          ...
          ****
            apply beq_oid_equal in H33; subst; auto.
            assert (Some (Heap_OBJ cls F lo' ll) = lookup_heap_obj (update_heap_obj h1 o (Heap_OBJ cls F lo' ll)) o
                 ).
            apply lookup_updated with h1 (Heap_OBJ cls_f1 FF1 lof1 x); auto.
            case_eq (beq_oid fo2 o0); intro.
            *****
              apply beq_oid_equal in H33; subst; auto.
            assert (Some (Heap_OBJ cls0 F0 lo0' ll0)  =
                  lookup_heap_obj (update_heap_obj h2 o0 (Heap_OBJ cls0 F0 lo0' ll0)) o0 
                 ).
          apply lookup_updated with h2 (Heap_OBJ cls_f2 FF2 lof2 x0); auto.

          exists cls. exists cls0.
          exists lo'. exists lo0'.
          exists F. exists F0.
          exists ll. exists ll0. 
          split; auto.
  ...
  +
    (* None -> left φ o1 = None *)
    intros. apply lookup_updated_heap_must_none in H16.
    apply H12 in H16. subst; auto.
  ...
  + (* flow_to lb L_Label = false  *)
    intros.
    case_eq (beq_oid o1 o); intro.
    ++ apply beq_oid_equal in H18. subst; auto.
       apply H14 with cls F lo ll; auto.  
    ++
      assert ( Some (Heap_OBJ cls1 F1 lb ll1) = lookup_heap_obj (update_heap_obj h1 o (Heap_OBJ cls F lo' ll)) o1). auto.
      apply lookup_updated_heap_old_obj in H19; auto.
      apply H14 with cls1 F1 lb ll1; auto. 

  + (*flow_to lb L_Label = false -> right φ o1 = None *)
    intros.
    case_eq (beq_oid o1 o0); intro.
    ++ apply beq_oid_equal in H18; subst; auto.
       apply H15 with cls0 F0 lo0 ll0; auto. 
    ++ 
    assert ( Some (Heap_OBJ cls1 F1 lb ll1) = lookup_heap_obj (update_heap_obj h2 o0 (Heap_OBJ cls0 F0 lo0' ll0)) o1); auto. 
    apply lookup_updated_heap_old_obj in H19; auto.
    apply H15 with cls1 F1 lb ll1; auto. 
Qed. Hint Resolve lbl_raise_H_obj_lbl_preserve_bijection.


Lemma lbl_raise_H_obj_lbl_preserve_l_eq_tm {DecOid : forall a1 a2 : oid, Decision (a1 = a2)} : forall  ct h1 h2 φ lo lo0 ll ll0 lo' lo0' F F0 o o0 cls cls0 t1 t2,
    wfe_heap ct h1 -> field_wfe_heap ct h1 ->
    wfe_heap ct h2 -> field_wfe_heap ct h2 ->  
    L_equivalence_heap h1 h2 φ ->
    Some (Heap_OBJ cls F lo ll) = lookup_heap_obj h1 o ->
    ...
    flow_to lo0 lo0' = true ->
    L_equivalence_tm t1 (update_heap_obj h1 o (Heap_OBJ cls F lo' ll))
                     t2  (update_heap_obj h2 o0 (Heap_OBJ cls0 F0 lo0' ll0)) φ.
Proof with eauto.
  intros.
  inversion H6; subst; auto.
  - generalize dependent t2.
    ...
    + 
      ...
      ++
        ...
        assert (Some (Heap_OBJ cls4 F4 lo2 ll2) =
              lookup_heap_obj (update_heap_obj h2 o0 (Heap_OBJ cls0 F0 lo0' ll0)) o3 ).
        apply lookup_updated_not_affected with o0 (Heap_OBJ cls0 F0 lo0' ll0) h2; auto.
        apply  L_equivalence_tm_eq_object_L with cls3 F3 lo2 cls4 F4 ll2; auto.
    ...
    + assert (o1 <> o). intro contra.
      ...
      assert (o3 <> o0). intro contra.
      subst; auto.
      rewrite <- H17 in H18; inversion H18; subst; auto.
      try (inconsist).
  ...
  - rewrite <- H4 in H14; inversion H14; subst; auto.
    rewrite <- H5 in H15; inversion H15; subst; auto.
    ...
    + assert (o1 <> o). intro contra.
      ...
      assert (o3 <> o0). intro contra.
      subst; auto.
      rewrite <- H20 in H5; inversion H5; subst; auto.
      try (inconsist).
    ...
    + case_eq (beq_oid o1 o); intro.
      ++
        ...
        apply lookup_updated with h1 (Heap_OBJ cls1 F1 lo1 ll1); auto.
        case_eq (beq_oid o3 o0); intro.
      * apply beq_oid_equal in H21; subst; auto.
        rewrite <- H17 in H5; inversion H5; subst; auto.
        assert (Some (Heap_OBJ cls2 F2 lo0' ll2) =
                lookup_heap_obj (update_heap_obj h2 o0 (Heap_OBJ cls2 F2 lo0' ll2)) o0 ).        
        apply lookup_updated with h2 (Heap_OBJ cls2 F2 lo2 ll2); auto.
        eauto using  L_equivalence_tm_eq_object_H.
      ...
      * assert (Some (Heap_OBJ cls2 F2 lo2 ll2) =
        ...
        intro contra; subst; auto. pose proof (beq_oid_same o0); try (inconsist).
        eauto using  L_equivalence_tm_eq_object_H.
Qed. Hint Resolve   lbl_raise_H_obj_lbl_preserve_l_eq_tm . 
...
Lemma lbl_raise_H_obj_lbl_preserve_l_eq_fs {DecOid : forall a1 a2 : oid, Decision (a1 = a2)} : forall  ct h1 h2 φ lo lo0 ll ll0 lo' lo0' F F0 o o0 cls cls0 fs1 fs2,
    wfe_heap ct h1 -> field_wfe_heap ct h1 ->
    wfe_heap ct h2 -> field_wfe_heap ct h2 ->  
    L_equivalence_heap h1 h2 φ ->
    Some (Heap_OBJ cls F lo ll) = lookup_heap_obj h1 o ->
    ...
    L_equivalence_fs fs1 h1 fs2 h2 φ ->
    L_equivalence_fs fs1 (update_heap_obj h1 o (Heap_OBJ cls F lo' ll))
                     fs2  (update_heap_obj h2 o0 (Heap_OBJ cls0 F0 lo0' ll0)) φ.
Proof with eauto.
  intros.
  generalize dependent fs2.
  induction fs1; intros; inversion H11; subst; auto.
  apply L_equal_fs; auto.
  eauto using  lbl_raise_H_obj_lbl_preserve_l_eq_tm.
Qed. Hint Resolve  lbl_raise_H_obj_lbl_preserve_l_eq_fs.  

Lemma lbl_raise_H_obj_lbl_preserve_l_eq_store {DecOid : forall a1 a2 : oid, Decision (a1 = a2)} : forall  ct h1 h2 φ lo lo0 ll ll0 lo' lo0' F F0 o o0 cls cls0 sf1 sf2,
    wfe_heap ct h1 -> field_wfe_heap ct h1 ->
    wfe_heap ct h2 -> field_wfe_heap ct h2 ->  
    L_equivalence_heap h1 h2 φ ->
    Some (Heap_OBJ cls F lo ll) = lookup_heap_obj h1 o ->
    ...
    L_equivalence_store sf1 h1 sf2 h2 φ ->
    L_equivalence_store sf1 (update_heap_obj h1 o (Heap_OBJ cls F lo' ll))
                     sf2  (update_heap_obj h2 o0 (Heap_OBJ cls0 F0 lo0' ll0)) φ.
Proof with eauto.
  intros.  
  inversion H11; subst; auto. 
  apply L_equivalence_store_L; auto.
  split; auto. intros. 
  assert (L_equivalence_tm v1 h1 v2 h2 φ).
  apply H12 with x; auto.
  eauto using  lbl_raise_H_obj_lbl_preserve_l_eq_tm.
  apply H12.   
Qed. Hint Resolve lbl_raise_H_obj_lbl_preserve_l_eq_store. 


Lemma lbl_raise_H_obj_lbl_preserve_l_eq_ctn {DecOid : forall a1 a2 : oid, Decision (a1 = a2)} : forall  ct h1 h2 φ lo lo0 ll ll0 lo' lo0' F F0 o o0 cls cls0 ctn1 ctn2,
    wfe_heap ct h1 -> field_wfe_heap ct h1 ->
    wfe_heap ct h2 -> field_wfe_heap ct h2 ->  
    L_equivalence_heap h1 h2 φ ->
    Some (Heap_OBJ cls F lo ll) = lookup_heap_obj h1 o ->
    ...
    L_eq_container ctn1 h1 ctn2 h2 φ ->
    L_eq_container ctn1 (update_heap_obj h1 o (Heap_OBJ cls F lo' ll))
                     ctn2  (update_heap_obj h2 o0 (Heap_OBJ cls0 F0 lo0' ll0)) φ.
Proof with eauto.
  intros.  
  generalize dependent ctn2. 
  induction ctn1; intros;subst; auto;
  ...
  apply  L_eq_ctn; auto.
  eauto using lbl_raise_H_obj_lbl_preserve_l_eq_tm.
  eauto using lbl_raise_H_obj_lbl_preserve_l_eq_fs.
  eauto using lbl_raise_H_obj_lbl_preserve_l_eq_store.
Qed. Hint Resolve lbl_raise_H_obj_lbl_preserve_l_eq_ctn.

Lemma lbl_raise_H_obj_lbl_preserve_l_eq_ctns {DecOid : forall a1 a2 : oid, Decision (a1 = a2)} : forall  ct h1 h2 φ lo lo0 ll ll0 lo' lo0' F F0 o o0 cls cls0 ctns1 ctns2,
    wfe_heap ct h1 -> field_wfe_heap ct h1 ->
    wfe_heap ct h2 -> field_wfe_heap ct h2 ->  
    L_equivalence_heap h1 h2 φ ->
    Some (Heap_OBJ cls F lo ll) = lookup_heap_obj h1 o ->
    ...
    L_eq_ctns ctns1 h1 ctns2 h2 φ ->
    L_eq_ctns ctns1 (update_heap_obj h1 o (Heap_OBJ cls F lo' ll))
                     ctns2  (update_heap_obj h2 o0 (Heap_OBJ cls0 F0 lo0' ll0)) φ.
    Proof with eauto.
  intros.  
  generalize dependent ctns2. 
  induction ctns1; intros;subst; auto;
  inversion H11; subst; auto.
  apply  L_eq_ctns_list; auto.
  eauto using lbl_raise_H_obj_lbl_preserve_l_eq_ctn. 
Qed. Hint Resolve lbl_raise_H_obj_lbl_preserve_l_eq_ctns. 


    Lemma lbl_raise_H_obj_lbl_preserve_l_eq_config {DecOid : forall a1 a2 : oid, Decision (a1 = a2)} :
      forall  ct h1 h2 φ lo lo0 ll ll0 lo' lo0' F F0 o o0 cls cls0
              t1 fs1 sf1 lb1
              t2 fs2 sf2 lb2 ctns1 ctns2,
    wfe_heap ct h1 -> field_wfe_heap ct h1 ->
    wfe_heap ct h2 -> field_wfe_heap ct h2 ->  
    L_equivalence_heap h1 h2 φ ->
    Some (Heap_OBJ cls F lo ll) = lookup_heap_obj h1 o ->
    ...
    flow_to lb2 L_Label = true ->
     L_equivalence_config (Config ct (Container t1 fs1 lb1 sf1) ctns1  h1)
                          (Config ct (Container t2 fs2 lb2 sf2) ctns2  h2) φ ->
     L_equivalence_config (Config ct (Container t1 fs1 lb1 sf1) ctns1  (update_heap_obj h1 o (Heap_OBJ cls F lo' ll)))
                          (Config ct (Container t2 fs2 lb2 sf2) ctns2  (update_heap_obj h2 o0 (Heap_OBJ cls0 F0 lo0' ll0))) φ .
    Proof with eauto.
      intros.
  ...
  generalize dependent sf1. generalize dependent sf2.
  generalize dependent ctns1. generalize dependent ctns2. 
  induction H13; subst; intros; inversion Heqconfig1; inversion Heqconfig2; subst; auto.
  apply L_equivalence_config_L; auto.
  assert (forall a1 a2 : oid, Decision (a1 = a2)). auto.
    apply lbl_raise_H_obj_lbl_preserve_l_eq_ctn with ct lo lo0; auto. 
    assert (forall a1 a2 : oid, Decision (a1 = a2)). auto.
    apply lbl_raise_H_obj_lbl_preserve_l_eq_ctns with ct lo lo0; auto.
Qed. Hint Resolve lbl_raise_H_obj_lbl_preserve_l_eq_config. 

      (* heap bijection preservation *)
Lemma update_field_preserve_bijection_opa : forall ct h1 h2 φ cls F lo
                                                 cls0 F0 lo0
                                                 lx lx0 v v0 f0
                                                 o o0 lb1 lb2 ll ll0 ell ell0
                                                 ,
    ...
    Some (Heap_OBJ cls F lo ll) = lookup_heap_obj h1 o ->
    Some (Heap_OBJ cls0 F0 lo0 ll0) = lookup_heap_obj h2 o0 ->
    flow_to (join_label (join_label lb1 lx) ell) (join_label lo ll) = true ->
    flow_to (join_label (join_label lb2 lx0) ell0) (join_label lo0 ll0) = true ->
    L_equivalence_tm (v_opa_l (ObjId o) ell) h1 (v_opa_l (ObjId o0) ell0) h2 φ ->
    flow_to lb1 L_Label = true ->
    flow_to lb2 L_Label = true ->
    flow_to ell L_Label = false ->
    flow_to ell0 L_Label = false ->
    L_equivalence_tm (v_opa_l v lx) h1 (v_opa_l v0 lx0) h2 φ ->
    L_equivalence_heap (update_heap_obj h1 o (Heap_OBJ cls (fields_update F f0 v) lo ll))
                       (update_heap_obj h2 o0 (Heap_OBJ cls0 (fields_update F0 f0 v0) lo0 ll0)) φ.
...
Proof with eauto.
  ...
  (*  flow_to ll1 L_Label = true /\ flow_to ll1 L_Label = false*)
    ...
    apply flow_trans with (join_label (join_label lb1 lx) ell); auto.
    apply join_def_flow2.

    assert (flow_to (join_label lo0 ll0) L_Label = false).
    apply flow_transitive with ell0; auto.
    apply flow_trans with (join_label (join_label lb2 lx0) ell0); auto.
    apply join_def_flow2.  
    ...
    + intros. assert ( L_equivalence_object o1 h1 o2 h2 φ). 
      ...
      ++ 
        ...
        +++
          apply beq_oid_equal in H31; subst; auto. 
          assert (Some (Heap_OBJ cls (fields_update F f0 v) lb ll) =
            lookup_heap_obj (update_heap_obj h1 o (Heap_OBJ cls (fields_update F f0 v) lb ll)) o
             ).
          eauto using lookup_updated.
          rewrite <- H27 in H4; inversion H4; subst; auto.
          case_eq (beq_oid o2 o0); intro.
          apply beq_oid_equal in H32; subst; auto.
          *
          assert (Some (Heap_OBJ cls0 (fields_update F0 f0 v0) lo0 ll0)  =
            lookup_heap_obj (update_heap_obj h2 o0 (Heap_OBJ cls0 (fields_update F0 f0 v0) lo0 ll0)) o0
             ).
          eauto using lookup_updated.          
          ...
          *
            assert (Some (Heap_OBJ cls2 F2 lb ll1) =
                   lookup_heap_obj (update_heap_obj h2 o0 (Heap_OBJ cls0 (fields_update F0 f0 v0) lo0 ll0)) o2
                 ).
            apply lookup_updated_not_affected with o0  (Heap_OBJ cls0 (fields_update F0 f0 v0) lo0 ll0) h2; auto.
            intro contra. subst; auto. pose proof (beq_oid_same o0); try (inconsist).
            eauto using object_equal_L.
     ...
     ++
       ...
       +++
          ...
          eauto using lookup_updated.
          rewrite <- H27 in H4; inversion H4; subst; auto.
          assert (flow_to (join_label lb ll1) L_Label = true).
          apply join_L_label_flow_to_L; auto.  try (inconsist).
       +++
          assert (Some  (Heap_OBJ cls1 F1 lb ll1) =
            lookup_heap_obj (update_heap_obj h1 o (Heap_OBJ cls (fields_update F f0 v) lo ll)) o1
          ...
          *
            ...
            **
              rewrite H44 in H41.
              rewrite H41 in H4; inversion H4; subst; auto.
              assert (Some (Heap_OBJ cls_f1 (fields_update FF1 f0 v) lof2 llf1) =
                      lookup_heap_obj (update_heap_obj h1 o (Heap_OBJ cls_f1 (fields_update FF1 f0 v) lof2 llf1)) o
             ).
              eauto using lookup_updated.
              destruct H43.
              ***
              destruct H43. destruct H44. destruct H45. subst; auto.             
              case_eq (beq_oid fo2 o0); intro.
              **** 
                apply beq_oid_equal in H44; subst; auto.
                rewrite H42 in H5; inversion H5; subst; auto.
                assert (Some (Heap_OBJ cls_f2 (fields_update FF2 f0 v0) lof2 llf2) =
                     lookup_heap_obj (update_heap_obj h2 o0 (Heap_OBJ cls_f2 (fields_update FF2 f0 v0) lof2 llf2)) o0
             ).
                eauto using lookup_updated.
              ...
              ****
                assert (Some  (Heap_OBJ cls_f2 FF2 lof2 llf2) =
                       lookup_heap_obj (update_heap_obj h2 o0 (Heap_OBJ cls0 (fields_update F0 f0 v0) lo0 ll0)) fo2
                      ).
               apply lookup_updated_not_affected with o0 (Heap_OBJ cls0 (fields_update F0 f0 v0) lo0 ll0) h2; auto.
               intro contra. subst; auto. pose proof (beq_oid_same o0); try (inconsist).
               exists cls_f1. exists cls_f2.
            ...
            **
              assert (Some  (Heap_OBJ cls_f1 FF1 lof1 llf1)  =
                       lookup_heap_obj (update_heap_obj h1 o (Heap_OBJ cls (fields_update F f0 v) lo ll)) fo1
                      ).
               apply lookup_updated_not_affected with o  (Heap_OBJ cls (fields_update F f0 v) lo ll) h1; auto.
              intro contra. subst; auto. pose proof (beq_oid_same o); try (inconsist).
              case_eq (beq_oid fo2 o0); intro.
              *** 
                apply beq_oid_equal in H46; subst; auto.
                rewrite H42 in H5; inversion H5; subst; auto.
                assert (Some (Heap_OBJ cls_f2 (fields_update FF2 f0 v0) lof2 llf2) =
                     lookup_heap_obj (update_heap_obj h2 o0 (Heap_OBJ cls_f2 (fields_update FF2 f0 v0) lof2 llf2)) o0
             ).
                ...
                exists cls_f1. exists cls_f2.
                exists lof1. exists lof2.
                exists FF1. exists (fields_update FF2 f0 v0).
                exists llf1. exists llf2.   
                split; auto.
              ***
                assert (Some  (Heap_OBJ cls_f2 FF2 lof2 llf2) =
               ...
               split; auto.
    + intros.
      apply lookup_updated_heap_must_none in H25.
      apply H17 in H25; auto. 

    + intros. 
      apply lookup_updated_heap_must_none in H25.
      apply H19 in H25; auto.
    ...
    + intros.
      ...
      ++ 
        apply  beq_oid_equal in H27.
        subst; auto.
        assert (Some (Heap_OBJ cls (fields_update F f0 v) lo ll)  =
                lookup_heap_obj (update_heap_obj h1 o (Heap_OBJ cls (fields_update F f0 v) lo ll)) o ).
        eauto using lookup_updated.
        apply H21 with cls F lo ll; auto.           
        rewrite H25 in H27; inversion H27; subst; auto.  
      ++
        assert (Some (Heap_OBJ cls1 F1 lb ll1) =
              lookup_heap_obj h1 o1
             ).
        eauto using  lookup_updated_heap_old_obj .
        destruct H21 with o1 cls1 F1 lb ll1; auto.
    ...
    + intros.
      ...
      ++
        ...
        apply H22 with cls1 F0 lb ll1; auto.
      ...
      ++
        ...
        eauto using  lookup_updated_heap_old_obj .
        destruct H22 with o1 cls1 F1 lb ll1; auto.
Qed. Hint Resolve update_field_preserve_bijection_opa. 


Lemma update_field_preserve_L_eq_tm_opa {DecOid : forall a1 a2 : oid, Decision (a1 = a2)} :
  forall  (φ:bijection oid oid) h1 h2 ct cls F lo o lb1 lx
          cls0 F0 lo0 o0 lb2 lx0 t1 t2 v v0 f0 ll ll0 ell ell0,
  wfe_heap ct h2 ->  wfe_heap ct h1 -> 
  L_equivalence_heap h1 h2 φ ->
  L_equivalence_tm t1 h1 t2 h2 φ ->
  Some (Heap_OBJ cls F lo ll) = lookup_heap_obj h1 o ->
  ...
  L_equivalence_tm t1 (update_heap_obj h1 o (Heap_OBJ cls (fields_update F f0 v) lo ll))
                    t2 (update_heap_obj h2 o0 (Heap_OBJ cls0 (fields_update F0 f0 v0) lo0 ll0)) φ.
Proof with eauto. 
  intros φ h1 h2 ct cls F lo o lb1 lx cls0 F0 lo0 o0 lb2 lx0 t1 t2 v v0 f0 ll ll0 ell ell0.
  intros.
  ...
  -
    case_eq (beq_oid o1 o); intro.
    +
      apply beq_oid_equal in H20.
      remember (update_heap_obj h1 o (Heap_OBJ cls (fields_update F f0 v) lo ll))  as h'.
      assert (Some  (Heap_OBJ cls (fields_update F f0 v) lo ll) = lookup_heap_obj h' o).
      apply lookup_updated with h1 (Heap_OBJ cls1 F1 lo1 ll1); rewrite <- H20; subst;  auto.
      case_eq (beq_oid o3 o0); intro.
      ++
        apply beq_oid_equal in H26.
        remember (update_heap_obj h2 o0 (Heap_OBJ cls0 (fields_update F0 f0 v0) lo0 ll0))  as h2'.
        assert (Some   (Heap_OBJ cls0 (fields_update F0 f0 v0) lo0 ll0)  = lookup_heap_obj h2' o0).
        apply lookup_updated with h2 (Heap_OBJ cls2 F2 lo1 ll1) ; rewrite <- H26; subst;  auto.

        assert (L_equivalence_object o1 h1 o3 h2 φ). 
        apply H15; auto.  inversion H28; subst; auto.
      *
        rewrite <- H29 in H22; inversion H22; subst; auto.
        rewrite <- H30 in H24; inversion H24; subst; auto. 
        apply L_equivalence_tm_eq_object_L with cls (fields_update F f0 v) lo cls0
                                          (fields_update F0 f0 v0) ll; subst; auto.
        try (rewrite_lookup).
        rewrite <- H3 in H29; inversion H29; subst; auto.
        rewrite <- H4 in H30; inversion H30; subst; auto.
      *
        rewrite <- H29 in H22; inversion H22; subst; auto.
        rewrite <- H30 in H24; inversion H24; subst; auto. 
        apply L_equivalence_tm_eq_object_L with cls (fields_update F f0 v) lo cls0
      ...
      ++
       ...
       apply L_equivalence_tm_eq_object_L with cls (fields_update F f0 v) lo cls2
                                          F2 ll; subst; auto.
       try (rewrite_lookup).
       rewrite <- H3 in H22; inversion H22; subst; auto.
       remember (update_heap_obj h2 o0 (Heap_OBJ cls0 (fields_update F0 f0 v0) lo0 ll0))  as h2'.
       assert (Some (Heap_OBJ cls2 F2 lo ll) = lookup_heap_obj h2' o3).
       apply lookup_updated_not_affected with  o0 (Heap_OBJ cls0 (fields_update F0 f0 v0) lo0 ll0) h2; auto.
       intro contra. rewrite contra in H26. pose proof (beq_oid_same o0).
       try (inconsist). auto.
    +
      remember (update_heap_obj h1 o (Heap_OBJ cls (fields_update F f0 v) lo ll))  as h1'. 
      assert (Some (Heap_OBJ cls1 F1 lo1 ll1) = lookup_heap_obj h1' o1).
      apply lookup_updated_not_affected with  o (Heap_OBJ cls (fields_update F f0 v) lo ll) h1; auto.
      intro contra. rewrite contra in H20. pose proof (beq_oid_same o).
       try (inconsist). 
       case_eq (beq_oid o3 o0); intro.
      ++
       apply beq_oid_equal in H26.
     ...
     *
       rewrite <- H29 in H22; inversion H22; subst; auto.
       rewrite <- H30 in H24; inversion H24; subst; auto.
       apply L_equivalence_tm_eq_object_L with  cls4 F3 lb 
                                          cls0 (fields_update F0 f0 v0) ll2; subst; auto.
       rewrite <- H4 in H30; inversion H30; subst; auto.
      *
        rewrite <- H29 in H22; inversion H22; subst; auto.
        rewrite <- H30 in H24; inversion H24; subst; auto. 
        apply L_equivalence_tm_eq_object_L with cls3 F3 lb
                                          cls0 (fields_update F0 f0 v0) ll0; subst; auto.
        try (rewrite_lookup).
        rewrite <- H4 in H30; inversion H30; subst; auto.
        rewrite <- H4 in H30; inversion H30; subst; auto.
      ++
  ...
  -
    inversion H2; subst; auto.
    rewrite <- H27 in H21; inversion H21; subst; auto.
    try (inconsist).
    ...
    +
      ...
      ++
        ...
        remember (update_heap_obj h2 o0 (Heap_OBJ cls0 (fields_update F0 f0 v0) lo0 ll0))  as h2'.
        assert (Some   (Heap_OBJ cls0 (fields_update F0 f0 v0) lo0 ll0)  = lookup_heap_obj h2' o0).
        apply lookup_updated with h2 (Heap_OBJ cls4 F4 lo4 ll4) ; rewrite <- H29; subst;  auto.
        subst; auto.

        apply L_equivalence_tm_eq_object_H with cls cls0 (fields_update F f0 v) lo 
                                                (fields_update F0 f0 v0) lo0 ll ll0; subst; auto.
        try (rewrite_lookup).
        try (rewrite_lookup).
      ++
        subst; auto.
        remember (update_heap_obj h2 o0 (Heap_OBJ cls0 (fields_update F0 f0 v0) lo0 ll0))  as h2'.
       assert (Some (Heap_OBJ cls4 F4 lo4 ll4) = lookup_heap_obj h2' o3).
       apply lookup_updated_not_affected with  o0 (Heap_OBJ cls0 (fields_update F0 f0 v0) lo0 ll0) h2; auto.
       intro contra. rewrite contra in H29. pose proof (beq_oid_same o0).
       try (inconsist).
       apply L_equivalence_tm_eq_object_H with cls cls4 (fields_update F f0 v) lo 
                                                F4 lo4 ll ll4; subst; auto.
       try (rewrite_lookup).
    +
      remember (update_heap_obj h1 o (Heap_OBJ cls (fields_update F f0 v) lo ll))  as h1'. 
      assert (Some (Heap_OBJ cls1 F1 lo1 ll1) = lookup_heap_obj h1' o1).
      ...
      ++
       ...
       assert (Some   (Heap_OBJ cls0 (fields_update F0 f0 v0) lo0 ll0)  = lookup_heap_obj h2' o0).
       apply lookup_updated with h2 (Heap_OBJ cls4 F4 lo4 ll4) ; rewrite <- H29; subst;  auto.
       apply L_equivalence_tm_eq_object_H with cls1 cls0 F1 lo1 
                                                (fields_update F0 f0 v0) lo0 ll1 ll0; subst; auto.
       try (rewrite_lookup).
      ++
        remember (update_heap_obj h2 o0 (Heap_OBJ cls0 (fields_update F0 f0 v0) lo0 ll0))  as h2'.
       assert (Some (Heap_OBJ cls4 F4 lo4 ll4) = lookup_heap_obj h2' o3).
       ...
       intro contra. rewrite contra in H29. pose proof (beq_oid_same o0).
       try (inconsist).
       apply L_equivalence_tm_eq_object_H with cls1 cls4 F1 lo1 
                                               F4 lo4 ll1 ll4; subst; auto.
Qed. Hint Resolve update_field_preserve_L_eq_tm_opa. 



Lemma update_field_preserve_L_eq_fs_opa {DecOid : forall a1 a2 : oid, Decision (a1 = a2)} :
  forall  (φ:bijection oid oid) h1 h2 ct cls F lo o lb1 lx
          cls0 F0 lo0 o0 lb2 lx0 fs1 fs2 v v0 f0 ll ll0 ell ell0,
  wfe_heap ct h2 ->  wfe_heap ct h1 -> 
  L_equivalence_heap h1 h2 φ ->
  L_equivalence_fs fs1 h1 fs2 h2 φ ->
  Some (Heap_OBJ cls F lo ll) = lookup_heap_obj h1 o ->
...
Proof with eauto. 
  ...
  induction fs1; intros; inversion H2; subst; auto.
  apply  L_equal_fs; auto.
  eauto using  update_field_preserve_L_eq_tm_opa. 
Qed. Hint Resolve update_field_preserve_L_eq_fs_opa.


Lemma update_field_preserve_L_eq_store_opa {DecOid : forall a1 a2 : oid, Decision (a1 = a2)} :
  forall  (φ:bijection oid oid) h1 h2 ct cls F lo o lb1 lx
          cls0 F0 lo0 o0 lb2 lx0 sf1 sf2 v v0 f0 ll ll0 ell ell0,
  wfe_heap ct h2 ->  wfe_heap ct h1 -> 
  L_equivalence_heap h1 h2 φ ->
  L_equivalence_store sf1 h1 sf2 h2 φ ->
  Some (Heap_OBJ cls F lo ll) = lookup_heap_obj h1 o ->
...
Proof with eauto. 
  ...
  apply  update_field_preserve_L_eq_tm_opa  with ct lb1 lx lb2 lx0 ell ell0; auto.
Qed. 
Hint Resolve update_field_preserve_L_eq_store_opa.




Lemma update_field_preserve_L_eq_ctn_opa {DecOid : forall a1 a2 : oid, Decision (a1 = a2)} :
  forall  (φ:bijection oid oid) h1 h2 ct cls F lo o lb1 lx
          cls0 F0 lo0 o0 lb2 lx0 ctn1 ctn2 v v0 f0 ll ll0 ell ell0,
  wfe_heap ct h2 ->  wfe_heap ct h1 -> 
  L_equivalence_heap h1 h2 φ ->
  L_eq_container ctn1 h1 ctn2 h2 φ ->
  Some (Heap_OBJ cls F lo ll) = lookup_heap_obj h1 o ->
...
Proof with eauto.
  ...
  auto.
  apply update_field_preserve_L_eq_tm_opa with ct lb1 lx lb2 lx0 ell ell0; auto.
  apply update_field_preserve_L_eq_fs_opa with ct lb1 lx lb2 lx0 ell ell0; auto.
  apply update_field_preserve_L_eq_store_opa with ct lb1 lx lb2 lx0 ell ell0; auto.
Qed. Hint Resolve update_field_preserve_L_eq_ctn_opa.
  
Lemma update_field_preserve_L_eq_ctns_opa {DecOid : forall a1 a2 : oid, Decision (a1 = a2)} :
    forall  (φ:bijection oid oid) h1 h2 ct cls F lo o lb1 lx
          cls0 F0 lo0 o0 lb2 lx0 ctns1 ctns2 v v0 f0 ll ll0 ell ell0,
  wfe_heap ct h2 ->  wfe_heap ct h1 -> 
  L_equivalence_heap h1 h2 φ ->
  L_eq_ctns ctns1 h1 ctns2 h2 φ ->
  Some (Heap_OBJ cls F lo ll) = lookup_heap_obj h1 o ->
...
Proof with eauto.
  ...
  induction ctns1; intros; inversion H2; subst; auto.  
  apply L_eq_ctns_list; auto.  
  apply update_field_preserve_L_eq_ctn_opa with ct lb1 lx lb2 lx0 ell ell0; auto.
  inversion H15; auto. 
  inversion H16; auto. 
  apply   IHctns1; auto. 
  inversion H15; auto. 
  inversion H16; auto. 
Qed. Hint Resolve update_field_preserve_L_eq_ctns_opa.
