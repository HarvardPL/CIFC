...
(* Field Write *)
  ...
  | ST_fieldWrite_normal_opa_obj : forall o h h' f lo lb cls F F' v ct fs ctns sf ell ll,
      Some (Heap_OBJ cls F lo ll) = lookup_heap_obj h o -> 
      F' = fields_update F f (runtime_val v) ->
      flow_to (join_label (join_label (runtime_label v) lb) ell) (join_label lo ll) = true -> 
      h' = update_heap_obj h o (Heap_OBJ cls F' lo ll) ->
      value v ->
  ...
  | ST_fieldWrite_leak1 : forall o h f lo lb cls F v ct fs ctns sf ll,
      Some (Heap_OBJ cls F lo ll) = lookup_heap_obj h o ->
  ...
  | ST_fieldWrite_leak2 : forall o h f lo lb cls F v ct fs ctns sf ll ell,
      Some (Heap_OBJ cls F lo ll) = lookup_heap_obj h o ->
      flow_to (join_label (join_label (runtime_label v) lb) ell) (join_label lo ll) = false -> 
      value v ->
    Config ct (Container (FieldWrite (v_opa_l (ObjId o) ell) f v) fs lb sf ) ctns h 
           ==>  Error_state
...
(* sequence *)
   (* context rule *)
  | ST_seq1 : forall h e1 e2 ct fs ctns lb sf,
      ~ value e1 -> 
      Config ct (Container (Sequence e1 e2) fs lb sf) ctns h ==> Config ct (Container e1 ((Sequence hole e2) :: fs) lb sf) ctns h

  | ST_seq2 : forall h v e2 ct fs ctns lb sf,
      value v ->
      Config ct (Container v ((Sequence hole e2) :: fs) lb sf) ctns h ==> Config ct (Container (Sequence v e2)  fs lb sf) ctns h

  | ST_seq3 : forall h v e2 ct fs ctns lb sf,
      value v ->
    Config ct (Container  (Sequence v e2) fs lb sf) ctns h ==> Config ct (Container e2 fs lb sf) ctns h

  | ST_val_return_obj : forall h ct ctns sf lb' sf' fs' v lb e,
      (*ctn must have return_hole*)
      value v ->
      (forall v0 ell, value v0 -> v <> (v_opa_l v0 ell))  ->
    Config ct (Container v nil lb sf) ((Container e fs' lb' sf') :: ctns) h 
           ==> Config ct (Container (v_opa_l v lb) fs' lb' sf') ctns h
           
  | ST_val_return_obj_ell : forall h ct ctns sf lb lb' sf' fs' ell v e,
    (*ctn must have return_hole*)
    Config ct (Container (v_opa_l v ell) nil lb sf) ((Container e fs' lb' sf') :: ctns) h 
