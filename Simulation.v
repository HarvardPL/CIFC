...
Fixpoint erasure_obj_null (t: option tm) (h:heap) : (option tm) :=
  match t with 
    ...
    | Some (ObjId o) => match (lookup_heap_obj h o) with 
                              | Some (Heap_OBJ cls fields lb) => if (flow_to lb L_Label) then Some dot else Some (ObjId o)
...
Fixpoint change_obj_label_heap (h:heap) (o : oid ): heap := 
    match h with 
      | nil => nil
      | h1 :: t => match h1 with 
                            | ( o0 , (Heap_OBJ cls F lb) ) => if (beq_oid o o0) then ( o0 , (Heap_OBJ cls F H_Label)) :: t
                                                                                  else h1 :: (change_obj_label_heap t o)
                      end
    end. 
...
(* Field Write *)
  ...
  (* field write object *)
  | Er_fieldWrite_obj : forall s h f ct o ,
      Config ct (FieldWrite (ObjId o) f dot) (SIGMA s h) =eH=> Config ct dot ((SIGMA s (change_obj_label_heap h o)))
...
Inductive multi_step_erasure_H : config -> config -> Prop :=
  | H_erasure_single_step : forall ct  t sigma t' sigma', 
          ...
          (t' = dot \/  ( flow_to (current_label sigma') L_Label = true)) ->
...
intro H_erasure1.    intro H_erasure2.
   remember (Config ct t1 sigma1) as t1_config. 
    ...
    induction H_erasure1; intros; inversion Heqt1_config; subst; 
      ...
      - intuition. assert (exists o0 : oid, ObjId o = ObjId o0). exists o. auto.
       ...
       - intuition. destruct H7.  inversion H1.  inversion H1. 
       - intuition. assert (exists o0 : oid, ObjId o = ObjId o0). exists o. auto. intuition. 
          intuition. assert (exists o0 : oid, ObjId o = ObjId o0). exists o. auto. intuition. 
       - inversion H4. auto.
...
Proof. intros  ct t sigma t1 t2 sigma1 sigma2.
   ...
   remember (Config ct t1 sigma1) as t1_config. 
    ...
    subst. inversion H8. inversion H2. inversion H4. 
    rewrite H0 in H7. inversion H7.
    intros. inversion Heqt1_config. subst.  
   
    inversion H_erasure2. destruct H7. subst. 
    assert (t' = dot /\ sigma' = sigma2). 
    apply erasure_H_deterministic with ct t0 sigma0; auto.
   destruct H1.  subst.  inversion H_erasure1. inversion H4. inversion H6. 
...
Qed. 
(*
Lemma erasure_sigma_fun_preserve_label : forall s h s' h', 
    SIGMA s' h'  = erasure_sigma_fun (SIGMA s h) ->
     (current_label (SIGMA s h)) =  (current_label (SIGMA s' h')).
Proof. 
    intros. unfold erasure_sigma_fun in H. unfold erasure_stack in H.
    induction s. inversion H. unfold current_label. unfold get_current_label.
    unfold get_stack. auto. 
unfold current_label. unfold get_current_label.
    unfold get_stack. unfold get_stack_label. auto. 
*)
...
Lemma induction_multi_step_erasure_H: forall e f sigma ct e' sigma', 
    ...
    e = dot \/ exists e0, multi_step_erasure_H (Config ct e sigma)  (Config ct e0 sigma').
Proof. 
...
Lemma reduction_preserve_erasure_equal : forall e e' sigma sigma' ct,
   Config ct e sigma ==> Config ct e' sigma' ->
      flow_to (current_label sigma) L_Label = false ->  
...
Proof. 
  intros CT t t0 t' t0' s h s' h' sigma_l_e sigma_r_e.
  ...
  generalize dependent T.
    ...
    - inversion Heqconfig. inversion Heqconfig'. subst. inversion H_erasure_l. subst.
        rewrite H_flow in H3. inversion H3. admit.
  ...
  -   inversion Heqconfig. inversion Heqconfig'. subst.  inversion H_erasure_l. subst.
        ...
        + subst. inversion H_erasure_r. subst. 
            ++ assert (Config CT e' (SIGMA s' h') =e=> Config CT (erasure_L_fun e') (SIGMA s'1 h'1)).
              ...
              destruct H1. subst.
              inversion H_typing. subst. inversion H7.  subst. inversion H6.  subst.  inversion H_reduction.
              ...
              unfold erasure_L_fun in H14. inversion H14.  subst.  rewrite H2 in H12. inversion H12. 
              subst. 
...
++ subst. admit. 
  -   inversion Heqconfig. inversion Heqconfig'. subst.  inversion H_erasure_l. subst. 
      ...
      + subst.  assert (t0 = dot \/ (exists t_r, t0 = (MethodCall dot id0 t_r) /\ 
        ...
        inversion H_erasure_r. subst. ++++++ assert (Config CT e' (SIGMA s' h') =e=>Config CT (erasure_L_fun e') (SIGMA s'1 h'1)).
       apply erasure_L_context; auto.  (*destruct IHH_reduction with h' s'  h s  *)
...
induction H_reduction; intros; inversion Heqconfig; inversion Heqconfig'; subst; auto.
    ...
    unfold subset in H_context. inversion H_context. 
      ...
      + inversion H_erasure. subst. rewrite H3 in H_context. inversion H_context. subst.
...
+  inversion H10. subst. rewrite <- H13. unfold get_heap. unfold current_label. unfold get_stack.
...
+ (* FieldWrite e1 f e2 *)
     inversion H_erasure.   rewrite H3 in H_context. inversion H_context. subst. 
      Lemma field_write_context_erasure_obj : forall CT e1 e2 s s'0 h h'0 e_r f, 
   multi_step_erasure_H (Config CT (FieldWrite e1 f e2) (SIGMA s h))
       (Config CT e_r (SIGMA s'0 h'0)) ->
    e1 = dot 
    \/ (exists s'1 h'1 o, multi_step_erasure_H (Config CT e1 (SIGMA s h)) (Config CT (ObjId o) (SIGMA s'1 h'1)) 
        /\ multi_step_erasure_H (Config CT (FieldWrite dot f e2) (SIGMA s'1 h'1)) (Config CT e_r (SIGMA s'0 h'0)) )
    \/ (e_r = dot /\ multi_step_erasure_H (Config CT e1 (SIGMA s h))
       (Config CT dot (SIGMA s'0 h'0)) /\ e2 = dot) 
    \/ (exists e0, multi_step_erasure_H (Config CT e1 (SIGMA s h))
       (Config CT e0 (SIGMA s'0 h'0))  /\ e_r = (FieldWrite e0 f e2))
  ...
  Proof. intros CT e1 e2 s s'0 h h'0 e_r f. intro H_erasure.
    remember (Config CT (FieldWrite e1 f e2)  (SIGMA s h)) as config.    remember (Config CT e_r (SIGMA s'0 h'0))  as config_r.
    generalize dependent e1.     generalize dependent e2. generalize dependent s. generalize dependent h. 
   generalize dependent e_r. generalize dependent s'0. generalize dependent h'0. 
    induction H_erasure; intros; inversion Heqconfig; inversion Heqconfig_r; subst. 
   + destruct H0. ++ subst. inversion H. subst.  left. auto. subst. right. left. split; auto. split; auto. exists o. auto. 
    (*+++ auto. +++ right. left. subst. split. auto. split; auto. inversion H. subst.   apply H_erasure_single_step; auto. auto.  *)
    ++ subst. inversion H. 
    +++ subst. right. right. right. left.  exists e1'. split.  apply H_erasure_single_step; auto. auto.  
    +++ left. auto. +++ left. auto.  +++ subst. right. left. split; auto. split; auto.  exists o. auto.  
   + subst. inversion H.
    ++ subst. induction sigma'. destruct IHH_erasure with h'0 s'0 e_r h0 s0 e2 e1'; auto.  
      +++ subst. inversion H_erasure. subst. ++++ destruct H9. +++++ subst.  inversion  H4. subst. 
        right. right. left. split; auto.  split; auto.  apply H_erasure_single_step; auto.
      +++++  subst. right.  right. right.  right. exists s0. exists h0. split; apply H_erasure_single_step; auto.
       ++++ subst. inversion H7. +++++ subst.  inversion H4. 
      +++++ subst. right. right. right.  right. exists s0. exists h0. split. apply H_erasure_single_step; auto. 
            apply H_erasure_multi_step with(FieldWrite dot f e2') sigma'; auto.
      +++++ subst. inversion H11. inversion H4. inversion H8. 
      +++ subst. destruct H1. ++++ destruct H1. subst. destruct H2. subst. 
      destruct H1 as [o]. subst. inversion H3. 
 
        destruct IHH_erasure with h'0 s'0 dot h0 s0 dot e1'; auto. +++++ subst.  destruct H1. inversion H1. 
        +++++ destruct H2. destruct H2. 
       ++++ destruct H1. destruct H3. right. subst. left. split. auto.  split; auto. 
         apply H_erasure_multi_step with e'  (SIGMA s0 h0); auto. 
      ++++ subst. destruct H1. +++++ destruct H1 as [e0]. destruct H1. subst. 
          right. right. left. exists e0.  split; auto. apply H_erasure_multi_step with e'  (SIGMA s0 h0); auto.
      +++++ destruct H1 as [s'1]. destruct H1 as [h'1].  destruct H1. subst. right. right. right. 
...
+ (* FieldWrite (ObjId o) f v *) 
  ...
  unfold get_heap. unfold current_label. unfold get_stack.
      induction s0.  unfold current_label in H_context. unfold get_stack in H_context. 
         unfold get_current_label in H_context. unfold flow_to in H_context. unfold L_Label in H_context.
    unfold subset in H_context. inversion H_context. 
        induction a. unfold current_label. unfold get_stack. inversion H12. subst. auto. 
...
Qed. 
 ...
 Lemma erasure_sigma_equal_after_erasure : forall ct t t' sigma sigma', 
    ...
    erasure_sigma_fun sigma =     erasure_sigma_fun sigma'.
 Proof. 
    intros ct t t' sigma sigma'.  intro H_erasure. intro H_flow. intro H_exclude_FW. remember (Config ct t sigma) as config.
    ...
    - admit. - unfold erasure_sigma_fun. unfold erasure_stack. unfold current_label in H_flow. 
      unfold get_stack in H_flow.  unfold get_current_label in H_flow. induction lsf. unfold get_stack_label in H_flow.
      rewrite H_flow. fold erasure_stack. auto. 
...
(*do a induction *)
        ...
        inversion H13.  subst. inversion H4. subst. inversion H_reduction. subst. inversion H10. +++subst. inversion H3. 
        +++ subst. induction sigma'.  inversion H15.  subst. inversion H9. subst. 
           destruct IHH_reduction with h'1 s'1 h_r s_r h' s' e' h s e ; auto. apply  H_erasure_single_step; auto. 
          apply erasure_H_context with s'1 h'1; auto.    apply  H_erasure_single_step; auto. 
          subst. inversion H16. inversion H4. subst. 
...
Proof.
   intros  e e' s h s' h' CT s'1 h'1 s_r h_r e_r. intro T. intro H_typing.
   intro H_reduction. 
   intro H_H_context. intro H_L_context. 
   intro H_erasure_l. intro H_erasure_r. 
   remember (Config CT e (SIGMA s h)) as config.    remember (Config CT e' (SIGMA s' h')) as config'.
   generalize dependent e. generalize dependent s. generalize dependent h. 
   generalize dependent e'. generalize dependent s'. generalize dependent h'. 
    generalize dependent T. 
     generalize dependent e_r. generalize dependent s_r. generalize dependent h_r.
   induction H_reduction; intros; subst; inversion Heqconfig; inversion Heqconfig'; subst; 
     try (rewrite H_L_context in H_H_context; inversion H_H_context).
   - subst. + inversion H_erasure_l. subst. ++ rewrite H_H_context in H2. inversion H2. 
      ++ subst. inversion  H_erasure_r. 
      +++ subst.  
       assert (Config CT e' (SIGMA s' h') =e=>Config CT (erasure_L_fun e') (SIGMA s'1 h'1)).
       apply erasure_L_context; auto.  
   assert( e = dot \/ (e_r = dot /\ multi_step_erasure_H (Config CT e (SIGMA s h))
       (Config CT dot (SIGMA s'0 h'0))) \/ (exists e0, multi_step_erasure_H (Config CT e (SIGMA s h))
       (Config CT e0 (SIGMA s'0 h'0))  /\ e_r = (FieldAccess e0 f))).
   apply context_erasure_field_access. auto.
   destruct H0. ++++ subst. inversion H_reduction. ++++  subst. destruct H0.
    +++++ destruct H0. subst. inversion H8. subst. inversion H6. subst. inversion H1.
    inversion H7. inversion H13. 
 
   assert (Config CT e (SIGMA s h) =e=>Config CT dot (SIGMA s_r h_r)).
       apply erasure_H_context with s'0 h'0; auto.  
    inversion H_typing. subst.
    destruct IHH_reduction with h_r s_r dot (classTy clsT) h' s' e' h s e; auto. 
(*
      inversion H8. 
        ++++ subst. destruct H13.  
        +++++ subst. inversion H5. subst.  inversion H_reduction.
        +++++ inversion H5.  subst.  
        assert (multi_step_erasure_H (Config CT  e (SIGMA s h)) (Config CT e'0 (SIGMA s'0 h'0))).
        apply H_erasure_single_step; auto.  
        assert (Config CT e (SIGMA s h) =e=>
              (Config CT e'0 (SIGMA s_r h_r))). apply erasure_H_context with s'0 h'0; auto. 
        unfold erasure_L_fun. fold erasure_L_fun.
        inversion H_typing. subst.
        assert (Config CT e'0 (SIGMA s_r h_r) =
                Config CT (erasure_L_fun e') (SIGMA s'1 h'1)).
        apply IHH_reduction with (h_r:=h_r) (s_r:=s_r) (e_r:=e'0)
                  (T:=(classTy clsT)) (h'0:=h') (s'0:=s') (h0:=h) (s0:=s) (e0:=e) (e'0:=e'); auto. 
        inversion H10. subst. auto.    
      subst. inversion H_reduction.
      ++++ subst. *)

with  s_r h_r e'0 (classTy clsT) h' s' e' h s e.  with h' s' h s (SIGMA s'1 h'1)  (SIGMA s_r h_r) (classTy clsT) (FieldAccess (erasure_L_fun e'))
                                dot e e'; auto.
apply IHH_reduction. with h' s' h s (SIGMA s'1 h'1)  (SIGMA s_r h_r) (classTy clsT) (FieldAccess (erasure_L_fun e'))
                                dot e e'; auto.


        assert (Config CT (FieldAccess e f) (SIGMA s h) =eH=> Config CT (FieldAccess e'0 f) (SIGMA s'0 h'0)).
        apply Er_fieldRead1. auto. 

   }
      +++ 
assert (Config CT e (SIGMA s h) =e=> (Config CT e_r (SIGMA s'0 h'0))).
 admit.



Lemma method_call_context_erasure : forall CT e meth e2 s h e_r s_r h_r,
  e <> dot ->
  multi_step_erasure_H (Config CT (MethodCall e meth e2) (SIGMA s h))
        (Config CT e_r (SIGMA s_r h_r)) ->
  exists e0 s0 h0, multi_step_erasure_H  (Config CT (MethodCall e meth e2) (SIGMA s h))
                                  (Config CT (MethodCall e0 meth e2)  (SIGMA s0 h0)) /\
              ( ( e0 = e_r  /\ s0 = s_r /\ h0 = h_r)     
      \/      multi_step_erasure_H  (Config CT (MethodCall e0 meth e2) (SIGMA s0 h0))
                                  (Config CT (MethodCall e_r meth e2) (SIGMA s_r h_r)) ).
Proof. intros. remember (Config CT (MethodCall e meth e2) (SIGMA s h)) as config. 
remember (Config CT e_r (SIGMA s_r h_r)) as config_r. 
generalize dependent e. generalize dependent s. generalize dependent h. 
generalize dependent e_r. generalize dependent s_r. generalize dependent h_r. 
generalize dependent e2. induction H0. 
+ intros. inversion Heqconfig. inversion Heqconfig_r. subst. inversion H.
 ++ subst. 
assert (multi_step_erasure_H (Config CT e (SIGMA s h)) (Config CT e' (SIGMA s_r h_r) )).
apply H_erasure_single_step; auto. right. auto. 
destruct H0. inversion H0. auto. exists e'. exists s_r. exists h_r.  split; auto. 
destruct H0. inversion H0. 
assert (Config CT (MethodCall e meth e2) (SIGMA s h) =eH=> Config CT (MethodCall e' meth e2)  (SIGMA s_r h_r)).
apply Er_MethodCall1. auto. 

apply H_erasure_multi_step with .
 ...
 inversion H0. destruct H7. subst. inversion H3. 
    + subst. intuition. 
...
- intros. inversion Heqconfig. inversion Heqconfig'. subst. 
 ...
 inversion H_typing. subst. 
    assert ((t0 = dot \/  ( flow_to (current_label (SIGMA s'0 h'0)) L_Label = true))).
    apply erasure_final with CT (MethodCall e id0 e2) (SIGMA s h) . auto. 
    destruct H0. subst. inversion H_erasure_l. subst. rewrite H2 in H12. inversion H12. 
    subst. 
...
admit. (*apply simulation_H with t t1 (SIGMA s h) (SIGMA s0 h0) s h; auto. *)
...
Fixpoint contains_return_statement (t:tm) := 
  match t with 
    | _ => false
  end.

Theorem reduction_preserve_empty_frame : forall ct t s s' h h' lb t' sf,
field_wfe_heap ct h -> wfe_heap ct h -> wfe_stack ct h s->
      s = (cons (Labeled_frame lb sf) nil) ->
      (Config ct t (SIGMA s h)) ==> (Config ct t' (SIGMA s' h')) -> 
      forall T, has_type ct empty_context h t T ->
      contains_return_statement t = false ->
      exists lb', s' = (cons (Labeled_frame lb' sf) nil).
Proof. 
    intros ct t s s' h h' lb t' sf.  
intro H_wfe_field. intro H_wfe_heap. intro H_wfe_stack. intro H_stack.  intro H_reduction.
      intro T. intro H_typing. intro H_contains_return.
      remember (Config ct t (SIGMA s h) ) as config. 
      remember (Config ct t' (SIGMA s' h')) as config'.
      generalize dependent t.  generalize dependent t'.        generalize dependent s.       generalize dependent h.        generalize dependent T. 
      generalize dependent lb.
    induction H_reduction; intros; inversion Heqconfig; inversion Heqconfig'; subst.
    - inversion H_typing. inversion H4. 
    - inversion H_typing. apply IHH_reduction with lb (classTy clsT) h (Labeled_frame lb sf :: nil) e' e; auto. admit.
    - inversion H11. admit. 
    Admitted. 
      
     

Theorem progress_revised : forall ct t s s' h h' lb v sf,
      field_wfe_heap ct h -> wfe_heap ct h -> wfe_stack ct h s->
      s = (cons (Labeled_frame lb sf) nil) ->
      (Config ct t (SIGMA s h)) ==>* (Config ct v (SIGMA s' h')) -> 
      forall T, has_type ct empty_context h t T ->
      contains_return_statement t = false ->
      value v -> exists lb', s' = (cons (Labeled_frame lb' sf) nil).
Proof.
      intros ct t s s' h h' lb v sf.
      intro H_wfe_field. intro H_wfe_heap. intro H_wfe_stack. intro H_stack.  intro H_multi_reduction.
      intro T. intro H_typing. intro H_contains_return. intro H_v. 
      remember (Config ct t (SIGMA s h) ) as config. 
      remember (Config ct v (SIGMA s' h')) as config'.
      generalize dependent t.   generalize dependent s.       generalize dependent h.   
      generalize dependent T.   generalize dependent lb. generalize dependent sf. 
       induction H_multi_reduction. 
      intros. admit. 
      intros. induction H.
      - admit.
      - subst. inversion Heqconfig. subst. inversion H.
      - subst.  admit.
      - subst. inversion H.
          + subst.  
