...
(* return statement *)
  ...
  | ST_return1 : forall s h s' h' e e' ct,
        ...
        s <> nil ->
        Config ct (Return e) (SIGMA s h) ==> Config ct (Return e') 
                        (SIGMA ( s' ++ ( cons ( last s empty_labeled_stack_frame) nil )) h')
  (* context rule error*)
...
match s with 
  |  (Labeled_frame lb sf :: s') => if (flow_to lb L_Label) then (erasure_L_fun t) else erasure_H_fun_whole (erasure_H_fun t) s' 
  | nil => t
end. 
...
(* test cases 
Definition t := unlabelOpaque (Return (unlabelOpaque (Return ((ObjId (OID 0)))))).
...
Definition t' := unlabelOpaque (Return ((ObjId (OID 0)))).
Definition s' := (Labeled_frame H_Label empty_stack_frame :: nil).

Compute (erasure_H_fun_whole t' nil ).
Compute (erasure_H_fun_whole t' s' ).
Compute (erasure_H_fun_whole t s' ).
Compute (erasure_H_fun_whole t' s ).
Print s. 
...
-  inversion H_valid_syntax. inversion H_typing. subst.  
    apply Valid_return. 
   destruct IHH_reduction with T0 (SIGMA s' h') h0 (removelast s0) e' e; auto.
...
Lemma removelast_prop : forall l0 sf l1 s',
removelast (Labeled_frame l0 sf :: l1 :: s') =  (Labeled_frame l0 sf)  ::removelast ( l1 :: s').
Proof. induction s'. auto. auto. Qed.  

Lemma removelast_erasure_stack : forall s h, 
   removelast (erasure_stack s h) = (erasure_stack (removelast (erasure_stack s h)) h).
Proof.  induction s. auto. destruct a. intro h.
remember ((removelast (erasure_stack (Labeled_frame l0 s0 :: s) h)) ) as tmp.
unfold erasure_stack in Heqtmp. fold erasure_stack in Heqtmp. 
case_eq (flow_to l0 L_Label). intro. rewrite H in Heqtmp. 
remember (erasure_stack s h) as s'.
destruct s'. unfold removelast in Heqtmp. subst. auto. 
assert ( removelast
           (Labeled_frame l0 (fun x : id => erasure_obj_null (s0 x) h)
            :: l1 :: s') = 
           (Labeled_frame l0 (fun x : id => erasure_obj_null (s0 x) h)
            :: removelast (l1 :: s'))). apply removelast_prop. 
rewrite H0 in Heqtmp. rewrite Heqs' in Heqtmp. rewrite Heqtmp. unfold erasure_stack.
fold erasure_stack. rewrite H. destruct IHs with h. 
assert (forall a, (fun x : id => erasure_obj_null (s0 x) h) a 
            = (fun x : id => erasure_obj_null (erasure_obj_null (s0 x) h) h) a).
intro x. 
assert (erasure_obj_null (erasure_obj_null (s0 x) h) h = erasure_obj_null (s0 x) h).
 apply erasure_obj_null_equal.  auto. 
apply functional_extensionality in H1. rewrite H1. auto. 
intro. rewrite H in Heqtmp. rewrite Heqtmp. auto. 
Qed. 
...
- 
  inversion H_valid_syntax. inversion H_typing. subst.  
    apply Valid_return. 
   destruct IHH_reduction  with T0 (SIGMA s' h') (h0) (removelast (erasure_stack s0 h0)) e' e; auto.
   unfold erasure_sigma_fun. 
    assert ((removelast (erasure_stack s0 h0)) =(erasure_stack (removelast (erasure_stack s0 h0)) h0)).
    apply removelast_erasure_stack. rewrite <- H0. auto.  
-   inversion H_valid_syntax. auto. apply surface_syntax_is_valid_syntax. auto.  
- inversion H_typing. subst.  inversion H9.  inversion H5. 
- inversion H_valid_syntax. inversion H_typing. subst. 
  apply Valid_sequence1.
...
Lemma value_erasure_dot : forall v lb sf s, 
  ...
  case_eq (flow_to l0 L_Label).  intro. assert ((erasure_H_fun v) = dot).
  apply value_erasure_to_dot. auto. rewrite H2. auto. 
...
case_eq (return_free t1 ); intro; auto. Admitted. 
    ...
    Lemma erasure_L_fun_dot : forall t, 
      erasure_L_fun t = dot -> t = dot. 
    Proof. induction t; auto; intros; try (unfold   erasure_L_fun in H; fold erasure_L_fun in H;
    inversion H). case_eq (flow_to l0 L_Label); intro; rewrite H0 in H1; inversion H1. 
case_eq (flow_to l0 L_Label); intro; rewrite H0 in H1; inversion H1. Qed. 
...
Lemma field_access_erasure_dot : forall t s' i,
   ...
   Proof with eauto. intros.  generalize dependent t. induction s'. intros.  unfold erasure_H_fun_whole.  auto. 
    ...
    rewrite H1 in H2. assert (erasure_L_fun t = dot). auto. apply erasure_L_fun_dot in H3. 
    rewrite H3 in H2. intuition. 

    intros.  unfold erasure_H_fun_whole in H1. rewrite H in H1.
...
Lemma field_access_erasure_helper : forall t e' s s' i, 
    ...
    unfold erasure_H_fun_whole. rewrite H4. unfold erasure_L_fun. fold erasure_L_fun. 
    rewrite H1. auto. 
  ...
  (Labeled_frame l0 s0 :: s)  = erasure_L_fun (FieldAccess (erasure_H_fun t) i)). 
    ...
    unfold erasure_H_fun_whole. rewrite H4.   fold erasure_H_fun_whole.
    induction a0. case_eq (flow_to l1 L_Label). intro. rewrite H6 in H1.
     unfold erasure_L_fun. fold erasure_L_fun.   rewrite H1. auto .
   ...
   apply return_free_true_imply_erasure_H_fun_dot in H7. rewrite H7 in H1.
    pose proof (dot_erasure_dot s'). rewrite H8 in H1. apply erasure_L_fun_dot in H1. 
    apply erasure_H_fun_imply_return_free in H1.
    ...
    assert (erasure_H_fun_whole (FieldAccess (erasure_H_fun e') i)
                     (Labeled_frame l1 s1 :: s') = erasure_L_fun (FieldAccess (erasure_H_fun e') i)).
...
Proof. intros. generalize dependent t. generalize dependent e'. generalize dependent s'.
    ...
    rewrite H1. rewrite H2. auto.  case_eq (return_free e' ). intro. unfold return_free. 



    Lemma erasure_H_L_comm : forall t, 
      erasure_L_fun (erasure_H_fun t) =       erasure_H_fun (erasure_L_fun t). 
    Proof. induction t; auto. unfold erasure_H_fun. fold erasure_H_fun. 
    admit.     admit.     admit.     admit.     admit. 
    admit.     admit.     admit.     admit.  - unfold erasure_H_fun.  fold erasure_H_fun. 
    case_eq (return_free t). intro. unfold erasure_L_fun. fold erasure_L_fun. 
    unfold erasure_H_fun. fold erasure_H_fun.  admit. 

    intro. unfold erasure_L_fun. fold erasure_L_fun.
    unfold erasure_H_fun. fold erasure_H_fun. assert (return_free (erasure_L_fun t) = false).
     admit. rewrite H0. rewrite IHt. auto. - admit.  
    - unfold erasure_H_fun.  fold erasure_H_fun. 
    case_eq (return_free t). intro. unfold erasure_L_fun. fold erasure_L_fun.
    apply return_free_true_imply_erasure_H_fun_dot in H. rewrite H in IHt. 
    unfold erasure_L_fun in IHt.  fold erasure_L_fun in IHt. case_eq (flow_to l0 L_Label) .
    intro. unfold erasure_H_fun. fold erasure_H_fun.
    assert (erasure_H_fun (erasure_L_fun t) = dot). auto. 
    apply erasure_H_fun_imply_return_free in H1. rewrite H1. auto. admit. auto. admit. 
    intro. unfold erasure_H_fun.  fold erasure_H_fun. 
        unfold erasure_L_fun.  fold erasure_L_fun. case_eq (flow_to l0 L_Label).
    intro. unfold erasure_H_fun. fold erasure_H_fun. 
    case_eq ( return_free (erasure_L_fun t)). intro. 
    apply return_free_true_imply_erasure_H_fun_dot in H1. rewrite H1 in IHt. 
    apply erasure_L_fun_dot in IHt.  apply erasure_H_fun_imply_return_free in IHt.
    rewrite IHt in H. inversion H. admit. admit. 
    intro. rewrite IHt. auto. 
    intro. unfold erasure_H_fun. fold erasure_H_fun.  
...
Lemma erasure_H_fun_final : forall t s, 
    flow_to (current_label (SIGMA s h)) L_Label = false -> 
    t' = erasure_H_fun_whole (t s)  ->
    erasure_H_fun_whole (Return t) s = Return (erasure_H_fun_whole t s) 
    \/ 
...
pose proof (valid_syntax_after_erasure t H0). subst.    
inversion H_erasure_l. subst.
  + rewrite H_flow_l in H5. inversion H5.
  + subst.  inversion H_erasure_r. subst. 
       (* rewrite H_flow_r in H6. inversion H6. subst.  *) admit. 
  ++  inversion H_typing. subst. 
    assert (Config CT (erasure_H_fun_whole t (removelast s)) (SIGMA  s' h'0) = Config CT (erasure_H_fun_whole e' s'0)  (SIGMA s'1 h'1)).
    apply IHt with h' s'0 h (removelast s) T0  e'; auto. admit. admit. admit. 
    apply erasure_H_context; auto.  admit.    auto. apply erasure_H_context; auto.  inversion H. subst. 
 admit. 
   assert (s = (removelast s) ++ (last s empty_labeled_stack_frame :: nil)). apply app_removelast_last. auto. 
  ...
  apply valid_syntax_after_erasure. auto.
    ...
    unfold erasure_H_fun_whole in H7. unfold return_free.
    apply erasure_H_fun_imply_return_free in H7. 
    rewrite H4 in H7. inversion H4. inversion H7. auto.

      induction a.     case_eq ( flow_to l2 L_Label). intro.
    unfold erasure_H_fun_whole in H7. fold erasure_H_fun_whole in H7.  rewrite H14 in H7. 
    assert ((erasure_H_fun t) = dot). admit.  
    apply erasure_H_fun_imply_return_free in H15.  rewrite H15 in H4. inversion H4. auto.
...
Lemma helper : forall CT t e' s h s' h', 
valid_syntax t ->
forall T, has_type CT empty_context h (Return t) T -> 
field_wfe_heap CT h -> wfe_heap CT h -> wfe_stack CT h s ->
flow_to (current_label (SIGMA s h)) L_Label = false ->
      flow_to (current_label (SIGMA s' h')) L_Label = false ->
Config CT (t) (SIGMA s h) ==> Config CT (e') (SIGMA s' h') ->
(erasure_H_fun_whole t s) = (erasure_H_fun_whole e' s') ->
(erasure_H_fun_whole (Return t) s) = 
(erasure_H_fun_whole (Return e') s').
Proof. 
  intros CT t e' s h s' h'. intro H_valid_syntax.  
  intro T. intro H_typing. intro H_wfe_fields. 
  intro H_wfe_heap. intro H_wfe_stack. 
  intro H_flow_l.   intro H_flow_r. intro H_reduction. 

remember ( Config CT t (SIGMA s h) ) as config. 
remember ( Config CT e' (SIGMA s' h') ) as config'.
...
inversion H_typing. subst. 
apply IHH_reduction with h' h T0; auto. 
 admit. - admit. 
...
assert (    (erasure_H_fun_whole (erasure_H_fun (Return t)) s)  = 
    (erasure_H_fun_whole (erasure_H_fun (Return e')) s')).

assert (s =s' \/ exists lsf, s = lsf :: s' \/ exists lsf, s' = lsf :: s).
admit.  destruct H11. 

Lemma return_erasure_helper_1 : forall t e' s, 
    valid_syntax t -> valid_syntax e' ->
     return_free t = return_free e' ->
    erasure_H_fun_whole (erasure_H_fun t) s = erasure_H_fun_whole (erasure_H_fun e') s ->
    (erasure_H_fun_whole (erasure_H_fun (Return t)) s)  = 
    (erasure_H_fun_whole (erasure_H_fun (Return e')) s).
Proof.  intros. generalize dependent t. generalize dependent e'.
    induction s.  intros. generalize dependent e'.  generalize dependent t.

    intros. unfold erasure_H_fun_whole in H2. unfold erasure_H_fun_whole. 
    unfold erasure_H_fun. fold erasure_H_fun. rewrite H1. rewrite H2. auto. 

    intros. unfold erasure_H_fun_whole in H2. induction a. case_eq (flow_to l0 L_Label). intro.
     rewrite H3 in H2. unfold erasure_H_fun. fold erasure_H_fun. rewrite H1. rewrite H2. auto. 

    intro. rewrite H3 in H2.  fold erasure_H_fun_whole in H2.
    unfold erasure_H_fun_whole. fold erasure_H_fun_whole. rewrite H3.  
     unfold erasure_H_fun. fold erasure_H_fun. rewrite H1.  case_eq ( return_free e'). 
    intro. auto. intro. 
    apply IHs.  
    unfold erasure_H_fun. fold erasure_H_fun. rewrite H2. rewrite H3.  rewrite H1. auto. 
...
Lemma helper : forall e' s t, 
    valid_syntax t ->  valid_syntax e' ->
    erasure_H_fun t = erasure_H_fun_whole e' s -> return_free t  = false -> return_free e' = false ->
    (Return (erasure_H_fun_whole e' s))
    = (erasure_H_fun_whole (Return e') s).
    Proof. intros e' s t. intro H_valid_t. intro H_valid_e'. intros. generalize dependent e'.   induction s. auto. induction a. case_eq (flow_to l0 L_Label); 
    intro; unfold erasure_H_fun_whole; rewrite H; auto.
     fold erasure_H_fun_whole. intros. destruct s. auto. unfold erasure_H_fun_whole. 
     unfold erasure_H_fun. fold erasure_H_fun.  rewrite H2. auto. 

    destruct l1. case_eq (flow_to l1 L_Label).  intro. 
    unfold erasure_H_fun_whole.  fold erasure_H_fun_whole. rewrite H3. 
unfold erasure_H_fun. fold erasure_H_fun.  rewrite H2. auto. 
    remember (Return (erasure_H_fun_whole (erasure_H_fun e') (Labeled_frame l1 s1 :: s))) as tmp. 
    unfold erasure_H_fun. fold erasure_H_fun. rewrite H2.
    
    intro. 
     case_eq (return_free (erasure_H_fun e')).
    intro. unfold erasure_H_fun_whole in H1. fold erasure_H_fun_whole in H1. rewrite H3 in H1. 
    apply return_free_true_imply_erasure_H_fun_dot in H4.  rewrite H4 in H1. 
    pose proof (dot_erasure_dot s). rewrite H5 in H1. 
    apply erasure_H_fun_imply_return_free in H1. rewrite H0 in H1. inversion H1. auto. 
    apply valid_syntax_after_erasure. auto.  intro. 
    rewrite Heqtmp. apply IHs;auto.     apply valid_syntax_after_erasure. auto. 
Qed. 

intro. unfold erasure_H_fun_whole. fold erasure_H_fun_whole. rewrite H_flow_l. rewrite H_flow_r. 
...
Lemma return_erasure_helper_1 : forall t e' s, 
  ...
  Proof. intros. generalize dependent t. generalize dependent e'.
    ...
    intros. unfold erasure_H_fun_whole in H1. unfold erasure_H_fun_whole. 
    unfold erasure_H_fun. fold erasure_H_fun. rewrite H2. rewrite H3.  rewrite H1. auto. 

    intros. unfold erasure_H_fun_whole in H1. induction a. case_eq (flow_to l0 L_Label). intro.
   unfold erasure_H_fun_whole in H1.  rewrite H4 in H1. unfold erasure_H_fun_whole. rewrite H4. 
    unfold erasure_H_fun. fold erasure_H_fun. rewrite H2. rewrite H3.  rewrite H1. auto. 

    intro. rewrite H4 in H1. fold erasure_H_fun_whole in H1. 

    unfold erasure_H_fun_whole. rewrite H4. fold erasure_H_fun_whole.
    unfold erasure_H_fun. fold erasure_H_fun. rewrite H2. rewrite H3.

   case_eq (return_free (erasure_H_fun t)). intro. 
    apply return_free_true_imply_erasure_H_fun_dot in H5. rewrite H5 in H1.
    induction s.   unfold erasure_H_fun_whole in H1. 
    unfold erasure_H_fun_whole. unfold erasure_H_fun. fold erasure_H_fun.
    rewrite <- H1. rewrite H5.  
 
    apply erasure_H_fun_imply_return_free in H5. rewrite H5. 
    assert (erasure_H_fun (erasure_H_fun e') = dot). auto. 
    apply erasure_H_fun_imply_return_free in H6. rewrite H6. auto.  
    apply valid_syntax_after_erasure. auto.  
    apply valid_syntax_after_erasure. auto. 
    induction a. 
    case_eq (flow_to l1 L_Label). intro. 
    unfold erasure_H_fun_whole in H1. fold erasure_H_fun_whole in H1. rewrite H6 in H1. 
    unfold erasure_H_fun_whole. fold erasure_H_fun_whole.  rewrite H6. 
    unfold erasure_H_fun. fold erasure_H_fun. 
    apply erasure_H_fun_imply_return_free in H5. rewrite H5. 
    assert (erasure_H_fun (erasure_H_fun e') = dot). auto. 
    apply erasure_H_fun_imply_return_free in H7. rewrite H7. auto.  
    apply valid_syntax_after_erasure. auto.  
    apply valid_syntax_after_erasure. auto.

    intro.  unfold erasure_H_fun_whole in H1. fold erasure_H_fun_whole in H1. rewrite H6 in H1. 
    unfold erasure_H_fun_whole. fold erasure_H_fun_whole.  rewrite H6. 
    unfold erasure_H_fun. fold erasure_H_fun. 
...
Lemma return_erasure_helper_2 : forall t e' s s' , 
    valid_syntax t -> valid_syntax e' ->
    erasure_H_fun_whole (erasure_H_fun t) s = erasure_H_fun_whole (erasure_H_fun e') s' ->
    return_free t = false -> return_free e' = false ->
    (erasure_H_fun_whole (erasure_H_fun (Return t)) s)  = 
...
Lemma return_erasure_helper_3 : forall t e' s s' , 
    valid_syntax t -> valid_syntax e' ->
    erasure_H_fun_whole (erasure_H_fun t) s = erasure_H_fun_whole (erasure_H_fun e') s' ->
    return_free t = false -> return_free e' = false ->
    (erasure_H_fun_whole (erasure_H_fun (Return t)) s)  = 
...
Lemma return_erasure_helper_4 : forall t e' s s' , 
    valid_syntax t -> valid_syntax e' ->
    erasure_H_fun_whole (erasure_H_fun t) s = erasure_H_fun_whole (erasure_H_fun e') s' ->
    return_free t = false -> return_free e' = false ->
    (erasure_H_fun_whole (erasure_H_fun (Return t)) s)  = 
...
intros. generalize dependent t. generalize dependent e'. generalize dependent s'.
    induction s.  intros. generalize dependent e'.  generalize dependent t. induction s'.

    intros. unfold erasure_H_fun_whole in H1. unfold erasure_H_fun_whole. 
    admit. 

    intros. unfold erasure_H_fun_whole in H3. induction a. case_eq (flow_to l0 L_Label). intro.
   unfold erasure_H_fun_whole in H1.  rewrite H4 in H1. unfold erasure_H_fun_whole. rewrite H4. admit. 


    intros. unfold erasure_H_fun_whole in H1. unfold erasure_H_fun_whole.  rewrite H4 in H1. 
    rewrite H4. fold erasure_H_fun_whole in H1. fold erasure_H_fun_whole.  
     case_eq (return_free (erasure_H_fun e')).
    intro. apply return_free_true_imply_erasure_H_fun_dot in H5. rewrite H5 in H1.  
    pose proof (dot_erasure_dot s'). rewrite H6 in H1.  apply erasure_H_fun_imply_return_free in H1.
    rewrite H1 in H2. inversion H2. 
    auto. apply valid_syntax_after_erasure. auto.

    
    intro. remember (erasure_H_fun (Return t)) as tmp. 
    unfold erasure_H_fun. fold erasure_H_fun. rewrite H3. 
    assert ( erasure_H_fun_whole (erasure_H_fun (Return t)) nil =
       erasure_H_fun_whole (erasure_H_fun (Return (erasure_H_fun e'))) s').
    apply IHs'; auto.  auto. apply valid_syntax_after_erasure. auto.  rewrite Heqtmp. 
    rewrite <- H6. unfold erasure_H_fun_whole. auto. 


    intros.  induction a.    case_eq (flow_to l0 L_Label).  intro.
    unfold  erasure_H_fun_whole in H1.  rewrite H4 in H1. fold erasure_H_fun_whole in H1. 

    unfold erasure_H_fun_whole. fold erasure_H_fun_whole. rewrite H4. 

    unfold erasure_H_fun. fold erasure_H_fun.  rewrite H1. rewrite H3. rewrite H2. 
     case_eq (return_free (erasure_H_fun e')).
    intro. unfold erasure_H_fun_whole in H1. fold erasure_H_fun_whole in H1. 
    destruct s'. unfold erasure_H_fun_whole in H1. auto.  destruct l1. 
    case_eq (flow_to l1 L_Label). intro. unfold erasure_H_fun_whole. fold erasure_H_fun_whole. 
    rewrite H6. auto. intro. unfold erasure_H_fun_whole in H1.  rewrite H6 in H1. 
    fold erasure_H_fun_whole in H1.  apply return_free_true_imply_erasure_H_fun_dot in H5.
    rewrite H5 in H1. pose proof (dot_erasure_dot s'). rewrite H7 in H1. 
    apply erasure_H_fun_imply_return_free in H1. rewrite H1 in H2.  inversion H2; auto. 
    auto. apply valid_syntax_after_erasure. auto.  intro. 

    apply helper with t; auto.    
    apply valid_syntax_after_erasure. auto.  

    intro.  unfold erasure_H_fun_whole in H1. rewrite H4 in H1.  fold erasure_H_fun_whole in H1. 
    unfold erasure_H_fun_whole. fold erasure_H_fun_whole.  rewrite H4. 
    remember (erasure_H_fun_whole (erasure_H_fun (Return e')) s') as tmp. 
    unfold erasure_H_fun.  fold erasure_H_fun. rewrite  H2.  rewrite Heqtmp. 

    case_eq (return_free (erasure_H_fun t)).
        intro. unfold erasure_H_fun_whole in H1. fold erasure_H_fun_whole in H1. 
    destruct s'. apply return_free_true_imply_erasure_H_fun_dot in H5. rewrite H5 in H1.
    pose proof (dot_erasure_dot s). rewrite H6 in H1. unfold  erasure_H_fun_whole in H1.
    assert (erasure_H_fun e' = dot). auto. 
        apply erasure_H_fun_imply_return_free in H7. rewrite H7 in H3.  inversion H3; auto.
    auto.  apply valid_syntax_after_erasure. auto. 
  
  destruct l1. 
    case_eq (flow_to l1 L_Label). intro. unfold erasure_H_fun_whole. fold erasure_H_fun_whole. 
    rewrite H6. auto. unfold erasure_H_fun_whole in H1.  rewrite H6 in H1. 
    fold erasure_H_fun_whole in H1.  apply return_free_true_imply_erasure_H_fun_dot in H5.
    rewrite H5 in H1. pose proof (dot_erasure_dot s). rewrite H7 in H1. 
     assert (erasure_H_fun e' = dot). auto.  apply erasure_H_fun_imply_return_free in H8.
     rewrite H8 in H3.  inversion H3; auto. 
    auto. apply valid_syntax_after_erasure. auto.  


    intro. unfold erasure_H_fun_whole in H1. rewrite H6 in H1. fold erasure_H_fun_whole in H1.
    remember (erasure_H_fun_whole (erasure_H_fun (Return (erasure_H_fun t))) s) as tmp0. 
    unfold erasure_H_fun_whole. rewrite H6. fold erasure_H_fun_whole.  
    apply return_free_true_imply_erasure_H_fun_dot in H5. rewrite H5 in H1. 
    destruct s'. pose proof (dot_erasure_dot s).  rewrite H7 in H1. unfold erasure_H_fun_whole in H1. 
    unfold erasure_H_fun in Heqtmp0. fold erasure_H_fun in Heqtmp0. 
    apply erasure_H_fun_imply_return_free in H5. rewrite H5 in Heqtmp0. 
    unfold erasure_H_fun. fold erasure_H_fun. rewrite H3. 
    assert (erasure_H_fun (erasure_H_fun e') = dot). auto. 
    apply erasure_H_fun_imply_return_free in H8. 
        unfold erasure_H_fun. fold erasure_H_fun. rewrite H8 . admit.  
...
apply IHs; auto. 
    ...
    assert (erasure_H_fun_whole (Return (erasure_H_fun t) )
  (Labeled_frame l0 s0 :: s)  = (erasure_H_fun (Return ( t) ))). 
    unfold erasure_H_fun_whole.  rewrite H4. unfold erasure_H_fun. fold erasure_H_fun. rewrite H2. auto.
...
Qed.  
    ...
    assert (s <> nil). admit. assert (s'<> nil). admit. destruct s. intuition. destruct s'. intuition.
    destruct l2. destruct l3. 
    case_eq (flow_to l2 L_Label).     case_eq (flow_to l3 L_Label).  
    intros. 
    unfold erasure_H_fun_whole in H7. rewrite H14 in H7. rewrite H15 in H7. admit.  
    intros. 
    unfold erasure_H_fun_whole in H7. rewrite H14 in H7. rewrite H15 in H7. fold erasure_H_fun_whole in H7. 
    unfold erasure_H_fun_whole. fold erasure_H_fun_whole. rewrite H14. rewrite H15. 
    assert (return_free (erasure_H_fun e') = false). admit. 
    unfold erasure_H_fun. fold erasure_H_fun. rewrite H4.  rewrite H8. 
        unfold erasure_H_fun. fold erasure_H_fun. rewrite H16. rewrite H7. 
  
    Lemma helper : forall e' s t, 
    erasure_H_fun t =
     erasure_H_fun_whole e' s -> return_free t = false -> return_free e' = false ->
    (Return (erasure_H_fun_whole e' s))
    = (erasure_H_fun_whole (Return e') s).
    Proof. intros. generalize dependent e'.   induction s. auto. induction a. case_eq (flow_to l0 L_Label); 
    intro; unfold erasure_H_fun_whole; rewrite H; auto.
     fold erasure_H_fun_whole. intros. destruct s. auto. unfold erasure_H_fun_whole. 
     unfold erasure_H_fun. fold erasure_H_fun.  rewrite H2. auto. 
...
unfold erasure_H_fun. fold erasure_H_fun.  rewrite H2. auto. 
    ...
    intro. rewrite Heqtmp. apply IHs;auto. case_eq (return_free (erasure_H_fun e')).
    intro. unfold erasure_H_fun_whole in H1. fold erasure_H_fun_whole in H1. rewrite H3 in H1. 
    apply return_free_true_imply_erasure_H_fun_dot in H4.  rewrite H4 in H1. Admitted. 
