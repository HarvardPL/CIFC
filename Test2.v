...
(*
(*heap space is well aligned*)
Inductive well_aligned_heap : Class_table -> typing_context -> heap -> Prop :=
  | empty_heap_well_aligned : forall ct ctx, 
        well_aligned_heap ct ctx empty_heap
  | heap_well_aligned : forall h h' o  ct gamma ho,
        h' = (o , ho) :: h ->
        (h = nil \/ (exists o' ho' h'', h = (o' , ho') :: h'' /\ compare_oid o o' = true) ) ->
        well_aligned_heap ct gamma h ->
        well_aligned_heap ct gamma h'.


(* heap is well formed is every field pointing to null of  a valid object identifer *)
Inductive wfe_heap : Class_table -> typing_context -> heap -> Prop :=
  | heap_wfe : forall h o ho cls_def F lo ct gamma field_defs method_defs cls_name,
...
Lemma extend_heap_preserve_heap_wfe : forall gamma CT h h' o c field_defs method_defs lb,
  ...
  Proof. 
    ...
    apply heap_wfe with (h:=h) (o:=o) (ct:=CT) (gamma:=gamma) 
        (cls_def:=cls_def) (F:=F)  (ho:=(Heap_OBJ cls_def F lb))
...
destruct H0. rewrite H0. left. auto. destruct H0 as [o']. destruct H0 as [ho'].
right. admit.
apply IHh' with (h:=h1).



  unfold update_heap_obj. rewrite H in Hy.
  fold update_heap_obj in Hy. inversion wfe_h. inversion Hy. subst.

  apply heap_wfe with (h:=(update_heap_obj h1 o (Heap_OBJ cls (fields_update F i v) lb') )) 
               (h':= ((o1, h) :: update_heap_obj h1 o (Heap_OBJ cls (fields_update F i v) lb')))
               (o:=o1) (cls_def:=(class_def cn0 field_defs0 method_defs0) )
               (F:=F0) (ct:=CT) (gamma:=gamma) (cn0:=cn0) (ho:=h) (lo:=lo) 
                (method_defs:=method_defs0) (field_defs:=field_defs0).
  auto. inversion H0. destruct H1. rewrite H1. left. auto. 
