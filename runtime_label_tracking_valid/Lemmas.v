...
Proof.
  ...
  apply ST_val_return_obj; auto. intro contra; inversion contra. intros. intro  contra; inversion contra.

  exists (Config CT (Container null fs0 lb0 sf0) ctns h); auto.

  exists (Config CT (Container (v_opa_l  (l lb1) lb) fs0 lb0 sf0) ctns h); auto;
    apply ST_val_return_obj; auto; intro contra. inversion contra. intros. intro contra0; inversion contra0.
...
Proof with eauto.
  ...
  destruct H0.
  pose proof excluded_middle_null t H. destruct H1.
  subst; auto.
  right; auto. intro contra; inversion contra; subst; auto.
  left. auto. 
...
Lemma field_value_opaque : forall h o cls F lb f t' CT cls' gamma ell,  
    wfe_heap CT h -> field_wfe_heap CT h ->
    Some (Heap_OBJ cls F lb) = lookup_heap_obj h o ->
    F f = Some t' ->
    ...
    ( t' = null \/ exists o',  t' = ObjId o').
Proof.
  intros h o cls F lb f t' CT cls' gamma ell. intro H_wfe_heap. intro H_wfe_fields. 
  intro Hy. intro Hy_Field. intro H_typing.  inversion H_typing. inversion H2.
  inversion H12; subst; auto. 
  subst.  destruct H27 as [F']. destruct H as [lo]. rewrite H in Hy. inversion Hy. subst.
  destruct H26 as [field_defs]. destruct H0 as [method_defs].
  rewrite <- H6 in H22; inversion H22; subst; auto. 
  inversion H_wfe_fields. subst.
  destruct H0 with o (class_def clsT field_defs method_defs) F' clsT lo method_defs field_defs
                   f cls'; auto.
...
Lemma field_write_preserve_field_wfe : forall CT gamma h h' o field_defs method_defs lb lb' v i F cls_def clsT cls',
    ...
    Some cls_def = CT clsT ->
    h' = (update_heap_obj h o
           (Heap_OBJ cls_def (fields_update F i (runtime_val v)) lb')) ->
    field_wfe_heap CT h'.
Proof. 
  ...
  apply beq_oid_equal in H3. subst; auto.
  exists o. exists  (fun x' : id => if beq_id i x' then Some (runtime_val (v_opa_l (ObjId o) lb0)) else F x').
  exists lo. exists field_defs0. exists method_defs0. split; auto. 
  unfold find_fields in H2. 
  
  destruct H25 as [field_defs1].
  destruct H26 as [F1].
  ...
  intro.
  destruct H25 as [field_defs1]. destruct H9 as [method_defs1].
  destruct H26 as [F1]. destruct H20 as [lo1].

  assert (Some (Heap_OBJ cls_def F1 lo1) =
          lookup_heap_obj ((o2, h0) :: h') o3).
  rewrite H6. 
  apply lookup_updated_not_affected with o
                                         (Heap_OBJ (class_def cls_name field_defs0 method_defs0) (fun x' : id => if beq_id i x' then Some (runtime_val (v_opa_l (ObjId o3) lb0)) else F x') lo)
                                         ((o1, h) :: h1); auto.
  ...
  exists o3. 
  exists F1. exists lo1. exists field_defs1. exists method_defs1. split; auto.

  intuition.
  inversion H4; subst; auto.  inversion H15.
  inversion H4; subst; auto.  inversion H15.
  destruct H18 with v1 lb1. intuition.
  inversion H4; subst; auto.  inversion H15.
  inversion H4; subst; auto.  inversion H15.
