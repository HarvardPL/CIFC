...
(* if *)
  | T_fs_if : forall h Gamma CT T' T s1 s2 fs, 
      tm_has_type CT Gamma h ((If hole s1 s2)) T ->
      fs_has_type CT Gamma h fs (ArrowTy T T') ->
      fs_has_type CT Gamma h ((If hole s1 s2) :: fs) (ArrowTy boolTy T')
...
Inductive config_has_type : Class_table -> typing_context -> config -> ty -> Prop :=
  | T_config_nil : forall h Gamma CT T ctn,  
      ...
      config_has_type CT Gamma (Config CT ctn nil h) T
  | T_config_ctns : forall h Gamma CT T T' ctn ctns Gamma' fs lb sf, 
      ...
      config_has_type CT Gamma (Config CT ctn ((Container return_hole fs lb sf) :: ctns) h) T'
