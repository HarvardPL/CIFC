...
(* label related *)
    ...
    | opaqueCall e => if (return_free e) then dot else opaqueCall (erasure_H_fun e)
(* statements *)
    | Skip => dot
...
Lemma exclude_middle_opaque_return : (forall t, ((exists v, t = opaqueCall (Return v)) \/ (forall v, t <> opaqueCall (Return v)))).
  Proof with eauto. intro t. induction t; try (right; intro v0; intro contra; inversion contra; fail).
  pose proof (exclude_middle_return t). destruct H. destruct H as [v]. left. exists v. rewrite H. auto. 
  right. intro v. intro contra.  inversion contra. destruct H with v. auto. 
Qed. 
...
Lemma case_analysis : forall t, 
      ((forall v : tm, value v -> t <> Return v) /\ (forall v : tm, value v -> t <> opaqueCall (Return v)) ) \/
      (exists v, value v /\ t = Return v) \/ (exists v, value v /\ t = opaqueCall (Return v)).
Proof. induction t; try (left; intuition; inversion H0; fail); try (left; split; intro v; intro; intro contra; inversion contra; fail).
     destruct IHt. destruct H. left. split. intro v. intro. intro contra. inversion contra.
      intro v. intro. intro contra. inversion contra. destruct H with v; auto. 
     destruct H. destruct H as [v]. destruct H. right. right. exists v. split; auto. rewrite H0. auto. 
     destruct H as [v]. destruct H. left. split. intro v0. intro. intro contra. inversion contra. 
      intro v0. intro. intro contra. subst. inversion contra.
     destruct IHt.  destruct H. pose proof (excluded_middle_value t). destruct H1. 
     right. left. exists t. auto. left. split. intro v. intro. intro contra. inversion contra. subst. intuition.
     intro v. intro. intro contra. inversion contra. 
     destruct H. destruct H as [v]. destruct H. left. split. intro v0. intro. intro contra. subst. inversion contra.
     inversion H1; subst; try (inversion H0; fail). inversion H3. inversion H3.   
    intro v0. intro. intro contra. subst. inversion contra.
     destruct H as [v]. destruct H.  left. split. intro v0. intro. intro contra. subst. inversion contra.
     inversion H1; subst; try (inversion H0; fail). inversion H3. inversion H3.   
    intro v0. intro. intro contra. subst. inversion contra.
Qed. 
...
Proof. intros CT v s h s' h' t' sigma_l_e t0 t0' sigma_r_e.  intro T. intro H_typing.
  intro H_flow.     intro H_dot_free. intro H_v.
  intro H_wfe_fields. 
  intro H_wfe_heap. intro H_wfe_stack. intro H_reduction. 
...
- inversion H_erasure_l. 
   ...
   + inversion H_erasure_r. 
    ++ subst. 
      ...
      apply return_value_preserve_H_context with s h v t' CT; auto.   rewrite H0 in H12.  inversion H12.

    ++ subst.  
      assert (return_free (Return v) = false).
      unfold return_free. auto. unfold  erasure_H_fun. rewrite H. 
      fold  erasure_H_fun. 
      rewrite H8.       rewrite H17. unfold erasure_sigma_fun. unfold erasure_stack. fold erasure_stack.
      inversion H_reduction. inversion H_v; subst; inversion H2. subst. 
      induction lsf. inversion H9. subst. inversion H15. subst. 
      unfold erasure_stack. fold erasure_stack. rename s'2 into s'.  
      assert (Config CT (Return dot)
                      (SIGMA (Labeled_frame l0 (fun x : id => erasure_obj_null (s0 x) h0) :: s')
                         (erasure_heap h0)) 
          ==> (Config CT dot (SIGMA (update_current_label s' (join_label l0 (get_current_label s'))) (erasure_heap h0)))).
      apply ST_return2 with (Labeled_frame l0 (fun x : id => erasure_obj_null (s0 x) h0) :: s') s'
                        (update_current_label s' (join_label l0 (get_current_label s'))) 
                        (erasure_heap h0) (Labeled_frame l0 (fun x : id => erasure_obj_null (s0 x) h0)) 
                        (join_label l0 (get_current_label s')); auto. auto.
      assert ((Config CT dot (SIGMA (update_current_label s' (join_label l0 (get_current_label s'))) (erasure_heap h0)))  =e=> 
                        Config CT (erasure_H_fun t')
                          (SIGMA  (erasure_stack
                                (update_current_label s' (join_label l0 (get_current_label s'))) h0)
                                    (erasure_heap h0))).  
      apply erasure_H_context; auto.  apply value_erasure_to_dot in H7. rewrite H7. 
      unfold erasure_H_fun. auto. unfold erasure_sigma_fun.
      remember (update_current_label s' (join_label l0 (get_current_label s'))) as s''.
       assert (  (erasure_stack s'' h0)  = (erasure_stack s'' (erasure_heap h0))).
      apply multi_erasure_stack_helper with CT. auto. rewrite H2.
      pose proof (multi_erasure_heap_equal h0). rewrite H4. auto. 
...
Lemma simulation_opaque_return_v : forall CT v s h s' h' t' sigma_l_e t0 t0' sigma_r_e, 
      forall T, has_type CT empty_context h v T -> 
      flow_to (current_label (SIGMA s h)) L_Label = false ->
      dot_free v = true->
      value v->
      field_wfe_heap CT h -> wfe_heap CT h -> wfe_stack CT h s ->
      Config CT (opaqueCall (Return v)) (SIGMA s h) ==> Config CT t' (SIGMA s' h') ->
      (Config CT (opaqueCall (Return v)) (SIGMA s h)) =e=> (Config CT t0 sigma_l_e) ->
      (Config CT t' (SIGMA s' h')) =e=> (Config CT t0' sigma_r_e) ->
      l_reduction (Config CT t0 sigma_l_e) (Config CT t0' sigma_r_e).
Proof. intros CT v s h s' h' t' sigma_l_e t0 t0' sigma_r_e.  intro T. intro H_typing.
  intro H_flow.     intro H_dot_free. intro H_v.
...
- inversion H_erasure_l. 
   ...
   + inversion H_erasure_r. 
    subst. assert (return_free (Return v) = false).
      unfold return_free. auto. unfold  erasure_H_fun. rewrite H. 
      fold  erasure_H_fun. 
      rewrite H0. unfold erasure_L_fun. fold erasure_L_fun. rewrite  H8.
      rewrite H17. unfold erasure_sigma_fun. unfold erasure_stack. fold erasure_stack.
      inversion H_reduction; try (destruct H4 with v; auto; fail). inversion H_v; subst; try (inversion H2). subst.
      inversion H4. subst. inversion H7. inversion H13. subst.  induction lsf. rename s'2 into s'.  
      ...
      assert (Config CT (v_opa_l dot l0) (SIGMA s' (erasure_heap h0)) =e=> 
            (Config CT (v_opa_l dot (current_label (SIGMA (Labeled_frame l0 s :: s') h0)))
                      (SIGMA s'1 h'1))).
      unfold current_label. unfold get_stack. unfold get_current_label. unfold get_stack_label.
      apply erasure_L_context; auto.
      unfold erasure_L_fun. case_eq (flow_to l0 L_Label); auto. auto.  
      rewrite H17. unfold erasure_sigma_fun.
      assert (  (erasure_stack s' h0)  = (erasure_stack s' (erasure_heap h0))).
      apply multi_erasure_stack_helper with CT. auto. rewrite H2.
      pose proof (multi_erasure_heap_equal h0). rewrite H4. auto.
      assert (  (erasure_stack s' h0)  = (erasure_stack s' (erasure_heap h0))).
      ...
      rewrite H17 in H2. unfold erasure_sigma_fun in H2.  auto. 
      unfold erasure_L_fun. rewrite H_flow. auto. subst.       
    ++ subst.  
      assert (return_free (Return v) = false).
      unfold return_free. auto. unfold  erasure_H_fun. rewrite H. 
      fold  erasure_H_fun. 
      ...
      apply erasure_H_context; auto. unfold erasure_sigma_fun. unfold erasure_sigma_fun in H17. 
      rewrite H17. 
      assert (  (erasure_stack s' h0)  = (erasure_stack s' (erasure_heap h0))).
      apply multi_erasure_stack_helper with CT. auto. rewrite H2.
      pose proof (multi_erasure_heap_equal h0). rewrite H4. auto.
      apply L_reduction with (Config CT (v_opa_l dot l0) (SIGMA s' (erasure_heap h0))); auto.
      unfold   erasure_sigma_fun in H17. rewrite <- H17. auto.  
      unfold current_label. unfold get_current_label. unfold get_stack. 
      unfold get_stack_label. unfold erasure_H_fun. fold erasure_H_fun. rewrite H. auto.
Qed. 
...
Proof with eauto. induction t; intros; try (unfold  erasure_H_fun; auto; fail); 
    ...
    apply T_v_l. auto. apply IHt.  auto. apply erasure_H_fun_preserve_value. auto. 
...
Proof.
    ...
    apply multi_step_concat with (Config c t1_r s0_r); auto. 
...
(*-----------------------------------------------------------------------------------------------*)
    ...
    Lemma beq_oid_equal : forall o o0, 
      beq_oid o o0 = true -> o = o0. 
    Proof. intros. induction o. induction o0. f_equal. 
    induction n. induction n0. auto. simpl in H. inversion H. simpl in IHn. induction n0. simpl in H. inversion H.
   simpl in H. case_eq (n =? n0). intro. 

 simpl in H.
    case_eq (n =? n0). intro.  rewrite H0 in IHn. assert (n = n0).  apply IHn.  auto. 
    simpl in H. rewrite <- H1 in H. induction n. inversion H.  case n.  
 ...
 (erasure_heap h) 
    ...
    destruct  IHt1 with h' s' h s (SIGMA s'1 h'1) (SIGMA  s'0 h'0)  (classTy clsT) (erasure_H_fun e1') (erasure_H_fun t1) e1'; auto.s
    inversion H_valid_syntax. auto. apply value_is_valid_syntax. auto. apply erasure_H_context; auto. 
    apply erasure_H_context; auto.  
...
Proof. intros. 
  ...
  unfold  erasure_H_fun in H.
assert (return_free e = true \/ return_free e = false). apply exclude_middle_return_t. destruct H1. 
rewrite H1 in H. 

  assert ((e' = Return dot \/  ( flow_to (current_label (SIGMA s' h')) L_Label = true))).
